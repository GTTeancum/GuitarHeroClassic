...
- 2026-07-15 source refresh after fetch: `rb3` still resolves both `HEAD` and
  ...
  A 2026-07-14/15 reread of the RB2 dump tightens that fence: the generic
  `CharBones::ScaleAdd()` range names the expected compression/local buffers
  (`ShortVector3`, `ByteQuat`, `ShortQuat`, and angle weights) but still does
  not expose a portable statement body; the
  `CharBones::ScaleAdd(class CharBones * const this)` overload row is literally
  an empty/bodyless `{}` row, and the sample-index
...
`support_viewer=6`, `stock_total=24`, `individual_proofs=true`, and
`linked_proof_pngs=true`. That audit ties the 18 in-game / viewer live-stack
playable proofs in `tools/arm_pose_diff_manifest.json` to the six support/base
direct-viewer output proofs in `tools/charbone_output_map_manifest.json`,
requires screenshot-bearing logs and individual PNG visual proofs rather than
contact sheets, requires each playable proof PNG to be explicitly linked from
the manifest or to be the same-stem companion of its manifest log, enforces each
manifest case's required log fragments before accepting coverage, requires every
linked proof PNG to be at least `1280x720`, and now parses both in-game and
viewer final playable `armw` rows to require leg-chain sanity across all 18
playable cases: knees below thighs, ankles below knees, toes not above ankles,
and at least one toe in a near-floor band. The current stock run reports
