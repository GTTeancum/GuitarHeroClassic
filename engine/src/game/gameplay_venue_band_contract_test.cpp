...
int main() {
  ...
  ok &= contains(force_camera_shot_c,
                 "source_msg=force_shot"
                 "source_handle=HANDLE_ACTION",
                 "camera force-shot diagnostics expose ihatecompvir HANDLE_ACTION routing");
  ok &= contains(force_camera_shot_c,
  ...
  ok &= contains(force_camera_shot_c,
                 ...
                 "camera force-shot diagnostics expose HANDLE_ACTION returning DataNode(0)");
  ok &= contains(force_camera_shot_c,
  ...
  ok &= contains(gameplay_c,
                 ...
                 "source CheckShotOver status mirrors the recovered CamShot gate");
  ok &= absent(gameplay_c,
               "bandcam_setframeex_guard",
               "normal gameplay CheckShotOver diagnostics do not claim a BandCamShot SetFrameEx guard");
  ok &= absent(gameplay_c,
               "before_bandcam_min_frame",
               "normal gameplay CheckShotOver diagnostics do not claim a BandCamShot minimum-frame gate");
  ok &= contains(gameplay_c,
                 "\"mShotOver_latched\"",
  ...
  ok &= contains(gameplay_c,
                 "source_check=CamShot::CheckShotStarted",
                 "shot_started bridge documents the source runtime CamShot bit");
  ok &= contains(gameplay_h_c,
  ...
  ok &= contains(gameplay_c,
                 ...
                 "source_current_handle=HANDLE_EXPRsource_current_return=%s"
                 "source_next_handle=HANDLE_EXPR"
                 "source_next_before_return=%ssource_next_after_return=%s"
  ...
  ok &= contains(gameplay_c,
                 ...
                 "source_next_handle=HANDLE_EXPR"
                 "source_next_before_return=%ssource_next_after_return=%s",
                 "regular camera sweep diagnostics expose HANDLE_EXPR current_shot/next_shot returns");
  ok &= contains(gameplay_c,
  ...
  ok &= contains(gameplay_c,
                 ...
                 "camera_manager_iterate_shot_bridge,"
                 "camera_manager_randomize_category_noop,",
                 "camera implementation status keeps source CameraManager same-shot restart, ForceCameraShot action return, ShotAfter, ShotAfter HANDLE_EXPR return, cycle_shot pending and return, current/next HANDLE_EXPR returns, iterate_shot, and current_shot/next_shot state beside pending Pick/Force handoffs");
  ok &= contains(gameplay_c,
                 "camera_manager_force_shot_pending,"
                 "camera_manager_force_shot_action_return_bridge,"
                 "camera_manager_shotafter_order_bridge,"
                 ...
                 "camera_manager_cycle_shot_return_bridge,"
                 "camera_manager_current_next_state,",
                 "camera implementation status keeps ForceCameraShot action return and ShotAfter object-order and HANDLE_EXPR return bridges beside OnCycleShot pending/return bridges between ForceCameraShot and current/next state accounting");
  ok &= contains(gameplay_c,
                 "camera_manager_shotafter_order_bridge,"
                 "camera_manager_shotafter_expr_return_bridge,"
                 "camera_manager_cycle_shot_pending_bridge,",
                 "camera implementation status keeps CameraManager::ShotAfter object-order and HANDLE_EXPR return immediately before cycle_shot pending accounting");
  ok &= contains(gameplay_c,
                 "camera_manager_cycle_shot_pending_bridge,"
                 "camera_manager_cycle_shot_return_bridge,"
                 "camera_manager_current_next_state,",
  ...
  ok &= contains(gameplay_c,
                 ...
                 "camera_manager_current_next_expr_return_bridge,"
                 "camera_manager_iterate_shot_bridge,",
                 "camera implementation status keeps current/next HANDLE_EXPR return accounting before OnIterateShot");
  ok &= contains(gameplay_c,
                 "camera_manager_current_next_state,"
                 "camera_manager_current_next_expr_return_bridge,"
                 "camera_manager_iterate_shot_bridge,"
                 "camera_manager_randomize_category_noop,",
                 "camera implementation status keeps OnIterateShot bucket walk accounting between current/next expression returns and RandomizeCategory no-op");
  ok &= contains(gameplay_c,
                 "camera_manager_shotmatches_filters,"
                 "camera_camshot_radio_flags_bridge,"
                 "camera_manager_findshot_moveitem,",
  ...
  ok &= contains(gameplay_c,
                 ...
                 "camera_camshot_setpreframe_noop,"
                 "camera_manager_calcframe_units,",
                 "camera implementation status keeps base CamShot SetPreFrame no-op between manager PrePoll/Poll order and CalcFrame timing");
  ok &= contains(gameplay_c,
                 "camera_camshot_setpreframe_noop,"
                 "camera_manager_calcframe_units,"
                 "camera_camshot_update_target_cache,",
  ...
  ok &= contains(gameplay_c,
                 ...
                 "source shot_over diagnostics expose the CamShot CheckShotOver gate before the handler fires");
  ok &= contains(gameplay_c,
                 "source_expr=!mShotOver&&!mLooping&&frame>=mDuration",
                 "source shot_over gate diagnostics name the recovered CamShot expression");
  ok &= contains(gameplay_c,
  ...
  ok &= contains(gameplay_c,
                 ...
                 "source_handle=HANDLE_EXPR"
                 "source_manager=CameraManager::ShotAfter",
                 "camera diagnostics expose source ShotAfter HANDLE_EXPR routing");
  ok &= contains(gameplay_c,
                 "source_manager=CameraManager::ShotAfter"
                 "current=%scount=0result=nonesource_return=DataNode(0)",
                 "camera ShotAfter empty branch exposes source HANDLE_EXPR DataNode(0) return");
  ok &= contains(gameplay_c,
                 "source_manager=CameraManager::ShotAfter"
                 "current=%scurrent_found=%dfirst=%safter=%safter_order=%zu"
                 "object_order=ObjDirItrresult=%ssource_return=DataNode(CamShot)",
                 "camera ShotAfter object branch exposes source HANDLE_EXPR CamShot return");
  ok &= contains(gameplay_c,
                 "camera_manager_shotafter_order_bridge,",
                 "camera implementation status counts the source CameraManager::ShotAfter object-order bridge");
  ok &= contains(gameplay_c,
                 "camera_manager_shotafter_expr_return_bridge,",
                 "camera implementation status counts the source CameraManager::ShotAfter HANDLE_EXPR return bridge");
  ok &= contains(gameplay_c,
  ...
  ok &= contains(gameplay_c,
                 "camera_manager_iterate_shot_bridge,",
                 "camera implementation status counts the source CameraManager::OnIterateShot bridge");
  ok &= contains(iterate_camera_shots_c,
                 "std::stable_sort(source_ordered.begin(),source_ordered.end(),",
