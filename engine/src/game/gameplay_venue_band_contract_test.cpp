...
int main() {
  ...
  ok &= contains(gameplay_c,
                 ...
                 "camera_lifecycle",
                 "camera implementation status counts source CamShot PlatformOk gating, SyncObjects category buckets, CameraManager random seed, one_bar_to seek-latch replay, worldbase beat check_shot bridge, worldbase script filters, CameraManager MakeCategoryAndFilters, BandDirector FindNextShot filters, FirstShotOk, ShotMatches filters, CamShot radio flag mutation, FindCameraShot MoveItem rotation, PickCameraShot pending handoff, ForceCameraShot pending handoff, current/next shot state, RandomizeCategory no-op, and WorldDir SyncCamShots disabled overrides between selection and lifecycle");
  ok &= contains(gameplay_c,
                 "camera_camshot_platform_ok_gate,"
                 "camera_manager_syncobjects_category_buckets,"
                 "camera_manager_random_seed_bridge,"
  ...
  ok &= contains(gameplay_c,
                 ...
                 "camera_manager_first_shot_ok_hook,"
                 "camera_manager_shotmatches_filters,",
                 "camera implementation status keeps worldbase script filters before MakeCategoryAndFilters and the CameraManager FirstShotOk/ShotMatches scan");
  ok &= contains(gameplay_c,
                 "camera_manager_pickshot_pending,"
                 "camera_manager_force_shot_pending,"
                 "camera_manager_current_next_state,"
