...
int main() {
  ...
  ok &= contains(gameplay_c,
                 "FoFiXsessioneventtype=%st=%.3fmask=0x%02xgems=%dpts=%dsource=%zutick=%urock=%.4fsp=%.4ffail=%d",
                 "FoFiX session event logs include mask gauges score delta source note and MIDI tick");
  ok &= contains(gameplay_c,
                 "++hit_count_;",
