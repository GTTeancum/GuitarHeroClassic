...
int main(int argc, char** argv) {
  ...
  std::printf("ghogx_ui_test: loaded %d UI script DTBs, %zu objects registered\n",
              n, mgr.registry().size());
  CHECK(n >= 35);                      // ~40 ui/gen DTBs
  CHECK(mgr.registry().size() > 100);  // ~180 screen/panel objects
  CHECK(mgr.resolve_function(Symbol("autosave_goto")) != nullptr);
  ...
  bool heard_select = false;
  for (const std::string& event : mgr.audio_events())
    heard_select = heard_select || event == "play_sequence:button_select";
  CHECK(heard_select);
  mgr.run_global_handler(Symbol("SCREEN_BACK_MSG"));
  bool heard_back = false;
  for (const std::string& event : mgr.audio_events())
    heard_back = heard_back || event == "play_sequence:button_back.cue";
  CHECK(heard_back);
  ...
  mgr.update(mgr.transition_time() + 0.01f);
  CHECK(!mgr.in_transition());
  if (Object* qp_screen = mgr.current_screen()) {
    qp_screen->handle_property(Symbol("backwards_anim"), DataArray());
    mgr.goto_screen(Symbol("main_screen"));
    CHECK(mgr.in_transition());
    CHECK(mgr.transition_is_back());
    mgr.update(mgr.transition_time() + 0.01f);
    CHECK(!mgr.in_transition());
  } else {
    CHECK(false);
  ...
  // Stock DTBs mark loading/pause-style screens `(animate_transition FALSE)`.
  // Entering them should complete the screen protocol immediately, and loading
  // screens should remain inspectable here instead of firing their gameplay
  // handoff `TRANSITION_COMPLETE_MSG`.
  if (Object* loading_screen = mgr.find_object(Symbol("loading_screen"))) {
    CHECK(!loading_screen->get_property(Symbol("animate_transition")).empty());
    mgr.goto_screen(Symbol("main_screen"));
    mgr.update(mgr.transition_time() + 0.01f);
  ...
  if (Object* pause_screen = mgr.find_object(Symbol("pause_screen"))) {
    CHECK(!pause_screen->get_property(Symbol("animate_transition")).empty());
    mgr.goto_screen(Symbol("main_screen"));
    mgr.update(mgr.transition_time() + 0.01f);
  ...
  if (Object* audio_screen = mgr.find_object(Symbol("audio_settings_screen"))) {
    ...
    audio_screen->handle_property(Symbol("BUTTON_DOWN_MSG"), DataArray());
    CHECK(mgr.in_transition());
    CHECK(mgr.transition_is_back());
    mgr.update(mgr.transition_time() + 0.01f);
    CHECK(mgr.current_screen() != nullptr &&
