...
void load_ui_objects(const NodeList& roots, ScreenManager& mgr) {
  for (const auto& rp : roots) {
    ...
    if (r.tag == 0x10) {
      ...
      if (!k.empty() && gh::dtb::is_string_like(*k[0]) && entry_is_handler(k)) {
        mgr.add_function(Symbol(nstr(*k[0])), rp);
      }
      continue;
    }
...
int load_all_ui_screens(const gh::ark::ArkV3Reader& ark,
  ...
  const std::string system_script = "config/gen/system_script.dtb";
  ...
  if (ark.find(system_script)) collect_macros_one_dtb(ark, ark_paths, system_script, &macros);
  ...
  if (ark.find(system_script)) { load_one_dtb(ark, ark_paths, system_script, mgr, &macros); ++n; }
