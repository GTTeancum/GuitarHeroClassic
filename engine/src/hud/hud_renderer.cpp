...
struct HudMatAnimVec3Key {
  float value[3] = {0.0f, 0.0f, 0.0f};
  float frame = 0.0f;
};

struct HudMatAnimFloatKey {
  float value = 0.0f;
  float frame = 0.0f;
};
...
struct HudMatAnimColorCurve {
  ...
  std::vector<HudMatAnimVec3Key> tex_translation_keys;
  std::vector<HudMatAnimVec3Key> tex_scale_keys;
  std::vector<HudMatAnimFloatKey> tex_rotation_keys;
...
std::optional<HudMatAnimColorCurve> decode_mat_anim_color_curve(
  ...
  if (!read_u32_advance(body, size, pos, trans_count) || trans_count > 256) {
    return std::nullopt;
  }
  curve.tex_translation_keys.reserve(trans_count);
  for (uint32_t i = 0; i < trans_count; ++i) {
    HudMatAnimVec3Key key;
    ...
    if (!std::isfinite(key.frame)) key.frame = 0.0f;
    curve.duration_frames = std::max(curve.duration_frames, key.frame);
    curve.tex_translation_keys.push_back(key);
  }
  uint32_t scale_count = 0;
  if (!read_u32_advance(body, size, pos, scale_count) || scale_count > 256) {
    return std::nullopt;
  }
  curve.tex_scale_keys.reserve(scale_count);
  for (uint32_t i = 0; i < scale_count; ++i) {
    ...
    if (!std::isfinite(key.frame)) key.frame = 0.0f;
    curve.duration_frames = std::max(curve.duration_frames, key.frame);
    curve.tex_scale_keys.push_back(key);
  }
  uint32_t rotation_count = 0;
  if (!read_u32_advance(body, size, pos, rotation_count) ||
      rotation_count > 256) {
    return std::nullopt;
  }
  curve.tex_rotation_keys.reserve(rotation_count);
  for (uint32_t i = 0; i < rotation_count; ++i) {
    HudMatAnimFloatKey key;
    ...
    if (!std::isfinite(key.frame)) key.frame = 0.0f;
    curve.duration_frames = std::max(curve.duration_frames, key.frame);
    curve.tex_rotation_keys.push_back(key);
  }
  ...
  if (curve.keys.empty() && curve.alpha_keys.empty() &&
      curve.tex_translation_keys.empty() && curve.tex_scale_keys.empty() &&
      curve.tex_rotation_keys.empty() && curve.texture_keys.empty()) {
    return std::nullopt;
  }
  return curve;
}
...
void HudRenderer::clear_loaded_resources() {
  ...
  star_path_tex_translation_keys_.clear();
...
bool HudRenderer::load(IDirect3DDevice9* dev, const std::string& hdr_path,
  ...
  auto copy_star_tex_translation_keys =
      [&](const char* anim_name, std::vector<Vec3AnimKey>& dst,
          float& duration_frames) {
        dst.clear();
        duration_frames = 100.0f;
        const auto it = star.mat_anim_color.find(anim_name);
        if (it == star.mat_anim_color.end()) return;
        duration_frames = std::max(1.0f, it->second.duration_frames);
        dst.reserve(it->second.tex_translation_keys.size());
        for (const HudMatAnimVec3Key& src : it->second.tex_translation_keys) {
          Vec3AnimKey key;
          key.x = src.value[0];
          key.y = src.value[1];
          key.z = src.value[2];
  ...
  for (size_t draw_index : draw_order) {
    ...
    if (q.tex) {
      ...
      dev->SetTextureStageState(
          0, D3DTSS_COLORARG1,
          q.prelit_alpha_emission ? (D3DTA_TEXTURE | D3DTA_ALPHAREPLICATE)
                                  : D3DTA_TEXTURE);
      dev->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_DIFFUSE);
    ...
    for (uint16_t id : q.idx) {
      ...
      const D3DCOLOR diffuse = q.color;
...
void HudRenderer::emit_star_power(std::vector<Quad>& out, float fill,
  ...
  const uint32_t sampled_fill_core_color =
      fill_core_color.value_or(!native_star_fill_.empty()
                                   ...
                                   : 0x00000000u);
  const uint32_t filter_fill_core_color =
      star_fill_color_keys_.empty()
          ? sampled_fill_core_color
          : sample_hud_mat_anim_color_frame(star_fill_color_keys_,
                                            fill_anim_frame);
  const float filter_tube_meter_alpha = sample_hud_mat_anim_alpha_frame(
      star_tube_meter_alpha_keys_, tube_meter_anim_frame);
  ...
  auto source_x_range = [](const std::vector<Quad>& layers) {
    ...
    for (const Quad& q : layers) {
      for (const Quad::V& v : q.verts) {
        ...
        range.min_z = std::min(range.min_z, v.wz);
        range.max_z = std::max(range.max_z, v.wz);
        any = true;
      }
    }
  ...
  StarClipRange stored_fill_range = core_fill_range;
  if (tube_meter_range.ok) {
    stored_fill_range.min_x = tube_meter_range.min_x;
    stored_fill_range.max_x = tube_meter_range.max_x;
    stored_fill_range.ok = true;
  }
  ...
  auto append_clipped_quad =
      [&](const Quad& src, const std::optional<uint32_t>& color_override,
        ...
        auto inside_x = [&](const Quad::V& v) { return v.wx >= clip_x; };
        auto intersect_x_at = [](const Quad::V& a, const Quad::V& b,
                                 float target_x) {
          const float denom = b.wx - a.wx;
          const float t =
              std::abs(denom) < 0.00001f ? 0.0f : (target_x - a.wx) / denom;
          Quad::V out_v;
          out_v.wx = target_x;
        ...
        auto clip_polygon =
            [](const std::vector<Quad::V>& in_poly, const auto& inside_fn,
               const auto& intersect_fn) {
              std::vector<Quad::V> out_poly;
              if (in_poly.empty()) return out_poly;
              out_poly.reserve(in_poly.size() + 2);
              for (size_t j = 0; j < in_poly.size(); ++j) {
                const Quad::V& a = in_poly[j];
                const Quad::V& b = in_poly[(j + 1) % in_poly.size()];
                const bool a_in = inside_fn(a);
                const bool b_in = inside_fn(b);
                if (a_in && b_in) {
                  out_poly.push_back(b);
                } else if (a_in && !b_in) {
                  out_poly.push_back(intersect_fn(a, b));
                } else if (!a_in && b_in) {
                  out_poly.push_back(intersect_fn(a, b));
                  out_poly.push_back(b);
                }
        ...
        auto intersect_z = [](const Quad::V& a, const Quad::V& b,
                              float clip_z) {
          const float denom = b.wz - a.wz;
          const float t =
              std::abs(denom) < 0.00001f ? 0.0f : (clip_z - a.wz) / denom;
          Quad::V out_v;
          out_v.wx = a.wx + (b.wx - a.wx) * t;
          out_v.wy = a.wy + (b.wy - a.wy) * t;
          out_v.wz = clip_z;
          out_v.u = a.u + (b.u - a.u) * t;
          out_v.v = a.v + (b.v - a.v) * t;
        ...
        clipped.prelit_alpha_emission = src.prelit_alpha_emission;
        ...
        for (size_t i = 0; i + 2 < src.idx.size(); i += 3) {
          ...
          std::vector<Quad::V> out_poly =
              clip_polygon(poly, inside_x,
                           [&](const Quad::V& a, const Quad::V& b) {
                             return intersect_x_at(a, b, clip_x);
                           });
          out_poly = clip_polygon(
              out_poly, [&](const Quad::V& v) { return v.wx <= max_x; },
              [&](const Quad::V& a, const Quad::V& b) {
                return intersect_x_at(a, b, max_x);
              });
          if (containment_range && containment_range->ok &&
              containment_range->max_z > containment_range->min_z) {
            const float min_z = containment_range->min_z;
            const float max_z = containment_range->max_z;
            out_poly = clip_polygon(
                out_poly, [&](const Quad::V& v) { return v.wz >= min_z; },
                [&](const Quad::V& a, const Quad::V& b) {
                  return intersect_z(a, b, min_z);
                });
            out_poly = clip_polygon(
                out_poly, [&](const Quad::V& v) { return v.wz <= max_z; },
                [&](const Quad::V& a, const Quad::V& b) {
                  return intersect_z(a, b, max_z);
                });
          }
  ...
  auto append_contained_meter_glow =
      [&](const std::vector<Quad>& source, float alpha_scale,
          const StarClipRange& source_range,
          const StarClipRange& tube_range) {
        if (!source_range.ok || !tube_range.ok ||
            !(source_range.max_x > source_range.min_x) ||
            !(source_range.max_z > source_range.min_z) ||
            !(tube_range.max_x > tube_range.min_x) ||
            !(tube_range.max_z > tube_range.min_z)) {
          return false;
        }

        // This is deliberately a visual band-aid, not a retail-behavior
        // claim. Keep the reconstructed broad glow centered on the permanent
        // tube and reveal it along the tube's axis. A small symmetric vertical
        // pad preserves a halo without allowing the glow to detach below it.
        StarClipRange glow_bounds = tube_range;
        const float tube_height = tube_range.max_z - tube_range.min_z;
        glow_bounds.min_z -= tube_height * 0.15f;
        glow_bounds.max_z += tube_height * 0.15f;

        bool drew = false;
        for (const Quad& src : source) {
          Quad aligned = src;
          for (Quad::V& v : aligned.verts) {
            const float tx = std::clamp(
                (v.wx - source_range.min_x) /
                    (source_range.max_x - source_range.min_x),
                0.0f, 1.0f);
            const float tz = std::clamp(
                (v.wz - source_range.min_z) /
                    (source_range.max_z - source_range.min_z),
                0.0f, 1.0f);
            v.wx = tube_range.min_x +
                   tx * (tube_range.max_x - tube_range.min_x);
            v.wz = glow_bounds.min_z +
                   tz * (glow_bounds.max_z - glow_bounds.min_z);
          }
          drew |= append_clipped_quad(aligned, std::nullopt, nullptr,
                                      alpha_scale, tube_range, &glow_bounds);
        }
        return drew;
      };
  ...
  if (fill > 0.005f && meter_fill_glow &&
    ...
    drew_native_fill_glow =
        append_contained_meter_glow(native_star_fill_glow_, tube_meter_alpha,
                                    tube_meter_range, core_fill_range);
  ...
  if (env_enabled("GHOGX_DEBUG_HUD_STAR_POWER") &&
    ...
    std::fprintf(
        ...
        "wide_fill_layer=amp_tube_glow_meter.mesh tube_aligned_band_aid "
    ...
    std::fprintf(
        ...
        "core_filter_frame=%.2f core_filter_color=%08x "
        "core_lit_frame=%.2f core_lit_color=%08x "
        "tube_filter_frame=%.2f tube_filter_alpha=%.3f "
        "tube_render_frame=%.2f tube_render_alpha=%.3f "
        "render_core_color=%08x render_tube_alpha=%.3f\n",
        fill, fill_anim_frame, filter_fill_core_color,
        fill_core_color_frame, sampled_fill_core_color,
        tube_meter_anim_frame, filter_tube_meter_alpha,
