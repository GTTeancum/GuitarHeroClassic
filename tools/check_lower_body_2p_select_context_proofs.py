#!/usr/bin/env python3
"""Verify the 2P select lower-body context proof batch.

This is the corrected character-select visual oracle: the stock two-player
select event, both 2P placers, two frames about ten frames apart, and both
...
class ProofCase:
    ...
    def stem(self) -> str:
        return (
            ...
            f"f{self.frame:03d}_{self.view}"
        )
...
def require_text(text: str, fragment: str, path: Path, label: str) -> None:
    ...
    require(
        compact_fragment in compact_text,
        f"{path}: missing {label}: {fragment}",
    )
...
def main() -> int:
    root = Path("engine/out/visual_proofs/lower_body_2p_select_context_20260715")
    cases = make_cases()
    try:
        require(
            tuple(case.stem for case in cases) == EXPECTED_PROOF_STEMS,
            "2P select context proof stems changed",
        )
