#!/usr/bin/env python3
"""Verify Metal1 UI/select flat-foot lower-body proof screenshots."""
...
def check_case(root: Path, case: ProofCase) -> CaseMetrics:
    ...
    require(log_path.is_file(), f"{case.view}: missing log {log_path}")
    require_png(png_path)

    text = read_log_text(log_path)
    require_text(
        text,
        "[char] char/metal1/og/gen/metal1_ui.milo_ps2:",
        log_path,
        "Metal1 UI character source",
    )
    require_text(text, "[char] loaded 'metal1'", log_path, "character load")
    require_text(
        text,
        "[clip] 'ui_loop' from char/metal1/anims/gen/metal1_ui.milo_ps2",
        log_path,
        "Metal1 UI loop source clip",
    )
    require_text(text, "[char] reference base enabled", log_path, "reference base")
    require_text(text, "[char] clip-frame override enabled: 30", log_path, "frame")
    ...
    def visible_world(bone: str) -> tuple[float, float, float]:
        row = visible_rows.get(("metal1", "lower-output", bone))
...
def main() -> int:
    root = Path("engine/out/visual_proofs/lower_body_metal1_ui_select_20260715")
    try:
