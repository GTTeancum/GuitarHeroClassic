...
## 2026-08-25 r221 frame 14 maximum-head-local pass

- Captured and inspected exactly one image at outfit-1 attack frame 14/yaw 225
  and camera distance 135 using r221 main, r207 fret, and r212 long strum.
  Visual verdict: pass the maximum head-local gate. The head turn remains
  natural, the neck stays seated, and the corrected torso remains upright.
  Evidence: `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_f014_yaw225_proofs/`
  and `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_f014_yaw225_pose_review_manifest.json`.
- Model, textures, all three layers, framing, screenshot, and command checks
  pass. The manifest fails only the unavailable idle-image comparison. This
  directly covers the audited maximum head-local rotation of `17.941364`
  degrees; cumulative head-relative-to-pelvis motion is `20.901576` degrees.
- Frame 40 is the last distinct stress frame for this attack clip, maximizing
  neck-local motion at `8.493415` degrees. Next turn must capture exactly one
  image using
  `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_frame40_review_case.json`.
- A frame-40 pass closes only the r221 attack-clip axial gate. The conversion
  policy must then be expanded and audited across body-performance clips while
  preserving zero-channel face calls before arms resume.
- Live DLC is restored to exactly four canonical banks at main SHA256
  `2902C712...8B`. No ISO, no runtime/viewer fix, all work at Idle priority,
  and no viewer/emulator process remains.
- Decision: `analysis/gh3_midori_r221_outfit1_frame14_dynamic_decision.json`.

## 2026-08-25 r221 frame 10 maximum-cumulative pass

- Captured and inspected exactly one image at outfit-1 attack frame 10/yaw 225
  using r221 main, r207 fret, and r212 long strum. Visual verdict: pass the
  maximum cumulative axial-motion gate. The head turns and dips naturally
  toward the guitar, the neck remains continuous, and the upright torso does
  not regress. Evidence:
  `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_f010_yaw225_proofs/`
  and `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_f010_yaw225_pose_review_manifest.json`.
- Model, textures, all three layers, screenshot, and command checks pass. The
  manifest also reports the expected absent idle comparison and a formal
  framing miss: visible top margin is `9px` versus the verifier's `10px`
  threshold. The hairstyle is fully visible, so this does not invalidate the
  direct pose judgment; the verifier threshold was not weakened.
- Frame 10 is the audited maximum head-relative-to-pelvis sample at
  `30.102606` degrees. Frame 14 subsequently passed; frame 40 remains as the
  maximum neck-local gate at `8.493415` degrees.
- This section's pending frame-14 instruction is superseded: frame 14 passed.
  Frame 40 is the next one-image maximum-neck-local gate.
- Live DLC is restored to exactly four canonical banks at main SHA256
  `2902C712...8B`. No ISO, no runtime/viewer fix, all work at Idle priority,
  and no viewer/emulator process remains.
- Decision: `analysis/gh3_midori_r221_outfit1_frame10_dynamic_decision.json`.

## 2026-08-25 r221 frame 17 pass and full axial stress audit

- Captured and inspected exactly one image at outfit-1 attack frame 17/yaw 225
  using r221 main, r207 fret, and r212 long strum. Model, textures, all three
  layers, framing, screenshot, and command checks pass; the manifest fails only
  the unavailable idle-image comparison. Visual verdict: pass this dynamic
  frame. The torso stays upright, neck line is continuous, and the animated
  head remains forward-facing with a modest natural turn instead of pitching
  backward. Evidence:
  `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_f017_yaw225_proofs/`
  and `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_f017_yaw225_pose_review_manifest.json`.
- Added `tools/gh3_midori_axial_motion_audit.py`. It samples all 145 packed r221
  attack frames in one converter pass, applies engine-HMX quaternion semantics,
  and ranks local neck/head and cumulative pelvis-relative head motion. Report:
  `analysis/gh3_midori_r221_outfit1_axial_motion_audit.json`.
- Frame 10 is the maximum cumulative head-relative-to-pelvis stress at
  `30.102606` degrees, with neck/head locals `0.029649`/`16.695413`. Frame 14
  separately maximizes head-local motion at `17.941364` degrees; frame 40
  maximizes neck-local motion at `8.493415` degrees. Maximum adjacent head step
  is `11.254749` degrees from frame 11 to 12; neck maximum is only `0.565306`.
- This section's pending frame-10 instruction is superseded: frame 10 passed.
  Frame 14 is the next one-image maximum-head-local gate, followed by frame 40.
- Live DLC is restored to exactly four canonical banks at main SHA256
  `2902C712...8B`. Python tests pass `136/136`; no ISO, no runtime/viewer fix,
  all work at Idle priority, and no viewer/emulator process remains.
- Decision: `analysis/gh3_midori_r221_outfit1_frame17_dynamic_decision.json`.

## 2026-08-25 r220 rejected; r221 attack-delta neck/head

- Captured and inspected exactly one image at outfit-1 attack frame 17/yaw 225
  using r220 main, r207 fret, and r212 long strum. All model, texture, layer,
  framing, screenshot, and command checks pass; the manifest fails only the
  unavailable idle-image comparison. Visual verdict: reject r220 immediately.
  The calibrated torso remains coherent, but the head is thrown sharply
  backward and the face points nearly straight upward. Evidence:
  `analysis/gh3_midori_r220_outfit1_bindcal_neckhead_yaw225_proofs/` and
  `analysis/gh3_midori_r220_outfit1_bindcal_neckhead_yaw225_pose_review_manifest.json`.
- The failure is a cross-clip absolute offset rather than useful motion. r220
  neck is already `43.287583` degrees from model bind at attack frame 0 and is
  still `43.096991` degrees away at frame 17; the source neck barely changes
  between those frames. Mapping source attack as an absolute pose therefore
  restores the same visibly invalid backward neck direction.
- Added `tools/gh3_midori_build_attackdelta_neckhead_glb_candidate.py` and built
  r221 from the visually proven r219 control. It calibrates source attack frame
  0 directly to r219 attack frame 0, then preserves all subsequent source
  neck/head motion deltas and timing. This is animated, not a static fallback;
  it deliberately discards only the unusable constant absolute neck offset.
- r221 preserves r219's calibrated spine and every non-neck/head channel
  exactly. At frame 17, neck/head are `0.217949`/`14.107417` degrees from model
  bind and cumulative head orientation relative to pelvis is `13.202759`
  degrees, versus r220's `55.068242`. All 145 samples replay with zero measured
  source-aligned local frame-check error; packed error is `0.000060709` degrees.
  SHA256: `86A142D9646502B5AC1863F0316B2B9A3B5C2656783DE1609F96F3C9A4DD7EF1`.
  Report: `analysis/gh3_midori_r221_outfit1_attack_deltahead_glb_candidate_report.json`.
- This section's pending frame-17 instruction is superseded: r221 frame 17
  passed. Frame 10 is the next one-image maximum-cumulative case. Do not
  recapture r220 or frame 17.
- Verification passes: Python `136/136`, native converter tests, and r221
  280-clip/145-sample/42-channel parsing. Live DLC is restored to exactly four
  canonical banks at main SHA256 `2902C712...8B`. No ISO, no runtime/viewer
  fix, all work at Idle priority, and no viewer/emulator process remains.
- Decision: `analysis/gh3_midori_r221_outfit1_attackdelta_neckhead_decision.json`.

## 2026-08-25 r219 static pass; r220 dynamic bind calibration

- Captured and inspected exactly one image at outfit-1 attack frame 17/yaw 225
  using r219 main, r207 fret, and r212 long strum. Model, textures, all three
  layers, framing, screenshot, and command checks pass; the manifest fails only
  the unavailable idle-image comparison. Visual verdict: pass r219 as the
  static absolute-orientation control. The calibrated torso is upright and
  continuous, the neck is seated normally, and the face looks forward rather
  than upward. Evidence: `analysis/gh3_midori_r219_outfit1_spinecal_bindhead_yaw225_proofs/`
  and `analysis/gh3_midori_r219_outfit1_spinecal_bindhead_yaw225_pose_review_manifest.json`.
- This does not approve animated neck/head motion or close the upper tier.
  Extended the source-GLB retargeter with model-bind target calibration and
  added `tools/gh3_midori_build_bindcal_neckhead_glb_candidate.py`. Source
  medium-idle frame 60 remains the neutral source; Midori outfit-1 model bind
  is now the target neutral instead of the invalid converted-idle neck.
- Built r220 from r219. Spine1/spine2/spine3, pelvis/legs, arms, hands, static
  face, and guitar channels are inherited exactly; only neck/head rotations are
  regenerated for all 145 attack samples. Maximum source-aligned packed error
  is `0.041837` degrees, maximum adjacent step is `11.254722` degrees, and
  inherited-channel error is zero. SHA256:
  `833AFF794E6CC0A1056EAE8BF4639C17F194D59084DF13255487974B08A87A14`.
  Report: `analysis/gh3_midori_r220_outfit1_attack_bindcalhead_glb_candidate_report.json`.
- r220 deliberately restores the full source motion. At frame 17, neck/head
  are `43.096991`/`19.613923` degrees from model bind and cumulative head
  orientation relative to pelvis is `55.068242` degrees. These are structural
  source-fidelity measurements, not visual approval. Reject immediately if the
  motion lands on an implausible pitch/twist axis.
- This section's pending r220 instruction is superseded: r220 was captured and
  rejected because its constant cross-clip neck offset points the face upward.
  r221 is the next one-image attack-delta case. Do not recapture r219/r220.
- Verification passes: Python `136/136`, native converter tests, and r220
  280-clip/145-sample/42-channel parsing. Live DLC is restored to exactly four
  canonical banks at main SHA256 `2902C712...8B`. No ISO, no runtime/viewer
  fix, all work at Idle priority, and no viewer/emulator process remains.
- Decision: `analysis/gh3_midori_r220_outfit1_bindcal_neckhead_decision.json`.

## 2026-08-25 r218 rejected; r219 bind-local neck/head control

- Captured and inspected exactly one image at outfit-1 attack frame 17/yaw 225
  using r218 main, r207 fret, and r212 long strum. All model, texture, layer,
  framing, screenshot, and command checks pass; the manifest fails only the
  unavailable idle-image comparison. Visual verdict: reject r218. The
  calibrated torso is substantially straighter, but the head still tilts
  backward and the face looks upward. Evidence:
  `analysis/gh3_midori_r218_outfit1_spinecal_neutralhead_yaw225_proofs/` and
  `analysis/gh3_midori_r218_outfit1_spinecal_neutralhead_yaw225_pose_review_manifest.json`.
- Absolute local-basis comparison shows that r218 faithfully used a bad
  reference. Against the outfit-1 model bind, converted medium-idle frame 60
  differs only `5.561521`/`0.481062`/`0.828598` degrees at spine1/2/3 and
  `0.050674` at head, but neck differs by `38.303272` degrees. Thus the visible
  backward head direction comes from the inherited medium-idle neck channel,
  not the corrected spine or head local.
- Extended the stabilization builder with an explicit model-bind reference
  mode and added `tools/gh3_midori_build_spinecal_bindhead_candidate.py`.
  r219 preserves r218's calibrated spine1/spine2/spine3 and all non-neck/head
  channels exactly, while holding neck/head at the character model's own
  bind-local rotations. The bind pair carries opposite approximately
  `22.3`-degree Y rotations, restoring their intended cancellation.
- r219 contains 280 clips; attack remains 145 samples and 42 channels. At frame
  17, neck/head are each within `0.029650` degrees of model bind and head
  relative to pelvis is `8.214477` degrees from the bind chain. Packed patch
  error and inherited-channel error are both zero. SHA256:
  `4EBAE44BFD66A0FFDB0DF41D439F5A65E9E25ED51EF25CAD27A4506BB5EF3B95`.
  Report: `analysis/gh3_midori_r219_outfit1_attack_spinecal_bindhead_candidate_report.json`.
- This section's pending r219 instruction is superseded: r219 passed as a
  static absolute-orientation control. r220 is the next one-image dynamic
  neck/head case. Do not recapture r218/r219.
- Verification passes: Python `136/136`, native converter tests, and packed
  inventory/replay checks. Live DLC is restored to exactly four canonical
  banks at main SHA256 `2902C712...8B`. No ISO, no runtime/viewer fix, all work
  at Idle priority, and no viewer/emulator process remains.
- Decision: `analysis/gh3_midori_r219_outfit1_bindhead_decision.json`.

## 2026-08-25 r216 rejected; r217/r218 root-outward spine correction

- Captured and inspected exactly one image at the identical outfit-1 attack
  frame 17/yaw-225 setup using r216 main, r207 fret, and r212 long strum.
  Model, textures, all three layers, framing, screenshot, and command checks
  pass; the manifest fails only the unavailable idle-image comparison. Visual
  verdict: reject r216. The body remains bipedal, but the face and chin still
  point unnaturally upward. Neutral neck/head locals did not fix the rendered
  upper chain. Evidence: `analysis/gh3_midori_r216_outfit1_upperchain_yaw225_proofs/`
  and `analysis/gh3_midori_r216_outfit1_upperchain_yaw225_pose_review_manifest.json`.
- Root-outward matrix diagnosis identifies the first bad joint. At frame 17,
  pelvis is exactly at its accepted medium-idle rotation, but r216 spine1 is
  `66.025398` degrees away locally. Spine2/spine3 are only `2.335619` and
  `4.125639` degrees away, while the head orientation relative to pelvis
  inherits a `70.058104`-degree delta. At frame 40, spine1 reaches
  `94.884654` degrees. The earlier converter mapped source stomach-lower motion
  through the wrong target local basis; this was not a neck-only failure.
- Refactored the automated GLB builder to accept an injected bone map without
  changing its r214 defaults, then added
  `tools/gh3_midori_build_axial_chain_glb_candidate.py`. r217 neutral-calibrates
  the complete source axial chain: stomach-lower->spine1,
  stomach-upper->spine2, chest->spine3, neck->neck, and head->head. It patches
  five rotation channels over 145 samples, preserves all other channels
  exactly, and replays the source-aligned locals within `0.043770` degrees.
  SHA256: `C514438460C911A0CE756FC631A07A3878AC6F522AA093C8E4F541271D2C6532`.
  Report: `analysis/gh3_midori_r217_outfit1_attack_axialchain_glb_candidate_report.json`.
- To keep visual approval in hierarchy order, added
  `tools/gh3_midori_build_spinecal_neutralhead_candidate.py` and built r218.
  It preserves r217's calibrated spine1/spine2/spine3 but holds neck/head at
  the known GH2 medium-idle locals. At frame 17, local deltas are pelvis `0`,
  spine1 `4.300150`, spine2 `12.536770`, spine3 `4.125639`, neck `0.014493`,
  and head `0.029649` degrees; head relative to pelvis is only `5.451353`
  degrees versus r216's `70.058104`. SHA256:
  `F2F1CEAE5D041FC4921D2B6508D8BCA7E8D4527CA90BC09DEB681219222B001E`.
  Report: `analysis/gh3_midori_r218_outfit1_attack_spinecal_neutralhead_candidate_report.json`.
- This section's pending r218 instruction is superseded: r218 was captured and
  rejected because its converted medium-idle neck reference is invalid. r219
  is the next one-image absolute neck/head control. Do not recapture r216/r218.
- Verification passes: Python `136/136`, native converter tests, and r218
  280-clip/145-sample/42-channel parsing. Live DLC was restored to exactly four
  canonical banks at main SHA256 `2902C712...8B`. No ISO, no runtime/viewer
  fix, all work at Idle priority, and no viewer/emulator process remains.
- Decision: `analysis/gh3_midori_r218_outfit1_axial_chain_decision.json`.

## 2026-08-25 r215 visual rejection and r216 stabilization control

- The prior upper-tier acceptance remains revoked. Pelvis and legs are accepted,
  but upper torso, neck, and head are unresolved. Do not resume arms/hands until
  this tier passes naturally in motion; guitar attachment remains last.
- Captured and inspected exactly one image using outfit-1 attack frame 17,
  yaw 225, r215 main, r207 fret, and r212 long strum. Model, textures, all
  three intended layers, framing, and screenshot checks pass. Visual verdict:
  reject r215 immediately. The body is bipedal, but the face is pitched almost
  straight upward and mostly hidden, with an unnatural head/neck direction.
  Evidence: `analysis/gh3_midori_r215_outfit1_upperchain_yaw225_proofs/` and
  `analysis/gh3_midori_r215_outfit1_upperchain_yaw225_pose_review_manifest.json`.
- Matrix-local diagnosis confirms the render. At frame 17, r215 neck and head
  are `43.097016` and `49.650811` degrees away from the accepted medium-idle
  orientation relative to spine3/chest. r214 was `30.341239`/`40.627933`, and
  r196 was `26.087396`/`26.087444`; neutral calibration did not make the pose
  anatomically plausible.
- Added `tools/gh3_midori_build_upper_chain_stabilized_candidate.py` and built
  attack-only r216 as a conversion-level control. It preserves r215 spine3 and
  every non-neck/head channel exactly, while holding only neck/head local
  rotations at GH2 medium-idle frame 60. Across all 145 attack samples its
  chest-relative neck/head deltas from that reference are both zero; packed
  error is at most `0.0000054` degrees and inherited-channel error is zero.
  Candidate SHA256:
  `9C7A89E3E10101DDF8D57C67D6ADFDAC76DA9F77739DE9A610AC10CB63CCADDE`.
  Report:
  `analysis/gh3_midori_r216_outfit1_attack_neutralhead_candidate_report.json`.
- This section's pending r216 instruction is superseded: r216 was captured and
  rejected. The failure begins at spine1; r218 is the next one-image torso
  control. Do not recapture r215 or r216.
- Verification passes: Python `136/136`, native converter tests, 280-clip
  inventory, 145-sample/42-channel attack replay. Live DLC remains exactly
  four canonical banks with main SHA256 `2902C712...8B`; r216 was not staged.
  No ISO, no runtime/viewer fix, all work at Idle priority, and no process
  remains. Outfit 2 is still deferred.
- Decision:
  `analysis/gh3_midori_r216_outfit1_upper_chain_stabilization_decision.json`.

## 2026-08-25 r214 visual rejection and r215 neutral calibration

- Captured and inspected exactly one image using the same outfit-1 attack
  frame 17/yaw-225 setup as the user-rejected r196 frame, changing only the
  main bank to r214. Model, textures, all three intended layers, framing, and
  screenshot checks pass; the manifest fails only the unavailable idle-image
  comparison. Loose DLC was restored immediately afterward.
- Visual verdict: reject r214. The body remains bipedal, but the head is thrown
  sharply backward with the chin elevated. This is an obvious upper-chain
  failure, not a borderline pose. Evidence:
  `analysis/gh3_midori_r214_outfit1_upperchain_yaw225_proofs/` and
  `analysis/gh3_midori_r214_outfit1_upperchain_yaw225_pose_review_manifest.json`.
- r214 reproduced source deltas but anchored them to the already-invalid r196
  attack frame-0 target basis. Against GH2 medium-idle frame 60, r196 attack
  frame 0 differs only `3.506844` degrees at chest but `30.416198` at neck;
  r214 frame 17 drives head world orientation `86.362882` degrees from idle.
- Extended the automated GLB builder with cross-clip calibration and normalized
  stock/alias GLB node names. Matching Blender pose records prove the retained
  attack and medium-idle bridges have byte-equivalent edit/`matrix_local`
  upper-chain transforms, so the neutral pair is compatible calibration data.
- Built r215 by mapping source medium-idle frame 60 to GH2 medium-idle frame 60,
  then applying the complete source attack chest/neck/head relationship. It
  patches the same three conversion channels across 145 samples, preserves all
  inherited channels exactly, has maximum packed source-aligned local error
  `0.043769` degrees, and maximum adjacent rotation `11.254722` degrees. Its
  frame-17 head world delta from idle is `64.422586` degrees versus r214's
  `86.362882`. Candidate SHA256:
  `A301C087DCDF9686F8930CD932798C4ECDD3F6D71506D5B03712FD7645B2BF96`.
  Report:
  `analysis/gh3_midori_r215_outfit1_attack_upperchain_neutralcal_glb_candidate_report.json`.
- This section's pending r215 instruction is superseded: r215 was captured and
  rejected. r216 is now the next one-image upper-chain control. Do not recapture
  r214 or r215.
- Live DLC remains four canonical banks with main SHA256 `2902C712...8B`.
  No ISO, no runtime/viewer fix, all work at Idle priority, and no process
  remains. Outfit 2, arms/hands expansion, and guitar attachment remain
  deferred until upper torso through head passes.
- Decision:
  `analysis/gh3_midori_r215_outfit1_upper_chain_neutralcal_decision.json`.

## 2026-08-25 r213 arm-bank diagnostic and r214 upper-chain reopening

- The prior "pelvis through head frozen" claim is superseded. Pelvis and legs
  remain accepted/regression-only, but upper torso, neck, and head are reopened.
  In the one image inspected this turn, outfit-1 attack frame 17 with r196 arms,
  r207 fret, and r212 long-strum sample 17 remained bipedal and the compact
  long-strum hand was locally coherent. The whole pose is rejected because the
  head/neck direction reads unnaturally upward/backward. Do not use this frame
  as whole-hierarchy approval.
- Added `tools/gh3_midori_upper_chain_diagnosis.py`. It samples the retained
  Blender/NXTools attack GLB and packed GH2 MILO in engine-HMX mode. Its GLB
  interpolation reproduces retained Blender pose-record chain angles within
  `0.007727` degrees. At frame 17, source-aligned target-local errors are
  `6.332877` degrees at chest/spine3, `22.587559` at neck, and `14.107377` at
  head. The source neck moves only `0.215665` degrees locally from frame 0 while
  GH2 moves it `22.702985`; attack has no head rotation channel because r126
  stripped it. Report:
  `analysis/gh3_midori_r214_outfit1_upper_chain_diagnosis.json`.
- Added `tools/gh3_midori_build_upper_chain_glb_candidate.py` and built the
  attack-only r214 conversion candidate from the exact r196 base used by the
  rejected frame. It retargets only `bone_spine3.mesh.quat`,
  `bone_neck.mesh.quat`, and `bone_head.mesh.quat` across all 145 attack
  samples. Pelvis, legs, lower spine, r196 arms, hands, face, and guitar data
  are inherited. Frame 0 is exact; packed source-aligned local error is at most
  `0.030443` degrees at frames 17/30, inherited-channel component error is zero,
  and maximum adjacent patch rotation is `11.254723` degrees. Candidate SHA256
  is `03CC416B75C0B79C7A60A0CDD0F13E86245C91E732370E053B257DD49E065904`.
  Report:
  `analysis/gh3_midori_r214_outfit1_attack_upperchain_glb_candidate_report.json`.
- The full r213 outfit-1 arm builder completed all 280 inventory clips and
  84,138 samples, reproducing r196 attack@30 and r197 fast-solo@90 exactly.
  It remains diagnostic and is not visually approved: 2,116 left and 5,609
  right reach clamps, a `179.865151`-degree adjacent forearm step, and a
  `16.263567` position step remain. It also incorrectly adds arm channels to
  static GH2 face-call clips; its worst right clamp is on
  `gh2_face_call_EyesClosed`. A later arm-bank rebuild must select body
  performance clips and preserve zero-channel face calls exactly.
- Verification passes: Python `136/136`, native converter tests, r214 clipset
  parse, and packed replay. Live DLC remains exactly four canonical banks with
  main SHA256 `2902C712...8B`; r213/r214 were not staged. No ISO, no runtime or
  viewer fix, all work at Idle priority, and no viewer/emulator remains.
- This section's pending r214 capture is superseded: r214 was visually rejected
  and r215 is the next one-image candidate. Only after upper torso through head
  passes should arms/hands resume; outfit 2 and guitar attachment remain
  deferred.
- Decision:
  `analysis/gh3_midori_r214_outfit1_upper_chain_reopen_decision.json`.

## 2026-08-25 r212 representative outfit-1 acceptance and full-bank audit

- Scope is now outfit 1 only. Defer outfit 2 until outfit 1 is solved. This
  historical section's pelvis-through-head freeze was superseded by the r214
  upper-chain reopening above; guitar attachment remains last.
- Captured and inspected exactly one image: outfit-1 r196 attack frame 30 at
  yaw 225 with r207 bar4-bend fret and r212 s04 strum. The body is clearly
  bipedal, both clavicle-to-wrist chains are continuous, the fret hand is
  compact, and the strum hand now reads as a recognizable curled hand connected
  to the sleeve rather than a flat paddle. Accept r212 for this representative
  pose. The guitar is edge-on and was intentionally not assessed.
- The one-case manifest fails only `visible_difference_from_idle` because the
  case has no idle reference; render, model, texture, layer, framing, and
  screenshot checks all pass. Live DLC restored to exactly four canonical
  banks and no viewer process remains.
- Added an automated full-bank topology audit and ran it against outfit 1 at
  Idle priority. All 23 strum clips and 1,045 samples pass: only the expected
  eight right non-thumb `02/03` channels change from r207, no unweighted `03`
  tip moves, no unexpected channel changes, and quaternion norms remain within
  `0.999999302..1.000000649`.
- The next visual risk is the long-strum family. Its maximum weighted `02` curl
  is `177.335599` degrees at
  `gh3_hnd_guit_strum_mido_norm_l01_d` sample 17, middle finger; 111 samples
  exceed 170 degrees. Treat those as pending visual stress cases, not approved
  merely because the packed audit passes.
- Decision:
  `analysis/gh3_midori_r212_outfit1_attack_bend_visual_decision.json`.
  Full-bank audit:
  `analysis/gh3_midori_r212_two_joint_strum_full_bank_audit.json`.
  Next one-image case is already prepared at
  `analysis/gh3_midori_r212_outfit1_longstrum_stress_case.json`; frame 17 hits
  the audited maximum-curl sample directly.
  No ISO, no runtime/viewer fix, all work at Idle priority. Actual GH2 PS2 build
  validation remains pending.

## 2026-08-25 r211 rejected and r212 two-joint strum candidate

- Captured and inspected exactly one image: outfit-1 fast-solo frame 90 at yaw
  135 with r197 main, r207 fret, and r211 stock-controller s04. Body, arms, and
  fret hand remain coherent, but the strum hand is still the same broad flat
  mass. Reject r211. Its 13.282991-degree controller change affected only the
  hand region (`1812` pixels) and did not repair the silhouette.
- Added read-only `milo_convert_tool inspect-character --mesh-vertices`, which
  emits packed Mesh28 position, normal, four weights, and bone slots at full
  float precision. Added an automated point-cloud comparator. Midori's bind
  hand is not oversized relative to Glam1: all-hand AABB extents are about
  `1.010x/0.994x/0.915x`, and PCA extents are `0.947x/0.953x/0.751x`.
- The structural mismatch is topology. Midori's source has three deform joints
  for each left non-thumb finger but only two for each right non-thumb finger;
  the right thumb has three. r207-r211 wrote stock GH2 right `03` curl into
  unweighted bones, discarding it.
- Added `--collapse-unweighted-right-finger-tips` to the all-hand converter.
  For right index/middle/ring/pinky only, it composes stock `02+03` onto the
  weighted Midori `02` and holds `03` at bind. Left fingers, right `01`, thumb,
  controller, arms, timing, and non-finger channels are unchanged.
- r212 strum:
  `analysis/gh3_midori_gh2_milos/gh3_midori_strum_handstockmotioncollapsedrighttips_r212.milo_ps2`,
  SHA256 `2A13341891F118011C11FA2CA411B3540C3F571DD7C281934915383D585A4118`,
  size `21291`, 23 clips/460 channels. Its generated fret bank is byte-identical
  to r207 (`FA984...`). At s04 sample 24, exactly eight channels change: the
  four right non-thumb `02/03` pairs.
- Python tests pass `133/133`; native converter tests and KERNEL32-only import
  gates pass. Live DLC was never staged and still contains exactly four banks.
  No ISO, no runtime source change, all work at Idle priority.
- Decision:
  `analysis/gh3_midori_r211_outfit1_fastsolo_bend_visual_decision.json`.
  Structural proof:
  `analysis/gh3_midori_r212_two_joint_strum_structural_validation.json`.
- Next image turn: use a known-coherent main pose with the right arm oblique to
  the camera, r207 fret, and r212 strum. One image only; reject immediately if
  the hand is not recognizably human. Actual GH2 PS2 build validation remains
  required before promotion.

## 2026-08-25 r210 rejection and r211 stock-controller candidate

- Captured exactly one image: outfit-1 fast-solo frame 90 at yaw 135 with
  r197 main, r207 bar4-bend fret, and r210 s04 strum. The body and frozen arm
  chains remain coherent, but the strum hand is still an oversized broad mass
  across the pickguard. Reject r210. The one-case manifest fails only because
  it intentionally has no idle reference.
- r210 did reach the rendered skin: right hand-to-finger length shortened from
  r209's `2.516548` to `2.246351`. More local-Z curl is therefore not the next
  useful variable.
- Corrected an earlier false baseline. r205 fast-solo yaw-135 and rejected r207
  differ by only `1561` full-image pixels and `317/13000` pixels in the strum
  hand/pickguard crop. The r205 "coherent strum hand" label was too generous;
  do not use it as visual approval evidence.
- Added `milo_convert_tool inspect-character --meshes`, reporting packed
  Mesh28 bone slots, bind offsets, weight sums, and weighted centers. The actual
  Midori hand/finger offsets match stock Glam1 at printed precision, and its
  weighted centers occupy the same stock bind-space neighborhood. Apparent
  cross-hand mesh bounds were disconnected palette partitions, not bad
  opposite-side weights; `--side-correct-hand-weights` is a decompressed-byte
  no-op on both outfits.
- The bounded remaining mismatch is controller orientation. r207 uses the r196
  source-pose wrist/controller quaternion with stock GH2 s04 fingers. Added
  `--stock-strum-controller-quat` to the converter-side hand builder plus a
  regression test. Built r211 by replacing only s04 in r207 with the matching
  stock `strum_short_04` controller quaternion. At review sample 24, controller
  position, all three arm channels, and all 15 finger channels are exact to
  r207; only `bone_strum_hand.mesh.quat` changes by `13.282991` degrees.
- r211 output:
  `analysis/gh3_midori_gh2_milos/gh3_midori_strum_handstockmotion_short04stockcontroller_r211.milo_ps2`,
  SHA256 `14A661271C626CB44D75F07527C03721E269F1664E0A563267FBCD8394A7F47E`,
  size `23370`, 23 clips. Python tests pass `132/132`; native converter tests
  and the KERNEL32-only DLL gate pass.
- Decision:
  `analysis/gh3_midori_r210_outfit1_fastsolo_bend_visual_decision.json`.
  Live DLC restored to four canonical animation banks, no ISO used, all work
  at Idle priority, and no runtime code changed.
- Next image turn: repeat the exact r210 case with r211 strum. Reject immediately
  if the hand remains broad; do not touch guitar attachment.

## 2026-08-25 r209 visual rejection and r210 first-knuckle curl

- Captured exactly one image: the identical outfit-1 fast-solo frame 90,
  yaw-135, bar4-bend/s04 case with r209 hand overlays. Logs prove the overlays
  contain 31 fret and 19 strum channels and no longer overwrite controller
  target quaternions. All render/layer/framing checks pass; the one-case
  manifest lacks only an idle-reference image.
- Visual verdict: reject r209. Body, r197 arms, and fret grip remain coherent,
  but the pose-specific main-owned wrist changes only marginally and leaves the
  broad strum-palm silhouette. r207-to-r209 changes 0.248409 percent of image
  bytes; right hand-to-finger length is 2.516548 and thumb length is 3.082885.
- r208 and r209 jointly reject distal-only curl and the 3.788366-degree
  attack/fast-solo controller difference as dominant causes. The next isolated
  conversion variable is first-knuckle flexion.
- Added bounded local-Z curl controls to
  `tools/gh3_midori_build_hand_detail_blend_candidate.py` and a regression test.
  Built r210 from r207, replacing only s04 with a static diagnostic pose:
  +35 degrees on each non-thumb first knuckle, +20 degrees on each non-thumb
  distal joint, thumb unchanged. Output:
  `analysis/gh3_midori_gh2_milos/gh3_midori_strum_handfirstcurl35distal20_r210.milo_ps2`,
  SHA256 `5F9E2363524FC264D62D070D96BAD18D2F746AA79794FCDAD8F1832365315204`.
  It retains 23 clips and 460 channels. Full pipeline tests pass `131/131`.
- Decision:
  `analysis/gh3_midori_r209_outfit1_fastsolo_bend_visual_decision.json`.
  Live DLC was restored, no ISO was used, and all work ran at Idle priority.
- Next image turn: the same case with r207 fret plus r210 strum. If the hand
  remains broad, diagnose converted hand mesh/bind geometry rather than adding
  more wrist or distal-finger offsets.

## 2026-08-25 r208 visual rejection and r209 controller ownership diagnostic

- Captured exactly one image: the identical outfit-1 fast-solo frame 90,
  yaw-135, bar4-bend/s04 case with only r208 compact strum fingers changed.
  All render/layer/framing checks pass; the one-case manifest again lacks only
  an idle-reference image.
- Visual verdict: reject r208. Pelvis through head, r197 arms, and the fret hand
  remain coherent. The strum palm still reads broad and flat across the
  pickguard. The tighter pose changes only 0.150716 percent of BMP bytes,
  shortens the inferred finger axis from 2.520671 to 2.476487, and shortens the
  thumb axis from 3.050363 to 2.999382. Distal curl is not the dominant error.
- Controller diagnosis: r207/r208 overlays write the r196 attack controller
  target quaternions over every main pose. The solved r197 fast-solo main clip
  contains its own pose-specific targets; its strum target differs from r196 by
  3.788366 degrees, but the overlay currently erases that distinction.
- Built r209 converter-only diagnostics by stripping
  `bone_fret_hand.mesh.quat` from all 36 fret clips and
  `bone_strum_hand.mesh.quat` from all 23 strum clips. All remaining selected
  samples are exact to r207. Outputs:
  `analysis/gh3_midori_gh2_milos/gh3_midori_fret_handstockmotion_nocontroller_r209.milo_ps2`
  (`59BD1BCBB0C25DBC2E24797F259AF4F214F0ECBE497388ABBD4878FA7BF340A1`)
  and
  `analysis/gh3_midori_gh2_milos/gh3_midori_strum_handstockmotion_nocontroller_r209.milo_ps2`
  (`39ABE6E5E9502E7C465DBC7DD365474E1BD04B1E1BE51031983F4A358A2BBBB0`).
- Promotion boundary: canonical main has both controller quaternions in only
  1/280 clips; the bounded r196/r197 branches have them in two clips. r209 is
  therefore valid only for solved attack/fast-solo diagnostics. Do not promote
  stripped overlays globally until all-main arm/controller conversion exists.
- Decision:
  `analysis/gh3_midori_r208_outfit1_fastsolo_bend_visual_decision.json`.
  Live DLC was restored, no ISO was used, and all work ran at Idle priority.
- Next image turn: repeat the identical case with r209 overlays and r197 main.

## 2026-08-25 r207 first hand-family visual and r208 compact-strum candidate

- Captured exactly one image this turn: outfit-1 fast-solo frame 90 at yaw 135,
  with r207 `bar4_bend` fret frame 2 and `s04` strum frame 24. The log proves
  all three expected layers loaded. The one-case manifest fails only
  `visible_difference_from_idle` because no idle image is included; all render,
  framing, model, texture, and layer checks pass.
- Visual verdict: reject this hand combination. Pelvis through head remain
  coherent, both r197 arm chains remain bipedal/continuous, and the fret hand
  is compact on the neck. The strum hand reads as an oversized flat paddle
  across the pickguard.
- The failure is not r207 motion resampling. `strum_short_04` has seven stock
  samples, but the converted finger rotations change by less than 0.2 degrees;
  its static pose is nearly the same as `strum_pick_01` except at the thumb.
- Screened every stock Glam1 strum sample numerically. The tightest reference is
  `strum_long_01` sample 9, averaging 79.545 degrees of non-thumb distal curl.
  It adds about 6-11 degrees of curl across the fingers and 17 degrees at the
  thumb tip versus the rejected pose.
- Built r208 by replacing only
  `gh3_hnd_guit_strum_mido_norm_s04_d` finger channels with that compact stock
  sample. Candidate:
  `analysis/gh3_midori_gh2_milos/gh3_midori_strum_handcompactlong01f9_r208.milo_ps2`,
  SHA256 `E0DB854F246FFDABF889067E74168B11F92E29FC12D8D6069CC53229ECDBE836`.
  All 23 clips/460 channels remain present; every non-s04 clip's first and last
  samples are exact to r207. Keep the r207 fret bank; the companion one-clip
  builder fret output is not part of r208.
- Decision:
  `analysis/gh3_midori_r207_outfit1_fastsolo_bend_visual_decision.json`.
  Live DLC was restored, no ISO was used, and all processes ran at Idle.
- Next image turn: repeat the identical yaw-135 fast-solo case with r207 fret
  plus r208 strum. Do not broaden the compact mapping until that one pose passes.

## 2026-08-25 r207 time-varying stock hand conversion

- Conversion tooling only: no game/viewer runtime source was changed. Added
  `full_samples`, `one_samples`, and `duplicate_samples` to
  `milo_convert_tool inspect-clipset` so stock GH2 legacy `.rotz` hand channels
  can be resampled from serialized clip metadata instead of inferred byte
  lengths. The rebuilt tool passes its forbidden-runtime-DLL check
  (`KERNEL32.dll` only) and native converter test.
- Extended `tools/gh3_midori_build_all_hand_stock_reference_candidate.py` with
  `--stock-motion-mode resample`. The default remains `static` so r206 stays
  reproducible. r207 phase-resamples each mapped stock finger clip onto the
  existing Midori sample count while preserving target timing, non-finger
  channels, controller-target quaternions, and r196/r197 arms.
- r207 fret output:
  `analysis/gh3_midori_gh2_milos/gh3_midori_fret_handstockmotionmapped_r207.milo_ps2`,
  SHA256 `FA984B2907E60FF5BC8DEBA3937848484505307D15AA1EC2111154BEFAD7289C`.
  It contains 36 clips and 687 standard `.pos`/`.quat` channels.
- r207 strum output:
  `analysis/gh3_midori_gh2_milos/gh3_midori_strum_handstockmotionmapped_r207.milo_ps2`,
  SHA256 `A7B9FAC3A02086128D351EC86B818C497C6BE7A44CC227A9431EFE86A220E011`.
  It contains 23 clips and 460 standard `.pos`/`.quat` channels.
- Numeric motion proof: three-sample bar3 bend varies 9/15 finger channels;
  61-sample `strum_long_01` varies 15/15. Quaternion norms remain within
  `0.99999930..1.00000066`. The m01 pick fingers correctly remain static
  because stock GH2 places that stroke motion in wrist/arm channels, which this
  hand-only pass intentionally does not overwrite.
- r207 selected bar3/m01 samples are exact to r206, which was exact to the
  directly reviewed r205 pair. Thus the representative coherent hand result is
  preserved; newly animated families still require direct visual review.
- Structural validation:
  `analysis/gh3_midori_r207_hand_stock_motion_mapped_structural_validation.json`.
  Decision: `analysis/gh3_midori_r207_hand_stock_motion_decision.json`.
  Python pipeline tests pass `130/130`; native `milo_convert_test` passes.
  Everything ran at Idle priority, with no ISO and no live DLC staging.
- Next: one image per turn, beginning with an oblique fret-bend pose. Do not
  promote until remaining hand families, outfit 2, broad main clips, and an
  actual GH2 PS2 pack/load are validated. Guitar attachment remains last.

## 2026-08-25 r202-r206 conversion-level hand reference branch

- All fixes in this branch are converter-emitted GH2 PS2 MILO data. No
  Midori-specific viewer, engine, or runtime behavior was added. Guitar
  attachment remains untouched and last in hierarchy order.
- r202 (fret thumb/index at bind) and r203 (all fret fingers at bind) were
  rejected. Both retained the flat, fanned GH3-derived finger silhouette, so
  percentage damping toward Midori bind is not a viable hand solution.
- Extracted stock Glam1 model/fret/strum banks from the local GH2
  `GEN/main_0.ark`; no ISO was mounted or used. Reference hashes are model
  `0380F74BBB7B15FF92540C9B0F0988678C31507A47E46F7D0CE7671A6173742E`,
  fret `C44CEC2031E6C6AF777C17651D153E06B95C27474F3961F8F545C0AB86C101EC`,
  and strum
  `7BCF173E85058EB32B8AEFB3B0922F1F9648B0A47C69B1302452996DF1DC034F`.
  Stock distal-joint rotation signs/magnitudes explain why the GH3-derived
  fingers opened in the wrong direction.
- Extended `tools/gh3_midori_build_hand_detail_blend_candidate.py` to convert
  stock GH2 first-joint quaternion and distal `rotz` references into full
  Midori `.mesh.quat` channels. r204 established a recognizable fret grip.
- r205 applied stock references to the selected bar3 fret and m01 strum clips.
  Attack frame 30 and fast-solo frame 90 were inspected sequentially in front,
  135-degree, and 225-degree views. Both hands are compact and connected while
  the r196/r197 arms remain coherent. Representative r205 hashes are fret
  `FF812F34C5AA0AE867CB643B254E2F4A1796FABD8FDC045234E302960B9EF181`
  and strum
  `A6B87A78F3B423F0FC85250B10020847B61D1892AF99FF12234DFE7DD050AD7C`.
- Added all-bank converter
  `tools/gh3_midori_build_all_hand_stock_reference_candidate.py`. r206 maps all
  36 fret and 23 strum clip names to explicit stock GH2 reference shapes. The
  packed outputs parse as 36 clips/687 channels and 23 clips/460 channels;
  every channel is ordinary `.pos` or `.quat`. Hashes are fret
  `9893EB9AB2D430529B57C78C2E02B3BFF80AFDD8E07F6E9430BF7629100C2385`
  and strum
  `69A08D00042A413464D128EAFB9606CF376E97609086BA3D3018C731B0FF24F5`.
- r206 is an all-name static-shape candidate, not a finished hand animation
  set: each clip repeats one coherent stock finger pose over its existing
  sample count. Clip timing and non-finger channels remain intact, but
  time-varying stock finger articulation has not yet been resampled.
- Structural validation:
  `analysis/gh3_midori_r206_hand_stock_mapped_structural_validation.json`.
  Decision:
  `analysis/gh3_midori_r202_r206_outfit1_hand_stock_reference_decision.json`.
  Full pipeline tests pass at Idle priority (`128 tests OK`). Live DLC remains
  unchanged with only its four canonical animation banks.
- Compatibility boundary: r206 is valid converter output and needs no runtime
  correction, but it has not yet been packed into or loaded by an actual GH2
  PS2 build. Do not promote it as PS2-validated.
- Next: preserve r196/r197 arms; automate time-varying stock finger resampling
  for mapped families that need it, then inspect remaining hand families one
  image per turn. Outfit 2 and broad main-clip coverage follow; guitar
  attachment stays last.

## 2026-08-25 r185-r201 arm continuity and hand-layer diagnosis

- Face remains static/no-op. Pelvis, legs, spine, chest, neck, and head remain
  frozen regression gates. Current order is clavicle, upper arm, forearm,
  wrist/hand, fingers, then guitar attachment.
- r185 proved visible-hand quaternion channels are overwritten by downstream
  `CharIKHand` controllers. r186 proved controller-target rotation routing
  works, but the old visible-axis calibration produced detached paddle hands.
  r187/r188 replaced that calibration with source-pose deltas on the controller
  target bind; wrist rotation alone did not fix the detached silhouettes.
- r189-r193 isolated two frame errors. Arm-only builds stripped fret/strum
  controller positions, and controller locals/arm targets were being solved in
  a coupled-guitar frame whose guitar transform was then stripped at runtime.
  Added `--keep-arm-controller-target-positions`; arm-only solves now preserve
  only those wrist targets while leaving guitar position/rotation untouched,
  and use `visible_arm_guitar_frame: unmodified-runtime`.
- Source-palm-fit targets were rejected in the corrected runtime frame. Attack
  wrists folded behind the torso in r193. The untouched stock controller
  targets were well separated and anatomically useful, so r194-r197 use
  `--visible-arm-target-mode current-proxies`.
- Added `--camera-yaw-deg` passthrough to the pose-review and loose-DLC capture
  wrappers. Every accepted arm diagnostic was inspected directly in front,
  135-degree, and 225-degree views.
- r194 showed that stock targets were the correct direction, but the source
  elbow plane left one hand visually detached. r195 proved wrist translation
  moves hand and forearm together and cannot repair that gap. r196 switched to
  `--visible-arm-elbow-hint-mode current`; attack frame 30 then showed two
  continuous clavicle-to-wrist chains in all three views.
- r197 repeated the exact r196 method on fast-solo frame 90 and passed the same
  three-view arm-continuity check. Current non-live arm baselines:
  `analysis/gh3_midori_gh2_milos/gh3_midori_1_main_staticface_armonly_stocktargets_currentelbow_attack_r196.milo_ps2`,
  SHA256 `454EA425E5CE25B6C655946433639D08DE0F0DB8776C62BAC0E4EE6F1346581E`,
  and
  `analysis/gh3_midori_gh2_milos/gh3_midori_1_main_staticface_armonly_stocktargets_currentelbow_fastsolo_r197.milo_ps2`,
  SHA256 `06959D6427B07F14147554197E294B3DB2A90C1D5699AEA3149BF9DC05F6CF49`.
- Arm decision:
  `analysis/gh3_midori_r188_r197_outfit1_arm_runtime_decision.json`.
  Treat the r196/r197 clavicle, upper-arm, forearm, wrist-position, and current
  elbow-plane method as frozen for outfit-1 hand work. It is not yet a shared
  all-clip or outfit-2 promotion.
- Applying the actual fret/strum overlays preserved arm continuity but exposed
  palm/finger distortion. Finger-only r198 (35 percent) and r199 (bind) controls
  showed that overlay `bone_fret_hand.mesh.quat` and
  `bone_strum_hand.mesh.quat`, not the 30 finger joints, dominated the bad palm
  orientation. r200 copied the arm-approved r196 controller quaternions into
  the two overlay clips and visibly improved both palms.
- Added automated builder
  `tools/gh3_midori_build_hand_detail_blend_candidate.py`. It can preserve
  non-finger overlay channels, blend only the 30 finger quaternions toward the
  model bind, and optionally copy controller quaternions from a fullclip report.
- r201 combines r196 controller quaternions with 35 percent authored finger
  motion. It is the best current hand branch, not approved. Fret thumb/index
  still protrude and need the next bounded finger-only correction. Candidate
  hashes: fret
  `3728E69AC24E12C2573CF7A116700D96C064F7F771D16D70B8758392A340A6B5`,
  strum
  `560EC042EF603BF59205A57959293E844611D0D9F519BC1B316A66CE13B53547`.
  Hand decision:
  `analysis/gh3_midori_r198_r201_outfit1_hand_runtime_decision.json`.
- All work used loose DLC with automatic restore/delete, no ISO, and Idle
  process priority. Live DLC remains unchanged. One-case capture manifests fail
  only `visible_difference_from_idle` because they intentionally omit an idle
  reference.
- Next step: keep r196/r197 arms and r200/r201 controller quaternions frozen;
  correct fret thumb/index finger joints first, then validate strum fingers.
  Guitar attachment remains last.

## 2026-08-25 r182-r184 resumed arm-only attack diagnostics

- User reinitialized the Midori goal and resumed work. Face remains
  static/no-op; do not resume face crop/viseme work for this branch.
- Captured r182 one-case no-face outfit-1 arm-only attack:
  `analysis/gh3_midori_r182_outfit1_armonly_attack_proofs/midori_1_attack_left_r182_armonly_f030.bmp`.
  The wrapper staged loose DLC only and removed the staged r182 path afterward.
  Structural review failed only `visible_difference_from_idle` because this
  intentional one-case capture had no idle reference.
- r182 verdict: reject. It improved guitar placement compared with r181, but
  the visible arm chain still rose through the face/head region. Updated status:
  `analysis/gh3_midori_r182_outfit1_staticface_armonly_attack_status.json`.
- r182 mesh evidence showed the left forearm part was the visible face-band
  offender: `midori_1_mesh0_part13.mesh` center
  `[0.34055, 3.93896, 57.5484]`, `face_positive_y_vertices=101`.
- Generated r182 render-feedback reports:
  `analysis/gh3_midori_r182_outfit1_armonly_attack_meshpart_report.json` and
  `analysis/gh3_midori_r182_outfit1_armonly_attack_meshpart_summary.json`.
- Ran report-only r183 screen:
  `analysis/gh3_midori_r183_outfit1_attack_armonly_r182feedback_score_report.json`.
  The useful outfit-1-only target was `[-1.1, -2.2, 4.0]`; do not use the
  shared attack override emitted by the scorer for this outfit-specific branch.
  Manual map:
  `analysis/gh3_midori_r183_outfit1_attack_armonly_override_map.json`.
- Built/captured r183 arm-only attack:
  `analysis/gh3_midori_gh2_milos/gh3_midori_1_main_staticface_armonly_attack_r183.milo_ps2`,
  SHA256 `BDDDC43A9AD9D135A0C282D5C1DF6D769695FC8434F057583FF82A88ED947420`.
  Screenshot:
  `analysis/gh3_midori_r183_outfit1_armonly_attack_proofs/midori_1_attack_left_r183_armonly_f030.bmp`.
  Decision:
  `analysis/gh3_midori_r183_outfit1_staticface_armonly_attack_visual_decision.json`.
  Verdict: improved, not approved. The left forearm no longer rises through
  the face/head region; part13 moved to `[-3.19093, -1.05006, 48.179]` with
  `face_positive_y_vertices=0`. Shoulder/forearm/hand silhouette still reads
  stiff/disconnected.
- Built/captured r184 as r183 plus visible clavicle solving:
  `analysis/gh3_midori_gh2_milos/gh3_midori_1_main_staticface_armonly_clav_attack_r184.milo_ps2`,
  SHA256 `E7E2D1FEA319CF5DB74B7315C52494DE9EB7A2DA7F44BBFA8229FE7986B39573`.
  Screenshot:
  `analysis/gh3_midori_r184_outfit1_armonly_clav_attack_proofs/midori_1_attack_left_r184_armonly_clav_f030.bmp`.
  Decision:
  `analysis/gh3_midori_r184_outfit1_staticface_armonly_clav_attack_visual_decision.json`.
  Verdict: best current outfit-1 attack branch, not approved. Shoulder/torso
  silhouette improves over r183 and part13 remains out of the head/face
  (`[0.311851, -0.041518, 46.4679]`, `face_positive_y_vertices=0`), but
  hand/forearm contact around the guitar remains visibly off.
- Next branch: preserve r184's arm-only/contact-skipped/clavicle path and
  improve hand/forearm contact for outfit-1 attack before broadening to other
  clips or outfit 2. Guitar attachment is still last; do not promote r181-r184.
...
### 2026-08-25 corrected partial-mask root cause and r222 axial rebuild

- Superseding status: pelvis through head is not visually approved. The user's
  concern that recent neck/head direction looks unnatural remains an open gate.
  Face is intentionally static/no-op. Arms, hands, fingers, and guitar remain
  deferred. Outfit 1 is the only active outfit.
- Found the source-authority error in GH3 PS2 partial-animation masks. NXTools
  reads the flag count little-endian, then reads each 32-bit mask word
  big-endian. The retained IR had read both as little-endian. For
  `gh3_guit_mido_a_attackl`, the authoritative words are `0xff232e00`,
  `0x00000000`, `0x04ffffff`; among the axial chain only `Bone_Neck` is
  allowed. Spine1/2/3 and head are excluded.
- Corrected `tools/gh3_midori_ir.py`, added deterministic migration tool
  `tools/gh3_midori_migrate_partial_masks.py`, and migrated all 279 partial
  clips in the retained 280-clip IR. The gzip SHA-256 is
  `B1DDFD016598A1FDC2D9734C9A2C8680D0D4EBCCFA8FB1F25F36C9BAD87BE787`.
  No ISO, mount, or re-extraction is needed for this branch.
- r221 is rejected as a full-clip/source-authoritative solution. Its force-
  partial GLB ends at source frame 40, so target samples 41 through 144 freeze
  at frame 40, and it animates bones excluded by the corrected mask. Retained
  r221 stills are local visual observations only. See
  `analysis/gh3_midori_r221_fullclip_retraction.json`.
- Added `tools/gh3_midori_ir_source_pose.py`. It evaluates all source frames
  directly from retained IR using the pinned NXTools contract
  `node_rest * inverse(edit) * source_quaternion`; excluded bones remain at
  node rest. Validation against the pinned preserve-partial attack GLB passes:
  max exact-key error `0.000024508` degrees, excluded-rest error
  `0.000001708` degrees. Between-key max is `2.962996221` degrees because
  direct quaternion slerp and Blender F-curves interpolate differently.
- Added `tools/gh3_midori_build_allmain_axial_ir_candidate.py` and built
  unpromoted r222 from the trusted r196 base. It patches only
  `bone_spine1.mesh.quat` through `bone_head.mesh.quat` for 242 non-face main
  source clips. Included source bones use same-clip frame 0 anchored to the
  outfit-1 model bind; excluded bones use model-bind local rotation. All other
  channels are inherited. Candidate:
  `analysis/gh3_midori_gh2_milos/gh3_midori_1_main_staticface_correctedpartial_axial_ir_r222.milo_ps2`,
  16,622,253 bytes, SHA-256
  `070395D13343A397A047337515396572E3DC2B185101A645A9EED71D98A6BDF7`.
- r222 covers 74,948 full-duration samples with no source-frame clamp. It keeps
  six face clips and 38 unassigned/runtime-alias clips untouched. Produced-MILO
  replay has exact inherited-channel hashes and max patched-quaternion error
  `0.000108641` degrees. Attack has all 145 samples, neck dynamic, and
  spine1/2/3/head at bind.
- Added `tools/gh3_midori_axial_stress_audit.py`. Across all 242 clips it found
  zero malformed quaternions, max frame-0 bind error `0.000046325` degrees,
  and max excluded-bone motion `0.000046325` degrees. Structural conversion
  status is valid, but visual status remains unresolved. Significant review
  extremes are: max local delta `65.198772` degrees on head, max adjacent local
  step `20.851385` degrees on neck, max pelvis-relative head delta
  `116.831846` degrees in `gh3_guit_midori_a_loss3` frame 368, and a normal
  gameplay break delta `103.737674` degrees in
  `gh3_guit_mido_a_med_break02` frame 292.
- Next visual gate: in the next turn, capture exactly one outfit-1 image from
  r222 using `gh3_guit_mido_a_med_break02` frame 292 and a side-readable camera
  that does not foreshorten the arms/head. Inspect that single frame directly.
  Do not advance to arms unless the axial pose is visually coherent. This turn
  already consumed its one permitted image, so no r222 image was captured.
- Verification: Python pipeline tests pass `140/140`; native
  `milo_convert_test` passes. All work ran at Idle priority. The hybrid loose
  DLC remains exactly four canonical banks; main SHA-256 is
  `2902C712F81ED32A8D7D14E147453F16196C7AE935E5F3D029C179648A36AC8B`.
  r222 was not promoted and no GHOGX/PCSX2 process remains running.

### 2026-08-25 r222 full-axial medium-break stress frame passed

- Captured and inspected exactly one image this turn: outfit 1,
  `gh3_guit_mido_a_med_break02` frame 292, yaw 225, camera distance 135, with
  only the r222 main layer active. Evidence:
  `analysis/gh3_midori_r222_outfit1_medbreak02_f292_yaw225_proofs/midori_1_r222_medbreak02_f292_yaw225_axial_stress.bmp`.
- Visual decision: pass this specific axial frame. The lower body is bipedal;
  torso, neck, and head form a continuous attached chain. The large leftward
  silhouette is hair motion in a dramatic break/headbang pose. Existing main-
  layer arms and guitar cross the face and remain outside the axial verdict.
- The capture manifest's only automated failure is
  `visible_difference_from_idle`; this isolated one-case manifest deliberately
  has no paired idle image. All load, texture, clip, one-layer, image, framing,
  and command checks pass. Decision:
  `analysis/gh3_midori_r222_medbreak02_f292_axial_visual_decision.json`.
- Added `tools/gh3_midori_axial_pose_parity.py`. For this exact frame, retained
  GH3 source cumulative spine1-through-head motion is `103.737736771` degrees;
  r222 MILO motion is `103.737686493` degrees. Difference is `0.000050278`
  degrees; every individual axial joint matches within `0.000037340` degrees.
  Target chest-neck and neck-head segment lengths remain constant. The dramatic
  orientation is source-authored, not conversion-added. Report:
  `analysis/gh3_midori_r222_medbreak02_f292_axial_pose_parity.json`.
- This passes the full-axial source-motion stress branch only. The global axial
  gate remains open because corrected partial clips have different semantics.
  Next one-image target is `gh3_guit_mido_a_attackl` frame 60, r222's maximum
  neck-local displacement (`39.176372` degrees), where only neck is dynamic and
  spine1/spine2/spine3/head locals remain bind. Use the same isolated main-only
  yaw-225 view. Do not begin arms before this neck-only partial gate passes.
- The restore-safe wrapper returned both loose-DLC mirrors to exactly four
  canonical banks. Main SHA-256 remains
  `2902C712F81ED32A8D7D14E147453F16196C7AE935E5F3D029C179648A36AC8B`;
  outfit model hashes are also unchanged. No runtime process remains.
- Prepared the next case at
  `analysis/gh3_midori_r222_outfit1_attack_f060_neckonly_review_case.json` and
  structurally validated it before capture. Attack frame 60 retained-source
  cumulative motion is `39.176409886` degrees; r222 is `39.176373227` degrees,
  difference `0.000036659` degrees. Spine1/spine2/spine3/head local deltas are
  zero; only neck is dynamic. Report:
  `analysis/gh3_midori_r222_attack_f060_axial_pose_parity.json`. Python tests
  remain `140/140`; native `milo_convert_test` passes.

### 2026-08-25 shared-stance axial correction and direct arm branch

- The outfit-1 attack frame-60 neck-only image is a local visual pass, not a
  global pelvis-through-head closure. The user's later concern about an
  unnatural upper-torso/neck/head viewing direction supersedes the earlier
  closure wording. Keep the global axial gate provisional until the revised
  shared-reference pose is inspected. Face remains static/no-op.
- Extended `tools/gh3_midori_ir_source_pose.py` to evaluate animated source
  translation. NXTools/GLB local translation is source node rest plus decoded
  SKA translation. Rotation is validated against the preserve-partial GLB and
  authored guitar translation keys against the force-partial GLB. The combined
  validator passes with max exact rotation error `0.000024508` degrees,
  excluded-rest error `0.000001708` degrees, and exact guitar-translation
  component error `0.000010430` source units. Between-key GLB differences are
  Blender F-curve interpolation, not an authored-key mismatch.
- Added `tools/gh3_midori_source_hand_target_ir.py` and report
  `analysis/gh3_midori_attack_source_hand_target_ir_report.json`. A direct
  full-clip attack IK bake is structurally rejected: the source left hand
  target is reachable in only `19/145` frames and peaks at `1.903707x` source
  arm reach; the right is reachable in only `17/145` and peaks at `2.666641x`.
  The attack clip is a partial layer, so forcing visible GH2 arms to every
  guitar-descendant target would reproduce the earlier non-bipedal failures.
- Auditing all 242 non-face body-main clips found 51 clips with directly
  authored, partial-allowed bicep, forearm, palm, and head motion. This makes a
  break clip the correct first visible-arm authority. Attack is not the first
  arm candidate.
- Found a second axial conversion issue in r222: each clip's own frame 0 was
  mapped to the outfit-1 bind pose. That erases stance-specific absolute
  orientation. For `gh3_guit_mido_a_med_break02`, the erased frame-0 offset is
  about `3-4` degrees through the torso and `12.486605` degrees at the head.
  The shared A-stance reference keeps attack effectively unchanged while
  preserving those authored medium-break offsets.
- Built unpromoted two-clip r223 from r222 with the shared A-stance reference
  (`gh3_guit_mido_a_attackl` frame 0):
  `analysis/gh3_midori_gh2_milos/gh3_midori_1_main_staticface_sharedstance_axial_ir_r223.milo_ps2`,
  SHA-256
  `816C52F0975FD701E067562504B79E2EEE64568C8B3FF96A49C29DD72FE3C3D1`.
  It replaces only attack and `gh3_guit_mido_a_med_break02`, preserves all
  inherited packed channels exactly, and has max patch replay error
  `0.000096049` degrees. Report:
  `analysis/gh3_midori_r223_outfit1_sharedstance_axial_ir_candidate_report.json`.
- Added `tools/gh3_midori_build_armchain_ir_candidate.py` and built unpromoted
  r224 on r223 for `gh3_guit_mido_a_med_break02`. It patches directly authored
  collars, biceps, and forearms; resets forearm/palm positions to outfit-1 bind
  offsets; keeps palm rotation neutral; and leaves fingers and guitar inherited.
  Candidate:
  `analysis/gh3_midori_gh2_milos/gh3_midori_1_main_staticface_medbreak02_direct_armchain_ir_r224.milo_ps2`,
  SHA-256
  `A8FF88954D7A288889CF2A89F024663BDEEA66E995A7B5903B53381BE8EFC533`.
  Inherited replay is exact, max patch replay error is `0.000104370`, max
  adjacent arm step is `19.681286` degrees, and max segment-length drift is
  `0.000035715`; structural status passes. Frame 106 gives both elbows a clear
  bend and supersedes frame 292 for the next visual. Report:
  `analysis/gh3_midori_r224_outfit1_direct_armchain_ir_candidate_report.json`.
- Next turn: capture exactly one outfit-1 r224 image of
  `gh3_guit_mido_a_med_break02` frame 106 with a side-readable camera. Judge
  pelvis through head and collar/bicep/forearm coherence. Palms, fingers, and
  guitar remain outside that verdict. Do not promote r222/r223/r224 yet.
- Verification: Python pipeline tests pass `143/143`; native
  `milo_convert_test` passes. Both loose-DLC mirrors remain exactly four
  canonical banks with main SHA-256
  `2902C712F81ED32A8D7D14E147453F16196C7AE935E5F3D029C179648A36AC8B`.
  No GHOGX/PCSX2 process remains. All CPU work ran at Idle priority; no ISO was
  mounted or used at runtime.

### 2026-08-25 full source-to-source outfit-1 body bank passes by data

- The earlier image-driven r222/r223/r224 next-step instructions are
  superseded. The user explicitly requires data-based acceptance because still
  images lose depth. No image was generated or used in this phase.
- The authoritative retained GH3 PS2 Midori bank contains `239` eligible body
  clips and `37,474` authored 30 fps poses over `1,241.17` seconds. Every one
  has an exact name-identical destination in the ordinary 280-clip GH2 Midori
  main MILO. The 41 destination-only clips are GH2 stock/default/face-call
  entries and remain unchanged.
- Added the resumable Idle-priority runner
  `tools/gh3_midori_full_bank_pipeline.py`. It automates retained GH3 IR to
  source GLB, source GLB to converted-Midori target GLB, every-frame Blender
  validation, calibrated GLB-to-ACP conversion, 239-clip native MILO assembly,
  and per-clip MILO replay. Checkpoints are dependency/hash gated. The runner
  uses one child process at a time, sets child priority to Idle, and limits
  common numeric thread pools to one thread.
- All stages pass `239/239`: source GLB exports, target retargets, GLB
  validations, ACP encodings, and native MILO replays. The GLB validators scan
  all `74,709` scene frames for exact target hierarchy/bind, noncollapsed biped
  topology, invariant segment lengths, finite/stable deformation, and
  serialized matrix round-trip. They compare all `37,474` authored poses for
  source-target chain direction, joint angle, and pelvis trajectory.
- Full-bank worst cases are: chain direction `0` degrees; joint angle
  `0.005300227` degrees; pelvis trajectory `0.000024426` units; relative
  segment length `0.000021550`; serialized GLB matrix component `0.000810921`;
  and deformation bounds ratio `1.642674`.
- The native candidate contains exactly `37,474` MILO samples and exactly 23
  channels per converted clip: 22 body quaternions plus
  `bone_pelvis.mesh.pos`. Face, finger, and guitar channels are absent. Every
  changed clip preserves its base body flags and empty event/enter/exit
  metadata; the clipset root header and all 41 GH2-only clips are unchanged.
- MILO replay against each validated GLB passes all `239/239` clips. Full-bank
  worst cases are: channel rotation `0.000110991` degrees; channel position
  `0.000436188`; local rotation `0.000348921` degrees; local position
  `0.000436188`; cumulative world rotation `0.002658366` degrees; cumulative
  world position `0.000660895`; and quaternion norm error `0.000000928`.
- Canonical aggregate:
  `.codex/current-evidence/midori-source-source-20260825/gh2/midori_outfit1_converted/full_bank/full_bank_validation.json`.
  Pipeline manifest is adjacent as `pipeline_manifest.json`; both report
  `status=ok`.
- The unpromoted ordinary GH2 candidate is
  `.codex/current-evidence/midori-source-source-20260825/gh2/midori_outfit1_converted/full_bank/gh3_midori_main.239_body_retarget.milo_ps2`,
  10,059,528 bytes, SHA-256
  `5455296780C38F48D74C11FC39F31ED9901C7D1D5A8F6054B13717244CDDF9CC`.
  The live loose main was not modified and remains SHA-256
  `2902C712F81ED32A8D7D14E147453F16196C7AE935E5F3D029C179648A36AC8B`.
- Supporting durable changes: generic retarget action labels now end in
  `_to_target_body`; clip inventory parsing records event/enter/exit metadata;
  multi-clip MILO validation accepts a declared change set and verifies root
  header, flags, events, unchanged clips, channels, timing, and local/world
  matrices; `tools/gh3_midori_full_bank_aggregate.py` consolidates the full
  result.
- Native `milo_convert_test` passes and Python pipeline tests pass `143/143`.
  No ISO, game runtime, or visual capture was used. Remaining scope is an
  explicit promotion/runtime decision and the deferred finger/guitar layers;
  face remains intentionally static/no-op. Keep the active goal open.

### 2026-08-26 source-derived guitar/hand completion and loose deployment

- This section supersedes the body-only channel/deferred-arm statements above.
  Direct retail Casey inspection identified the missing model contract:
  `main.drv` owns itself, each hand MIDI driver owns its side weight, and each
  `CharIKHand` starts at weight `0` with that same side-weight owner. The model
  emitter now serializes that exact graph. Corrected outfit-1 model SHA-256 is
  `A3BFB63851E09A71CA3BB9FF31643F49A850C789102CECC8E12D676AD5C414D1`;
  its v2 model manifest passes all controller, guitar-proxy, and weight-graph
  checks with 46 meshes, 71 transforms, and 70 nonidentity transforms.
- Added a source-derived hand-IK policy over every authoritative GH3 sample.
  Natural left reach separates at attached max `1.665586440`, detached min
  `3.606086270`, threshold `2.450764859`; right separates at attached max
  `1.557897276`, detached min `3.942201123`, threshold `2.478213952`. Only
  `gh3_guit_midori_a_loss3` and `gh3_guit_midori_a_win2` are detached on both
  sides. The policy only vetoes existing GH2 hand bits; it never enables a
  side whose base clip disabled it.
- Rebuilt the complete 239-clip bank with exactly 25 converted channels per
  clip: 22 body quaternions, pelvis position, and source-derived
  `bone_pos_guitar.mesh` position/quaternion. Face remains static/no-op.
  Finger animation stays in the ordinary GH2 fret/strum layers, and native
  hand IK is layered only where the source policy and base flags both permit
  it. All 41 GH2-only clips remain unchanged.
- Fresh provenance passes at every level: source, retarget, Blender body
  validation, guitar reconstruction, hand policy, ACP, serialized MILO replay,
  and native controller replay all pass `239/239`. Authored, guitar, ACP,
  serialized, and runtime inventories are all exactly `37,474` samples.
  Detached `loss3`/`win2` replay with authored arms untouched; enabled clips
  change only upper-arm/forearm locals, reach exact palm positions, preserve
  source-signed elbow bend, and leave the guitar controller-stable.
- Canonical candidate:
  `.codex/current-evidence/midori-source-source-20260825/gh2/midori_outfit1_converted/full_bank/gh3_midori_main.239_body_guitar_ik_retarget.milo_ps2`,
  11,156,190 bytes, SHA-256
  `0EC2FAEA6F2C0524E8DDF40EB7E07DE76CE00044ABE9B5DD915D1016212BCE0D`.
  Aggregate v2 at adjacent `full_bank_validation.json` reports `status=ok`,
  no failed gates, 239 source clips, 280 destination clips, and 37,474 native
  runtime frames. Key maxima: MILO world rotation `0.002624933` degrees,
  world position `0.000525095`, runtime guitar rotation `0.000041194`
  degrees, enabled palm position `0`, enabled palm rotation `0.000002958`
  degrees, elbow bend matrix `0.000000148`, and enabled reach `2.588454372`.
- Added `tools/gh3_midori_source_bridge_deploy.py`. It refuses stale aggregate
  or model evidence, preserves the untouched donor, and deploys only the
  validated outfit-1 model/main bank into the source-tree loose DLC. Deployment
  and verify-only both pass; manifest:
  `analysis/gh3_midori_source_bridge_deploy_manifest.json`. The deployed main
  under `GuitarHeroOGX-main-ui-engine/DLC/community.gh3.midori` has candidate
  hash `0EC2...CE0D`. The analysis and hybrid donor remain hash
  `2902C712F81ED32A8D7D14E147453F16196C7AE935E5F3D029C179648A36AC8B`.
  The no-argument full-bank runner now uses the analysis donor, so a future
  hybrid deployment cannot silently turn the output into its own base.
- Verification: Python pipeline tests `143/143`; native `milo_convert_test`;
  `ghogx_character_clip_driver_flags_test`;
  `ghogx_character_left_hand_contract_test`; and
  `ghogx_character_source_truth_contract_test` all pass. The left-hand
  contract test was updated only to include the viewer's two newer dump
  arguments in its exact call-path assertion. All work ran sequentially at
  Idle priority. No ISO was mounted or used, and no image was generated.
- Keep the active goal open. The remaining external boundary is an actual GH2
  PS2 build/emulator load from loose/package assets; current evidence proves
  conversion, serialization, source-tree loose deployment, and headless native
  controller behavior, not a PS2 gameplay run.

### 2026-08-26 source-to-source bridge completion audit passes

- This section supersedes the instruction above to keep the goal open for a
  stock PS2 build/emulator run. The requested boundary is the automated,
  ark-external Guitar Hero Classic conversion; stock-disc/PCSX2 compatibility
  is useful future validation, but it is not part of this bridge objective.
- Fixed a checkpoint-lineage defect in
  `tools/gh3_midori_full_bank_pipeline.py`: policy, ACP, MILO replay, and
  runtime reports must now match both the expected input hash and resolved
  source path. Equal bytes at a deployed runtime path can no longer satisfy an
  untouched-donor checkpoint. The focused regression is in
  `tools/gh3_midori_pipeline_test.py`.
- Reran the complete pipeline at Idle priority from the untouched GH2 donor
  `analysis/gh3_midori_gh2_milos/gh3_midori_main.milo_ps2`, SHA-256
  `2902C712F81ED32A8D7D14E147453F16196C7AE935E5F3D029C179648A36AC8B`.
  All eight stages pass `239/239`: source GLB, target retarget, Blender
  validation, guitar reconstruction, source hand-IK policy, ACP encoding,
  MILO replay, and native runtime replay. The rebuilt candidate remains
  byte-identical at SHA-256
  `0EC2FAEA6F2C0524E8DDF40EB7E07DE76CE00044ABE9B5DD915D1016212BCE0D`.
- The refreshed aggregate passes every gate for 239 converted clips, 280
  destination clips, and 37,474 authored/MILO/runtime samples. Its SHA-256 is
  `1239E938FD035E8633FA6B06F8D292431F78223CA3C5E885398A9AA6C52B6D04`.
- Added and reran the literal all-record GH3 export. Every one of the 280
  authoritative animation records has a path-keyed GLB: 239 body/performance
  clips plus 41 source-only face, hand, accessory, default, and path-distinct
  frontend records. The idempotence pass reported `new_exports=0`. Face stays
  static/no-op in GH2; GH2 owns the fret/strum finger layers.
- Independent provenance passes 34/34 checks while rehashing 4 untouched GH2
  retail files, 296 GH3 retail PAK entries, all 239 body GLBs, and all 280
  animation-record GLBs. Report:
  `.codex/current-evidence/midori-source-source-20260825/source_asset_provenance.json`.
- Ordinary loose deployment is current without copying replacement bytes.
  Native Guitar Hero Classic catalog validation loads outfit 1 with 46 meshes,
  71 bones, and all 345 clips across banks `[280, 6, 23, 36]`; all 14 static
  face-call no-ops load intentionally. Native motion validation changes all 46
  meshes for `gh3_guit_mido_a_med_break02` between frames 0 and 106. No ISO is
  referenced or required at runtime.
- Requirement-level audit
  `analysis/gh3_midori_source_bridge_completion_audit.json` reports
  `status=complete`: 13/13 requirements pass, none fail, none are pending,
  1,673 report hashes and 280 source GLBs checked. Python regressions pass
  `146/146`. No worker remains, all CPU work used Idle priority, and no image
  was generated.

### Blocked follow-up requirements

- The native runtime capture invalidated the data-only completion verdict. It
  is a broad failed upper-body solve, not merely detached hands/guitar:
  clavicle/shoulder and arm-chain placement is incoherent, one hand is fully
  separated, the other arm/hand is collapsed or obscured, head/neck orientation
  is unnatural, and guitar root/orientation/contact are wrong. The legs remain
  recognizably bipedal, but pelvis outward through the upper body is not
  visually solved. Completion requires a coherent native-runtime full-biped
  capture; serialization, matrix replay, and deformation bounds alone are
  insufficient.
- Investigate the GH2 animation-call naming contract before final packaging.
  Do not yet assume that it is character-specific: determine whether stock
  characters share general clip names, use per-character names, resolve calls
  through DTB data, or combine those mechanisms. Trace the actual caller, DTB,
  and MILO name inventories first. Then automate any required deterministic
  naming, aliasing, or call translation while preserving Midori's converted
  motion and the original GH2 bank roles. Matching hierarchy alone is not
  sufficient.

### 2026-08-26 latest native visual proof hard rejected

- The user directly hard rejected
  `analysis/gh3_midori_frame106_headless/contact_v3_idle/native_capture_frame063/midori_1_medium_idle_f063_selected.bmp`.
  It must not be cited as acceptable visual proof, a solved biped, or evidence
  that the arm/guitar conversion is complete. The failure is broad and visible,
  including the arm silhouettes and guitar geometry; it is not excused by the
  rear three-quarter camera.
- The data-first contact work remains diagnostic only. Complete v2 helper banks
  pass all 26 isolated native controller clips, and selected `med_idle`,
  `bad_break01`, and `med_break02` pass serialized/native numeric contracts, but
  none of that supersedes the rejected image.
- The broad v7 guitar-contact rollout was stopped immediately after this user
  rejection. Exactly 93 of 239 clips have current passing v7 guitar reports;
  no rollout process remains. Do not resume catalog propagation until the
  visible failure in a representative native capture is corrected.
- Keep the active goal open. A future completion claim still requires a new,
  directly coherent native-runtime full-biped image; use screenshots sparingly
  and no more than one image per turn.

### 2026-08-29 source-bind/full-deformation correction is active

- The latest user verdict remains authoritative: the previous native capture
  has a broken torso/shoulder region and a suspect right ankle. It remains hard
  rejected and must not be reused as visual acceptance evidence.
- Root cause: the production model stage used
  `stock_retarget_scope=upper-limbs`. Any vertex with even a small upper-limb
  influence was moved to the Casey bind while the rest of the mesh stayed in
  Midori's source bind. The result was one mathematically inconsistent
  mixed-bind mesh, so no single animation retarget could satisfy the torso,
  shoulders, arms, and source-pivot ankle motion.
- `tools/gh3_midori_build_pipeline.py` now stages with
  `stock-retarget-scope=none`. The outfit-1 model remains wholly in the GH3
  source bind while retaining GH2 names, hierarchy, and runtime objects. The
  experimental MILO is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/model_milos_source_bind/gh3_midori_1.milo_ps2`,
  SHA-256 `F4F4B0B7FBE2731B1C0BA7E4C22A483B54AE903E166B3E847F0F64DB86F64CCC`.
- `tools/gh3_midori_blender_arm_retarget.py` v3 now transfers each mapped
  bone's complete bind-relative skin deformation, not rotation alone:
  `D_source = source_pose * inverse(source_bind)`,
  `D_target = similarity * D_source * inverse(similarity)`, and
  `target_pose = D_target * target_bind`. It keys local position, quaternion,
  and scale for all 48 mapped body/finger bones. Face remains static/no-op and
  guitar channels remain omitted from this body-stage proof.
- At `med_idle01` authored frame 126, all 4,607 exported vertices match the
  authoritative GH3 source deformation. Maximum errors are `0.000136197`
  overall/right ankle, `0.000056559` torso/shoulder, `0.000040038` left arm,
  and `0.000040807` right arm. All 209 authored frames pass the full-matrix
  sweep; the global maximum matrix-component error is `0.000083923`.
- The dynamic proof clip is `gh3_guit_mido_a_bad_break01`, selected because
  its source elbow ranges are 36.40-112.34 degrees left and 38.95-143.50
  degrees right. All 113 authored frames pass the full-matrix sweep. At
  authored frame 52, torso/shoulder error is `0.000065189`, right arm
  `0.000053787`, and right ankle `0.000176246`. One hand-dominant left-arm
  vertex has a preserved strict-gate outlier of `0.0269727` GH2 units
  (`0.038%` of character height), caused by the float32 GLB matrix round trip;
  its left-arm RMS is `0.00200003`. Keep both the strict rejected report and
  the explicit `0.05` float32-tolerance report; do not silently erase it.
- ACP now emits dense `.pos` and `.quat` channels for every mapped bone. Both
  clips serialize as 96 channels per sample with hand-IK flags cleared. The
  two-clip MILO candidate is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/gh3_midori_main.two_clip_source_bind_full_deformation.milo_ps2`,
  SHA-256 `D9FB3A5545E4A32FA995544E68CCEA21448A2CA12D5E3483E56C1A08BB6F6817`.
  Dynamic-clip MILO replay maxima are `0.0000031273` world position and
  `0.0000174136` degrees world rotation.
- Native loose-asset replay (no ISO) of dynamic sample 26, corresponding to
  source/GLB frame 52, loads 46 meshes and all 96 channels. Every native mesh
  bound matches the GLB pose; maximum component error is `0.000113231`.
  `tools/gh3_midori_candidate_runtime_probe.py` now performs the temporary
  candidate install atomically at Idle priority and refuses success unless the
  installed model and main MILOs are restored to their original hashes. The
  installed hashes remain model `A3BFB638...` and main `0EC2FAEA...`.
- Production checkpoint promotion is in progress: full-bank retarget checks
  now require v3 full-matrix/translation metrics; validation uses the new
  all-authored-frame deformation sweep; donor construction preserves
  `Control_Root -> bone_pelvis.mesh`. Python regressions pass 157/157.
- One `med_idle01` rear three-quarter image was generated during diagnosis but
  is intentionally not offered as proof: its authored arms are nearly straight
  at about 172 degrees, so it is not discriminating. The next and only visual
  proof image should use native `bad_break01` sample 26 from a useful
  three-quarter angle. Keep the active goal open until that image is coherent;
  numeric success still does not supersede the user's visual acceptance.

### 2026-08-29 direct full-deformation pipeline and guitar-root promotion

- The source-bind correction now includes the instrument root. The rig mapping
  transfers authoritative GH3 `bone_guitar_body` deformation directly to GH2
  `bone_pos_guitar.mesh`; the old Casey contact solver, hand-IK policy layer,
  upper-arm twist gauges, and separate fret/strum banks are no longer part of
  the production full-bank pipeline. Face remains static/no-op.
- The full-deformation transfer now covers 49 mapped bones and writes 98 dense
  `.pos`/`.quat` channels. Every authored frame of `bad_break01` and
  `med_idle01` passes the Blender deformation sweep. Dynamic maxima are
  `0.000055313` matrix / `0.000063528` translation for torso and shoulders,
  `0.000705719` / `0.000933635` for the right ankle, `0.000099182` for the
  right arm, and `0.000045776` for guitar attachment. All remain below the
  explicit `0.001` full-matrix tolerance; the larger dynamic leg values are
  float32 GLB round-trip error, not a different ankle solve.
- `tools/gh3_midori_full_bank_pipeline.py` is now a direct six-stage pipeline:
  source GLB, full-deformation retarget, every-authored-frame sweep, ACP,
  candidate MILO, and serialized MILO replay. Dormant legacy stages and their
  required inputs were removed rather than merely hidden. Python regressions
  pass 157/157.
- A fresh two-clip ordinary pipeline run passes 2/2 at every stage. Candidate:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_direct_two_clip/gh3_midori_main.2_full_deformation.milo_ps2`,
  SHA-256 `8810AEF725CF518C32CD424ECC8393EFC55EBC545E22F8407C8DA560BD0B27E6`.
  Manifest SHA-256 is
  `DB2FEB30F324A610A6AE9A2C638673FD615434EA000A90DF5A8B7E7F5F6932C1`.
  MILO replay maxima are `0.000003522` world position and
  `0.000017414` degrees world rotation across the two clips.
- The exact automated candidate passes native loose-asset replay at dynamic
  sample 26 with 46 meshes. Post-controller `bone_pos_guitar` matches the
  serialized MILO transform within `0.000032147` position and
  `0.000000519` rotation component. Validation report SHA-256 is
  `F98077D92A6FC8A8D25CC4351C93E704DB03D5F9F68EF7B70F222256909340B8`.
  Installed assets were restored; no ISO was mounted or used.
- The single native image generated before direct guitar-root promotion,
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/native_bad_break01_frame026/midori_1_bad_break01_f026_front_three_quarter.bmp`,
  is useful only as body evidence: it reads as a coherent biped with repaired
  torso/shoulders, bent arms, attached hands, legs, and right ankle, but its
  guitar is upright behind/through the head. It is not final visual acceptance.
  The next turn may capture exactly one image of the new automated 98-channel
  candidate at the same dynamic sample and camera. Keep the goal active.

### 2026-08-29 native prop-basis correction after body repair

- The direct-root image at
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_direct_two_clip/native_bad_break01_frame026_visual/midori_1_bad_break01_f026_front_three_quarter.bmp`
  is not acceptance evidence. The source-bind/full-deformation body is visibly
  bipedal there, but the guitar is displaced far to character-left. That image
  and the earlier direct guitar-root policy are superseded.
- Native retail Xplorer inspection established exact local contact positions
  under `bone_pos_guitar.mesh`: fret `[-3.102310, -0.158573, 22.415691]` and
  strum `[4.182864, -6.415741, -0.025471]`. The first calibration revision
  incorrectly treated those native GH2 row-vector offsets as Blender
  column-vector offsets. It therefore appeared correct in GLB while producing
  the wrong native prop orientation. The explicit bridge is now native GH2
  local `[x,y,z]` to Blender local `[x,z,-y]`.
- `tools/gh3_midori_blender_guitar_basis_calibrate.py` v2 derives one constant
  right-multiplied correction from the canonical authoritative GH3 guitar pose
  to those retail GH2 prop contacts. The full-bank runner now has a required
  calibration checkpoint before source export/retarget. Stale v1 calibration
  reports are rejected. `tools/gh3_midori_blender_target_contact_selector.py`
  v2 applies the same axis boundary when ranking proof poses.
- A fresh one-clip pipeline in
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_native_basis_guitar_one_clip`
  passes calibration, source export, retarget, all-authored-frame deformation
  sweep, ACP, MILO construction, and serialized MILO replay. Candidate SHA-256
  is `0EA66A00358EF6A956DFB73038172E9B11070BD41EFEC9756218FB3F63F3E69C`.
  Calibration reduces total two-hand contact error from `57.787934` to
  `4.637966`, split evenly at about `2.31898` GH2 units per hand because the
  Midori hand-center span and retail Xplorer contact span differ.
- `fst_break01` source frame 0 / native sample 0 is the only sampled frame with
  both hands near their retail contacts. Elbow bends are `76.8908` degrees
  left and `132.8014` right. Native loose replay places fret contact at
  `[-12.386, 9.357, 53.453]` versus left hand
  `[-11.1744, 8.1693, 51.8713]`, and strum contact at
  `[0.365, -3.142, 36.808]` versus right hand
  `[-0.8461, -1.9543, 38.3895]`.
- Added `tools/gh3_midori_native_prop_contact_compare.py`. Its automated
  GLB-to-native gate passes with maximum position drift `0.000582403`; native
  contact errors are `2.319565` left and `2.319167` right, each about `10.5%`
  of pelvis-to-head height. The runtime probe uses only loose assets and
  restores installed model/main hashes atomically. No ISO is used.
- The body correction remains independently current. Across all 172 authored
  `fst_break01` samples, torso/shoulder maximum matrix/translation errors are
  `0.0000724792` / `0.0000858095`; right ankle maxima are `0.000226021` /
  `0.000268917`. MILO replay maxima are `0.0000188605` degrees world rotation
  and `0.00000346719` world position. These metrics prove transport fidelity,
  not visual acceptance.
- Python regressions pass `159/159`. All work remains sequential at Idle CPU
  priority. Exactly one image was generated in this turn, and it was the
  superseded guitar-left result above. On the next turn, generate at most one
  native image of the v2 calibrated candidate at `fst_break01` sample 0. Do
  not claim completion unless the full body, hands, and guitar are visually
  coherent. Keep the active goal open.

### 2026-08-29 rejected calibrated frame and corrected 60-fps pose selection

- The one native image generated in this turn,
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_native_basis_guitar_one_clip/native_fst_break01_frame000_visual/midori_1_fst_break01_f000_three_quarter.bmp`,
  is hard rejected by the user. It must not be cited as visual acceptance. The
  raised fret hand visibly misses the neck and the overall result remains a
  hard fail even though the repaired torso/legs and calibrated prop root are
  numerically coherent. No second image was generated.
- Found a clock defect in both proof-pose selectors. Imported 60-fps GLBs were
  evaluated in Blender's factory-default 24-fps scene. A reported selector
  frame 152 therefore sampled source frame 380. The source-only selector and
  the first exact-conversion selector agreed with each other but did not agree
  with ACP/MILO/native frame indexing. The earlier
  `conversion_pose_selector.full_bank.json` v1 ranking and target selector v2
  report are invalid proof-pose evidence.
- Fixed `tools/gh3_midori_blender_conversion_pose_selector.py` and
  `tools/gh3_midori_blender_target_contact_selector.py` to pin scene FPS to 60
  after each factory reset. Their formats are now
  `gh3_midori_conversion_pose_selector_v2` and
  `gh3_midori_target_contact_selector_v3`; both record `scene_fps=60`.
  `tools/gh3_midori_native_prop_contact_compare.py` rejects stale selector
  formats. Regression tests pin the 60-fps contract.
- The corrected exact scan covers all 239 body clips and 37,474 sampled source
  frames. Valid report:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/conversion_pose_selector.full_bank.v2.json`,
  SHA-256
  `B04AAA4C9B0797BEDA3DCB44DA5FD2C3CFA763EE3128EF4B9C58FD103ACD0326`.
  The best corrected pose is `gh3_guit_midori_c_med_break04` source frame 378,
  ACP/MILO sample 189. Elbows are `105.8241` and `122.3943` degrees; final
  retail-contact errors are `1.963039` left and `1.120372` right. The same
  clip's prior frame-152 claim is discarded.
- A fresh one-clip production conversion passes calibration, source export,
  full-deformation retarget, every-authored-frame validation, 98-channel ACP,
  MILO construction, and 205-frame serialized replay. Candidate:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_exact_break04_one_clip/gh3_midori_main.1_full_deformation.milo_ps2`,
  SHA-256
  `C4E710495AA5A828A8502DC315A86ABACF4AC357C25AE5B1082A344FCAE8A73C`.
  Independent target selector v3 reproduces source frame 378 with the same
  contact values; report SHA-256 is
  `9ED4B4B663AD870BE041AA304AA106F260CE49C9F16427A511B874BDBF4E4A79`.
- Native loose replay of MILO sample 189 reproduces the corrected GLB
  pelvis/head/hands/prop contacts within `0.0000883971` GH2 units. Extended
  `gh3_midori_native_prop_contact_compare.py` v2 to derive retail contacts from
  the runtime `bone_pos_guitar` world matrix when non-IK clips omit redundant
  prop-debug rows. It verifies the retail anchor hash and projects native
  row-vector local offsets directly. The gate passes with normalized contact
  errors `0.0986914` left and `0.0563246` right. Installed model/main assets
  were atomically restored.
- Tested the untouched GH2 two-hand controller flags on this corrected source
  pose. Controllers leave pelvis, spine, neck/head, both clavicles, both upper
  arms, and guitar root unchanged. Left IK closes the fret hand exactly, but
  right IK moves the hand away from the actual retail Xplorer strum contact
  because GH2's character-side `bone_strum_hand` helper is offset from the prop
  contact. The right contact error worsens from `1.120` to about `2.08`; the
  two-hand trial is not promotable.
- Built an isolated left-only controller trial. It preserves the source-authored
  right arm and moves only the left forearm/hand onto the fret target. Candidate:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_exact_break04_left_ik_trial/gh3_midori_main.left_ik_trial.milo_ps2`,
  SHA-256
  `2CB774F8347E9B037984F8926AF8A56479DAE76E18289386AAD1084BC508AEEF`.
  All 205 serialized samples and 98 channels pass replay. This is the leading
  next visual candidate, not a production/full-bank policy.
- Next turn: generate exactly one native image of outfit 1,
  `gh3_guit_midori_c_med_break04` MILO sample 189, using the left-only trial and
  a side-readable three-quarter camera. Inspect the full biped directly. Do not
  propagate controller flags until that image passes. If it fails, reject it
  immediately and return to data. Face remains static/no-op; full-bank
  controller policy and animation-call naming audit remain pending.
- Verification: Python regressions pass `162/162`. All work used Idle CPU
  priority, single-thread numerical libraries, local extracted `GEN` plus loose
  DLC, and no ISO. Keep the active goal open.

### 2026-08-29 rejected left-IK image and source-bind lineage repair

- The one native image generated for this turn,
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_exact_break04_left_ik_trial/native_frame189_visual/midori_1_med_break04_f189_left_ik_yaw225.bmp`,
  is hard rejected. The torso/shoulder region and right ankle read as broken,
  the guitar is nearly vertical through the torso, and neither visible palm
  reads as useful playing contact. It must not be cited as acceptance. No
  further image was generated this turn.
- The rejection exposed a concrete input-lineage regression. The later
  full-bank defaults retargeted against obsolete target GLB SHA-256
  `44F7F6DDC97D4C7EF19691E714B41739DC0672D5F1078AEC39E0B84FB3D6F018`
  while native replay used the source-bind runtime model SHA-256
  `F4F4B0B7FBE2731B1C0BA7E4C22A483B54AE903E166B3E847F0F64DB86F64CCC`.
  The pipeline's default runtime model was also the obsolete DLC copy
  `A3BFB63851E09A71CA3BB9FF31643F49A850C789102CECC8E12D676AD5C414D1`.
  The authenticated target for the source-bind model is
  `target_bind_source_model/midori1_source_bind_target.glb`, SHA-256
  `329AAFA5892EE38CD5AD85F35E8EBB4A26091045BC7D42C468C17E4B16006E80`.
- Added `tools/gh3_midori_blender_hand_landmarks.py` to derive visible-palm
  landmarks from vertices weighted at least 0.9 to each hand bone. The
  obsolete target reports local offsets of about 2.005 left / 1.398 right.
  The authenticated source-bind target reports 5.600585 left / 5.950690
  right with bind prediction errors below `0.000008`; those offsets match the
  actual native visible-hand displacement from the hand pivots.
- `gh3_midori_full_bank_pipeline.py` v7 now defaults to the authenticated
  target and source-bind runtime MILO and refuses to run unless the target
  sidecar proves that its GLB hash was exported from that exact model hash.
  Arm/finger entry points use the same target. Guitar calibration v3 fits the
  retail prop to weighted visible palms, body retarget rejects pre-v3
  calibration, conversion selector v3 and target selector v4 rank visible
  palms, and both selectors reject odd 60-fps frames that cannot be encoded in
  the 30-fps MILO.
- Fresh one-clip source-bind pipeline:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_source_bind_visible_palm_break04_one_clip`.
  Calibration, source export, retarget, all-authored-frame sweep, ACP, MILO,
  and serialized replay all pass. Candidate SHA-256 is
  `620736B4C77BFFD90494E4478C7D5D641C552BB8FB9DEC3770D053DCA2C4A8B2`;
  manifest SHA-256 is
  `BD925594EBC5BAAC855E8BA66486336A877DB08851C82E630F88B0DC705F02F6`.
- The rebuilt animated GLB retains palm offsets 5.603374 left / 5.945370
  right. Across all 205 authored frames, torso/shoulder maximum
  matrix/translation errors are `0.000061512` / `0.000065547`; right-ankle
  maxima are `0.000323296` / `0.000343133`. A loose-asset native audit of
  pelvis, spine1-3, neck, head, both clavicles, both upper arms, and right
  ankle passes at sample 189. Worst runtime rotation component error is
  `0.000000600` at the neck and worst position error is `0.000047333` at
  the left clavicle.
- Target selector v4 now chooses the encodable source frame 378 / MILO sample
  189. Weighted visible-palm errors are 2.478535 left and 0.700047 right.
  Native replay agrees with the GLB within `0.000531603`, but the palm-contact
  report remains intentionally rejected because the left normalized error is
  `0.124602`, above the `0.12` gate. Do not weaken the gate. The previous
  left-only native controller trial targeted the hand pivot and is superseded;
  any next arm correction must account for the visible-palm local landmark.
- Verification passes `164/164` Python regressions. All commands remained
  sequential at Idle priority with single-thread numerical libraries. Runtime
  used extracted `GEN` plus loose DLC, restored installed hashes, and did not
  mount or read an ISO. Keep the active goal open. On the next turn, generate
  at most one native image from the new source-bind candidate at sample 189 to
  judge the repaired torso/shoulders/right ankle before any full-bank policy.

### 2026-08-29 rejected break04 body, native surface gate, and win3 replacement

- The one native image generated in this turn,
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_source_bind_visible_palm_break04_one_clip/native_frame189_visual/midori_1_med_break04_f189_source_bind_yaw225.bmp`,
  is hard rejected. The user identified the whole torso/shoulder area and the
  right ankle as broken. It must not be cited as acceptance. No second image
  was generated, honoring the one-image-per-turn limit.
- Added a data-only native deformed-pose route to
  `tools/gh3_midori_candidate_runtime_probe.py`. `--dump-deformed-pose` writes
  the existing `GH2POSED` vertex/normal payload without a screenshot. Loose
  assets are used, installed model/main files are restored, and no ISO is
  involved.
- `tools/gh3_midori_blender_native_deformed_compare.py` is now v3. It compares
  the converter-input bind GLB's raw index and attribute streams directly with
  the Mesh28 stage/native dump, recovers animated corner identity with bind
  position/weights/UV/normal, compares native posed positions, explicitly
  evaluates GH2 direct weighted normal transport, and gates torso/shoulder and
  right-ankle triangle area ratios. Blender modifier display normals remain a
  non-gating diagnostic because they use different normal behavior.
- The bind GLB is byte-for-byte semantic evidence for the model conversion:
  raw topology is exact, maximum position drift from the Mesh28 stage is
  `0.00000379633`, normal drift is `0.0000000333447`, UV drift is zero, and
  semantic weight drift is `0.00000000238419`. At rejected `break04` source
  frame 378/native sample 189, native positions still match the animated GLB
  within `0.000178895` overall, `0.000068850` torso/shoulders, and
  `0.000152433` right ankle. Thus topology and runtime transport were not the
  source of the visible failure.
- The new surface gate agrees with the user's rejection. At `break04` frame
  378 the torso/shoulder surface compresses to `0.0789411` of bind area and
  stretches to `4.77911`; the required coherent interval is `[0.2, 3.0]`.
  The v3 report is intentionally `rejected` only on
  `torso_shoulder_surface_area_coherent`. This closes the prior false positive
  where matrix fidelity alone was treated as body coherence.
- A first replacement trial, `gh3_guit_mido_a_fst_break01` frame 120/native
  sample 60, was rejected without an image because its surface gate was worse:
  23 torso/shoulder shaded-normal inversions, minimum area ratio `0.073733`,
  and right-ankle minimum area ratio `0.407331`.
- Added `tools/gh3_midori_blender_surface_frame_rank.py`. It pins Blender to
  60 fps, scans every encodable frame, rejects triangle area ratios below
  `0.2` or above `3.0`, and penalizes unreadable elbow bends. Six fresh
  source-bind retargets were searched in addition to `break04` and
  `fst_break01`. The best usable result is `gh3_guit_midori_a_win3` source
  frame 96/native sample 48: zero severe torso/ankle faces, elbow bends
  `83.3023` and `92.3241` degrees, torso area `0.206383..2.63826`, and
  right-ankle area `0.431232..1.97776` in the broad ranking pass.
- Promoted that one clip through all-frame validation, ACP, MILO construction,
  and serialized replay in
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_source_bind_body_pose_search`.
  Candidate main SHA-256 is
  `7B1A31A852C5DFA4989DE03A5F072BCF7BC3648F4E3CC314A261287F3F2A9A05`;
  the model remains source-bind SHA-256
  `F4F4B0B7FBE2731B1C0BA7E4C22A483B54AE903E166B3E847F0F64DB86F64CCC`.
- Native sample 48 passes the v3 data gate. Maximum animated-GLB/native
  position drift is `0.000429973` overall, `0.0000775300` torso/shoulders,
  and `0.000187912` right ankle. The focused native surface interval is
  `0.206406..2.63815` torso/shoulders and `0.431218..1.43797` right ankle.
  Explicit GH2 weighted-normal evaluation matches native at `0.183521`
  degrees maximum and `0.0106024` degrees RMS; the large Blender display-normal
  delta is therefore not a conversion defect.
- Verification passes `165/165` Python regressions. All commands remained
  sequential at Idle priority with single-thread numerical libraries. Keep the
  active goal open. Next turn may generate exactly one native image of outfit
  1, `gh3_guit_midori_a_win3` sample 48, for visual acceptance. Do not call it
  solved from data alone. Arms/hands/fingers/guitar remain after the body passes.

### 2026-08-29 win3 sample-48 native body visual

- The turn's single native image is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_source_bind_body_pose_search/native_win3_frame048_visual/midori_1_win3_f048_yaw225.bmp`.
  The loose-asset runtime probe passed, loaded all 46 meshes, restored the
  installed model/main hashes, used Idle priority, and did not use an ISO.
- This frame is materially different from the rejected `break04` proof. The
  pelvis-through-head silhouette reads as one coherent body, both shoulders
  remain attached to the torso, and the right ankle is attached. The legs are
  crossed as part of the authored `win3` pose rather than torn apart.
- This is only a body-boundary candidate pending the user's visual verdict.
  The arms/hands are not solved and the black guitar is badly placed through
  the body; neither is being treated as accepted. Do not proceed outward from
  the shoulders until the user accepts pelvis/torso/neck/head/legs/ankles in
  this exact frame, or supplies a new body correction.
- No second image was generated. Keep the active goal open.

### 2026-08-29 projected-leg body proof at yaw 165

- The preceding yaw-225 proof preserved healthy 3D joint separation but hid
  the ankles in camera depth. `tools/gh3_midori_blender_surface_frame_rank.py`
  is now v3: in addition to 60-fps surface strain and elbow metrics, it uses
  the native orbit-camera right axis `[cos(yaw), sin(yaw), 0]` to score
  projected knee, ankle, and toe separation across restrained front and
  three-quarter camera candidates.
- For `gh3_guit_midori_a_win3` source frame 96/native sample 48, the selector
  retains the same zero-severe-face pose and chooses yaw 165. Projected ankle
  separation rises from `0.121294` pelvis-to-head units at yaw 225 to
  `0.242231` at yaw 165; projected knee/toe separation is `0.249090` and
  `0.292096`. No animation or conversion data changed.
- The turn's single native image is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/pipeline_source_bind_body_pose_search/native_win3_frame048_visual_yaw165/midori_1_win3_f048_yaw165.bmp`.
  Both knees, ankles, and shoes are separately readable. The pelvis-through-head
  body and both shoulder attachments remain coherent. The arms/hands and black
  guitar are still visibly unresolved and remain outside this body-only
  candidate.
- The loose-asset runtime probe passed with 46 meshes and restored installed
  model/main hashes. Verification remains `165/165`. All work stayed
  single-threaded at Idle priority, with no ISO. Keep the active goal open and
  await the user's body-boundary verdict before proceeding outward into arms.

### 2026-08-30 source-authority closure and hierarchy-aware hand palette

- Supersede the two preceding visual conclusions. The user hard rejected the
  yaw-165 sample-48 proof, specifically the torso/shoulder region and right
  ankle. It must not be cited as accepted. The user later said the work was
  "starting to look more coherent," but no frame has received formal visual
  approval and the active goal remains open.
- Retail Casey `rock1_stand_medium_02` and Midori rebound unchanged to Casey's
  rig have comparable surface-strain distributions. Midori therefore survives
  an untouched GH2 animation about as well as the retail mesh. The rebound GLB
  imports an action range of 0..75 at 30 fps although its report says 0..150;
  keep this exporter/report discrepancy recorded.
- An independent Neversoft MultiTool SKE/SKA extraction and a pure raw-glTF
  pose evaluator now clear GH3 compressed keys, SKE bind matrices, hierarchy
  composition, basis conversion, and Control_Root cancellation. Across win3
  frames 0/48/96, mapped world rotation error is at most `0.0010744` degrees
  and world position error at most `0.00000518`. The report is
  `analysis/gh3_midori_nmt_win3_pose_composition_audit.json`.
- Direct source-GH3 versus converted-target deformation at source frame 96
  cleared torso/shoulders (`0.0000552` max), right arm (`0.0000411`), and
  right ankle (`0.0002148`). The only conversion outlier was source mesh 0,
  vertex 3073 in the left hand: GH2's four-bone Mesh28 triangle palette had
  discarded a `0.21875` pinky-tip influence and renormalized the palm and
  middle-finger weights, producing `0.06588` error.
- `tools/gh3_midori_model_stage.py` now reduces an over-palette finger triangle
  by collapsing a distal joint into an already retained proximal joint on the
  same chain before weight ranking. Excluded finger weight is then remapped to
  the nearest retained same-chain ancestor, with same-side hand as the final
  fallback. This is a general hierarchy rule, not a Midori vertex override.
  `tools/gh3_midori_blender_deformed_corner_compare.py` now reports per-vertex
  and full-triangle legal palette counterfactuals using posed bone matrices.
- For both bad left-hand triangles, data selects `index03 -> index02`, retaining
  the palm, middle02, and pinky03 influences exactly. The analogous right-hand
  overflow selects `middlefinger02 -> middlefinger01`. Outfit 1 restages as 45
  chunks with all 4,992 faces and zero dropped source weight, then builds as a
  normal loose GH2 PS2 character MILO at
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/model_milos_ancestor_fallback/gh3_midori_1.milo_ps2`.
- The formerly rejected source frame 96 now passes source-to-target deformation:
  left arm/hand maximum error is `0.0000367`, right arm `0.0000462`,
  torso/shoulders `0.0000552`, and right ankle `0.0002148`. Ten evenly spaced
  win3 poses from frame 0 through 540 all pass those focused gates. A pure
  index03 vertex reaches `0.02364` at the loop endpoints after index03 is
  folded into index02; this is about 0.04 percent of character height and is
  explicitly deferred finger detail, not an arm-chain failure.
- The fresh win3 ACP and ordinary MILO replacement are at
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/ancestor_fallback_validation`.
  Serialized replay passes all gates across 271 samples and 98 channels;
  maximum world rotation error is `0.0000206` degrees and maximum world
  position-component error is `0.00000503`. Face channels remain absent/static.
- This turn generated exactly one native image:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/ancestor_fallback_validation/native_win3_frame086_yaw150/midori_1_win3_f086_yaw150.bmp`.
  It is win3 native sample 86 (source frame 172), yaw 150, chosen for readable
  bent elbows and separated legs. The loose-asset probe passed 45/45 meshes,
  wrote the native deformed payload, restored both installed MILOs byte-for-byte,
  used Idle priority, and did not access an ISO. The image is an unapproved
  visual candidate; do not call it solved.
- Native vertices at that frame match the animated GLB within `0.000393`
  overall, `0.0000804` torso/shoulders, and `0.000216` right ankle. The native
  report is nevertheless `rejected` by the absolute torso surface gate because
  one shoulder-region triangle reaches area ratio `0.17967` below the `0.2`
  threshold. Source-to-target deformation already matches the authoritative
  GH3 pose, so record this as an absolute authored-pose strain warning, not a
  transport mismatch. Visual acceptance remains the user's decision.
- `tools/gh3_midori_candidate_runtime_probe.py` now takes
  `--expected-mesh-count`; the old hardcoded 46-chunk check was invalid after
  legal repartitioning. Verification passes `169/169`. Continue outfit 1 only,
  data-first, one image per turn, Idle/single-threaded, loose extracted assets,
  static face, and no ISO. Guitar attachment remains intentionally unresolved.

### 2026-08-30 first current-model in-song run and guitar/visible-palm split

- Switched the acceptance boundary to actual Guitar Hero Classic song runtime.
  A staged loose-DLC tree combined the hierarchy-aware outfit-1 model SHA-256
  `13099CDE50E0DC5637CEF9A13F41DFD4AE1B0C0269FBE0CDA62920469A06F7A5`
  with the previously proven GH2-native Midori main/fret/strum banks. The
  installed DLC was not modified. The run used extracted `GEN`, never mounted
  or read an ISO, and ran sequentially at Idle priority.
- A no-image `Shout at the Devil` autoplay probe passed in the actual song
  loop. Midori loaded as `guitarist0`, the `flyingv_v2` prop attached to
  `bone_pos_guitar.mesh`, both hand drivers remained at weight `1`, and the
  active left wrist pivot was `0.2694` units from fret spot 11. Autoplay ended
  the bounded run at score `100`, streak `2`, hits `2`, misses `0`, with
  `state=playing`.
- The turn's only generated image is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/gameplay_model_baseline_trial/midori_outfit1_shoutatthedevil_f60.bmp`,
  SHA-256
  `558CD3980795F2EA1B43F22F9A835AE478DD50CEB7BA1F017506BE93BABA48FE`.
  It is a real in-song frame at about song time 31 seconds with autoplay and
  the normal `stand_medium_01` performance call active.
- The frame is not an acceptance image. The GH2-native guitar basis now gives
  the prop a plausible scale and across-body diagonal placement, so do not
  return to the newer one-frame `156.8456` degree guitar-basis correction that
  produced the near-vertical viewer failure. However, the visible fret hand is
  displaced from the neck even though the wrist pivot contact metric passes.
  This is the expected difference between a wrist/palm bone pivot and Midori's
  weighted visible-palm landmark, whose left local offset length is
  `5.600586` units (right `5.859969`) in the hierarchy-aware target.
- The next fix is conversion-level, not an engine override: keep the current
  GH2-native guitar root channels unchanged and offset every fret/strum
  `bone_fret_hand` / `bone_strum_hand` target by the corresponding rotated
  visible-palm landmark. GH2 `CharIKHand` should then place the wrist behind
  the requested contact so the rendered palm lands on the fret/strum point.
  Validate all target samples headlessly, then spend the next turn's single
  image on another real-song capture. Keep the active goal open.
- The representative eight-clip hierarchy batch converted all eight GLBs but
  stopped before packaging because `gh3_guit_mido_a_bad_break01` exceeded the
  strict `0.001` all-bone serialization tolerance only at the distal left index
  chain (`0.0018413` translation). Torso, both arms, right ankle, and guitar
  attachment all passed their regional gates. Do not include that clip through
  a weakened gate; the failure is retained as finger-detail evidence.

### 2026-08-30 conversion-level palm-contact banks and real-song proof

- Preserved the accepted GH2-native guitar root and added the generic
  `milo_convert_tool patch-clipset-palm-contact` operation. It changes only a
  requested hand target's `.pos` rows using
  `target_pos - palm_local * target_local_rotation`; the rotation is decoded
  with the engine's actual HMX row-matrix quaternion convention. The automated
  wrapper is `tools/gh3_midori_patch_palm_contact_targets.py`. This is a MILO
  conversion change, not an engine pose override.
- The wrapper reads Midori's weighted visible-palm landmarks from the
  hierarchy-aware GLB calibration, converts Blender `[x,z,-y]` coordinates
  back to native GH2 `[x,y,z]`, patches the unique current fret/strum banks,
  and then independently samples every persisted target row. It validated all
  617 target samples: 21 fret rows and 596 strum rows. Target quaternions,
  clip/channel/event inventories, model, and main bank stayed unchanged. Worst
  reconstructed contact error was `0.0000028743`.
- Corrected hand-bank hashes are fret
  `9D81CCC2429A1D0DB4DD69148651106ADED6C4047C5CF80BB8219003585E181F`
  and strum
  `2BD1105BB5A9666BC7B259E4B8C936B225551253288F755E8A6961C0D2FCF52D`.
  The full report is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/gameplay_palm_contact_trial/palm_contact_patch_report.json`.
  Verification is `172/172` Python tests plus all native `milo_convert_test`
  checks.
- Staged a fresh loose-DLC runtime tree under
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/gameplay_palm_contact_trial/runtime_v1`.
  It retains model SHA-256
  `13099CDE50E0DC5637CEF9A13F41DFD4AE1B0C0269FBE0CDA62920469A06F7A5`
  and main bank SHA-256
  `0EC2FAEA6F2C0524E8DDF40EB7E07DE76CE00044ABE9B5DD915D1016212BCE0D`.
  The installed DLC was not modified. Both bounded runs used extracted `GEN`,
  no ISO, Idle process priority, and one thread.
- The no-image and capture runs both reached the actual `Shout at the Devil`
  song loop. Midori loaded 45/45 meshes with two `CharIKHand` controllers,
  `flyingv_v2` attached to `bone_pos_guitar.mesh`, the 31-clip/seven-map hand
  graph loaded, and both hand weights remained source-driven at `1.0` under
  `stand_medium_01`. The runtime exercised `finger_hold_pinky` plus
  `strum_short_01` initially and `finger_open` plus `strum_short_04` around the
  capture. Final state was playing at score 100, streak 2, hits 2, misses 0.
- The turn's one image is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/gameplay_palm_contact_trial/midori_outfit1_shoutatthedevil_palmcontact_f60.bmp`,
  SHA-256
  `F39F9ACA4C2CE71F857F14DB9E304D543C5E8B6CC6521EDE13F363A0D6754EB5`.
  Self-review passes immediate biped coherence, the guitar's scale/diagonal
  placement, and visible fret-hand contact on the neck. Do not overclaim full
  bilateral hand acceptance: the strum hand is partly occluded by the guitar
  body in this frame, and user visual acceptance is still pending. The compact
  run report is `gameplay_palm_contact_trial/gameplay_validation.json`.
- Keep the goal active. The next useful visual should use a song time/pose that
  exposes the strum arm and hand while retaining this exact model, main bank,
  guitar root, and corrected hand-bank hashes. Do not spend images rechecking
  the now-stable guitar root unless data changes.

### 2026-08-30 data-ranked strum-side gameplay proof

- Added diagnostic-only right upper-arm/forearm/hand pose rows and a front
  camera yaw-offset control to the engine. Added
  `tools/gh3_midori_rank_runtime_strum_visibility.py`, which ranks complete
  post-controller real-song poses by projected right-arm length, elbow bend,
  guitar-axis clearance, and wrist/strum-target error. These changes do not
  alter the converted model, animation banks, IK, or guitar transform.
- A no-image `Shout at the Devil` sweep logged 105 complete poses from song
  time 30.0 through 41.56 seconds. Seven candidate yaw offsets were evaluated.
  The selected row is song time `37.790`, frame `467`, yaw `+0.45` radians:
  projected arm ratio `0.871238`, projected bend `84.5437` degrees,
  guitar-axis clearance `6.81263`, and exact wrist/strum-target error `0`.
  The rank evidence is
  `gameplay_palm_contact_trial/strum_visibility_rank.json`.
- Rehashed the staged files before capture. Model remained
  `13099CDE50E0DC5637CEF9A13F41DFD4AE1B0C0269FBE0CDA62920469A06F7A5`,
  main remained
  `0EC2FAEA6F2C0524E8DDF40EB7E07DE76CE00044ABE9B5DD915D1016212BCE0D`,
  fret remained
  ...
  `2BD1105BB5A9666BC7B259E4B8C936B225551253288F755E8A6961C0D2FCF52D`.
- The turn's one image is
  `gameplay_palm_contact_trial/midori_outfit1_shoutatthedevil_strumproof_f467_yaw045_v2.bmp`,
  SHA-256
  `A72773DDA4481FC39500C122804AD8686163E2846E2226D49951CF0A553E85CC`.
  It is a real gameplay frame at song time about `37.783`, under
  `stand_medium_02`, `strum_long_02`, and `finger_vibrato_pinky`. Both source
  hand weights are `1.0`; the bounded run ended playing at score `1007`, 11
  hits, and zero misses. The guitar's scale and across-body placement pass for
  this pose, and the strum forearm/hand are visible reaching the guitar body.
- Do not call this full visual acceptance. The fret-side arm crosses the
  face/head awkwardly, and the occlusion means this frame cannot independently
  approve head/neck posture. Preserve the guitar placement and corrected hand
  bank hashes; the next data-first pass should select or correct a pose with a
  clean fret-side silhouette rather than recalibrating the guitar root. Full
  report: `gameplay_palm_contact_trial/strum_visibility_validation.json`.
- Verification is `175/175` Python tests, all `milo_convert_test` checks, and
  three existing character contract executables. Both engine runs used only
  extracted `gh2_ps2_hybrid_assets/GEN` plus loose DLC, never an ISO, at Idle
  priority with one-thread library limits. Keep the active goal open.

### 2026-08-30 bilateral data-ranked gameplay proof candidate

- Extended the diagnostic pose rows with `bone_neck` and `bone_head`, and
  upgraded `tools/gh3_midori_rank_runtime_strum_visibility.py` to v2 bilateral
  scoring. It now evaluates both complete arm chains, both exact hand-target
  errors, projected guitar-neck length, arm-to-head and arm-to-neck clearance,
  wrist separation, and projected elbow bends. This remains diagnostic-only;
  no model, animation, IK, guitar, or hand-bank data changed.
- A longer no-image `Shout at the Devil` sweep logged 247 complete poses from
  30.0 through 56.18 seconds and evaluated seven yaw offsets. The selected row
  is song time `30.600`, frame `36`, yaw `-0.45` radians under
  `stand_medium_01`, `strum_short_03`, and `finger_vibrato_middle`. Both arms
  retain at least `0.871296` of their 3D length in projection, the guitar axis
  retains `0.999248`, the nearest arm clears the head by `2.35619` projected
  head lengths, and both wrist/target errors are exactly zero.
- This directly fixes the previous proof's selection weakness: the old
  frame/yaw had minimum bilateral projection `0.526630`, guitar projection
  `0.783692`, and head clearance `0.839118` head lengths. The new frame scores
  `15.025911` versus `8.647357` under the same bilateral metric.
- The turn's one image is
  `gameplay_palm_contact_trial/midori_outfit1_shoutatthedevil_bilateral_f36_yawm045_v1.bmp`,
  SHA-256
  `5E82A7BF853868DB2512E9E40F6A09677D98D73032D95FDDF67EF7E87EEF3E55`.
  Self-review marks it as the first full-body visual proof candidate: head and
  neck are unobstructed; both arm chains read continuously; the fret hand is
  on the neck; the strum hand reaches the guitar body; guitar scale/placement,
  torso/shoulders, hips, knees, and ankles are coherent. User visual acceptance
  remains pending, so do not close the goal from self-review alone.
- Runtime proof is actual `Shout at the Devil`: 45/45 model meshes, two
  `CharIKHand` controllers, 31 hand clips, seven maps, and `flyingv_v2`
  attached to `bone_pos_guitar.mesh`. Both source hand weights remained `1.0`.
  The bounded capture ended playing at score `100`, two hits, and zero misses.
- Rehashing before capture confirmed unchanged model/main/fret/strum hashes:
  `13099CDE50E0DC5637CEF9A13F41DFD4AE1B0C0269FBE0CDA62920469A06F7A5`,
  `0EC2FAEA6F2C0524E8DDF40EB7E07DE76CE00044ABE9B5DD915D1016212BCE0D`,
  `9D81CCC2429A1D0DB4DD69148651106ADED6C4047C5CF80BB8219003585E181F`,
  and `2BD1105BB5A9666BC7B259E4B8C936B225551253288F755E8A6961C0D2FCF52D`.
  Full report:
  `gameplay_palm_contact_trial/bilateral_visibility_validation.json`.
- Verification is `176/176` Python tests, all `milo_convert_test` checks, and
  three character contract executables. Both runs used extracted `GEN` plus
  loose DLC, no ISO, Idle priority, and one-thread library limits. Keep the
  active goal open pending user review and the full completion audit.

### 2026-08-30 exact published-package native gameplay proof

- Added `tools/gh3_midori_published_gameplay_proof.py`. It validates the
  ordinary outfit1 package and publication report, runs a no-image native
  preflight, fails closed on rig/controller/animation/guitar-target checks,
  and then captures one real-song frame from the same package. Both the Python
  wrapper and `ghogx_app.exe` run at Idle priority with one-thread library
  limits. Runtime uses only extracted `gh2_ps2_hybrid_assets/GEN` plus
  `GuitarHeroOGX-main-ui-engine/DLC`; no ISO is used.
- The preflight and capture directly loaded the complete-hand model
  `DBC150E44C773608EAFC8DD7FBEB4FD2E63EEF11C77DA8B56ED6E4EF5B8A20E9`:
  45/45 meshes, 71 bones, two `CharIKHand` controllers, one `CharIKMidi`, 31
  hand clips, and seven hand maps. This supersedes reliance on inherited
  screenshots from the 67-bone visual base.
- At `Shout at the Devil` song time `30.600`, body clip
  `stand_medium_01`, strum clip `strum_short_03`, and fret clip
  `finger_vibrato_middle` are active. The Flying V is attached to
  `bone_pos_guitar.mesh`; left wrist to fret target and right wrist to strum
  target errors are both exactly `0.000000`. Guitar-axis projection is
  `0.999212`, minimum bilateral arm projection is `0.871173`, and arm-to-head
  clearance is `2.356188` head lengths. The bounded run exits in
  `state=playing` at song time `30.633`.
- The one current image is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/gameplay_palm_contact_trial/published_gameplay_proof/midori_outfit1_shoutatthedevil_published_f36_yawm045_v1.bmp`,
  SHA-256
  `236F65E6C0E391A479E772CA80C988DF6AD29E35E451734EC71891D0F70D59D1`.
  Self-review promotes it only as a candidate: guitar scale/diagonal placement
  is coherent, the fret hand is on the neck, the strum hand reaches the guitar
  body, and the visible body remains bipedal. User visual acceptance remains
  pending.
- Proof report:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/gameplay_palm_contact_trial/published_gameplay_proof/published_gameplay_validation.json`.
  Updated `gh3_midori_publish_outfit1.py` and
  `gh3_midori_source_bridge_completion_audit.py` to make this direct
  published-package run the visual authority. Publisher verify-only passes;
  completion audit is `13 passed / 0 failed / 1 pending` with status
  `conversion_bridge_verified_visual_acceptance_pending`.
- Verification is `180/180` Python tests. Keep the active goal open until the
  user explicitly accepts the visual or asks for another in-song pose.

### 2026-08-30 head-neutral published gameplay candidate

- Upgraded `tools/gh3_midori_rank_runtime_strum_visibility.py` to v3. It now
  parses local head/neck rotation matrices from native post-controller rows,
  reports their rotation angles, and includes head/neck neutrality in pose
  ranking. Added SHA-256 binding for the no-image sweep log. Added matching
  parser/ranker coverage; full Python verification is now `181/181`.
- Ran one no-image exact-package autoplay sweep from song time `30.0` through
  about `60.0`, logging 296 complete poses at Idle priority and one-thread
  limits. Evidence:
  `published_gameplay_proof/head_neutral_sweep.log`, SHA-256
  `35529A73DDA7A93DD3F1F467497D6940FFF04CA9CBCD8BDD0337739DC189774C`,
  and `published_gameplay_proof/head_neutral_rank.json`, SHA-256
  `B3DCBD381960C553DAD2F6183D3DEC1D77BCE5201FBE210FCECC043464943CEA`.
- The first distinct alternative at song time `34.100` was rejected before
  image generation because minimum bilateral arm projection was `0.83`, below
  the retained `0.85` gate. The next candidate at `33.200`, frame `192`, yaw
  `-0.45` passes: head local rotation `0.000` degrees, neck local rotation
  `16.184` degrees, guitar projection `0.999238`, bilateral arm projection
  `0.856330`, arm-to-head clearance `2.335352` head lengths, and both wrist
  target errors exactly `0.000000`. Active hand layers are `strum_short_03`
  and `finger_powerchord_2`.
- Upgraded `tools/gh3_midori_published_gameplay_proof.py` to consume a
  hash-verified v3 no-image rank report as preflight, use dynamic frame/yaw
  evidence names, enable autoplay explicitly, and log capture poses every
  `0.1` seconds. The exact package run ended `state=playing`, score `396`, six
  hits, and zero misses.
- Current one-image candidate:
  `published_gameplay_proof/midori_outfit1_shoutatthedevil_published_f192_yawm045_v2.bmp`,
  SHA-256
  `2E9F33CDDC4E451ED2D09AF2E6C9D6830F31293385F4575BC4DADACA74359604`.
  It supersedes frame 36 as the current user-review image. Self-review: upright
  head/neck, continuous arms, fret hand visibly on the neck, strum hand at the
  guitar body, coherent across-hips guitar placement, and coherent visible
  torso/legs. User acceptance is still pending.
- Publisher verify-only passes with the new proof. Completion audit remains
  `13 passed / 0 failed / 1 pending` with only explicit user visual acceptance
  pending. No ISO was used and no conversion MILO, runtime pose policy, or
  guitar calibration changed. Keep the goal active.

### 2026-08-30 thirty-second full-biped native sweep

- Extended diagnostic-only gameplay pose rows to include pelvis, spine1-3,
  both clavicles, both complete leg chains through the toes, and the existing
  arm/head/guitar chains. Added active main-body clip attribution to
  `gh3_midori_rank_runtime_strum_visibility.py`. No converted model, animation,
  controller, guitar, or pose behavior changed.
- Rebuilt `ghogx_app.exe` at Idle priority with one Ninja job. The first build
  invocation exposed the repository's stale hardcoded `build_env.bat` working
  directory and made no changes; the successful build used the absolute
  configured build directory. Current executable SHA-256 is
  `899BD763FB745A3CE179D5D76E228725C9AD36B6D0A0C8D2020B20B2A6313D34`.
- Added `tools/gh3_midori_validate_published_biped_sweep.py` and ran a no-image
  `Shout at the Devil` autoplay sweep from song time `30.0` through about
  `60.0`, from the exact ordinary outfit1 package and extracted `GEN`. It
  records `301` complete full-biped rows, `296` steady rows after the explicit
  `0.5s` startup blend, and all seven observed body clips:
  `stand_medium_01-04` and `stand_slow_01-03`.
- The validator treats controller-driven forearm-to-hand reach separately from
  rigid downstream chains and allows bounded interpolation of direct pelvis
  children. Across the steady window, all downstream segment-length relative
  ranges are below `0.000006`; direct pelvis children remain above `94%` of
  median length; IK reach remains below `1.51x` median; full body span never
  falls below `61.119588`; left/right hips never merge; and both wrist-target
  errors are exactly `0.000000` in every pose.
- Sweep report:
  `published_gameplay_proof/full_biped_sweep_validation.json`, status
  `full_biped_runtime_sweep_verified`. Sweep log SHA-256 is
  `E3C492EA25B3441EF69913442544A9FC05482046FAE4F4AFC2B72A928C5775D8`.
- Refreshed the current frame-192 gameplay proof from the rebuilt executable.
  The screenshot remains byte-identical at
  `2E9F33CDDC4E451ED2D09AF2E6C9D6830F31293385F4575BC4DADACA74359604`,
  and the visual report's app hash now matches the executable on disk.
- Made the sweep a required publisher and completion-audit input. Publisher
  verify-only passes. The audit is now `15 requirements / 14 passed / 0 failed
  / 1 pending`; only explicit user visual acceptance remains pending. Python
  tests are `182/182`, and focused native
  `ghogx_gameplay_venue_band_contract_test` passes. No ISO was used. Keep the
  goal active.

### 2026-08-30 retail GH2 PS2 in-song guitar proof

- The conversion-level guitar contract was compared directly against retail
  Casey. `bone_pos_guitar`, `bone_fret`, `bone_strum`, `bone_fret_hand`, and
  `bone_strum_hand` have identical local matrices in the Midori and Casey
  models. Both packages also have the same `CharIKMidi fret.ik`, left
  `CharIKHand` (`bone_L-hand.mesh` to `bone_fret_hand.mesh`), and right
  `CharIKHand` (`bone_R-hand.mesh` to `bone_strum_hand.mesh`) contract, with
  orientation and stretch enabled. No model, animation, controller, or guitar
  matrix was changed for this proof.
- Added `tools/gh3_midori_ps2_autoboot_splash.dta` and
  `tools/gh3_midori_ps2_runtime_proof.py`. The runner extracts the untouched
  retail GH2 PS2 image without mounting it, maps the ordinary outfit1 package
  onto Casey's five stock paths, compiles/encrypts the diagnostic attract-mode
  splash, creates and verifies a split patch ARK, and rebuilds an ephemeral
  `ISO9660 + UDF 1.02` image with ImgBurn. It then creates an isolated PCSX2
  runtime, copies an existing GH2 save card to bypass the first-run modal,
  boots `SLUS-21447`/CRC `2A6C845B`, and dumps one native GS frame. There is no
  synthesized controller input or emulated-memory mutation.
- The runner and all child processes use Idle priority, one-thread library
  limits, disabled MTVU, and zero software-renderer extra threads. Its
  extracted disc, patch stage, rebuilt image, isolated emulator, captures, and
  logs live under system temp and are deleted automatically. Existing proof
  output is protected unless `--overwrite` is explicitly supplied.
- The retained close camera is real `Shout at the Devil` attract-mode gameplay:
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_shoutatthedevil_ps2_f3900.png`,
  512x448, SHA-256
  `E529DB860E87EBDB82DAF1C6CAC151A14E3C38FB2266DEE5F00561F4DBD0718E`.
  Midori is visibly animated with the Flying V across her body, the fret hand
  on the neck, the strum hand at the body, and coherent shoulders, elbows,
  wrists, and stance. `Press Any Button To Rock` is GH2's attract-mode exit
  prompt; the performance is already running behind it. Venue camera timing is
  load-phase dependent, so only this calibrated close frame is retained.
- Hash-bound report:
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_shoutatthedevil_ps2_f3900_validation.json`.
  `gh3_midori_ps2_runtime_proof.py --verify-only` passes. Full focused Python
  verification is `186/186`. The completion audit is now `16 requirements / 15
  passed / 0 failed / 1 pending`; only explicit user visual acceptance remains.
  Keep the active goal open.

### 2026-08-30 deterministic PS2 close-camera proof

- Added `inspect-camshots` to `milo_convert_tool` and parsed the untouched
  `world/small2/gen/small2.milo_ps2`. Retail `flr_near_rt01` is an authored
  `flr_near_rt` CamShot with two keyframes, fixed `0.817276` FOV, and target
  `guitarist0:bone_spine1.mesh`. The native `milo_convert_test` passes.
- Extended `tools/gh3_midori_ps2_runtime_proof.py` to extract retail
  `ui/gen/game.dtb`, decompile it, and insert a diagnostic-only `game_screen`
  poll that resolves `flr_near_rt01` through `find_obj world` and reasserts it
  once per second during attract playback. The patched DTB is compiled,
  encrypted, and staged with the Casey-path overlay. This changes no converted
  model, animation bank, controller, hand target, or guitar matrix.
- The first forced-camera attempt passed a symbol where `do_force_shot`
  requires a CamShot object and faulted before capture. It was rejected and
  cleaned. The guarded object-resolution version boots cleanly with GH2 CRC
  `2A6C845B` and captures exactly one native GS frame.
- Current PS2 proof:
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_shoutatthedevil_ps2_forced_rt01_f3900.png`,
  512x448, SHA-256
  `61DA13D03AB4EA723DD04FCBC0A8B70B809EA3FDC5715D64A457297868E54FAF`.
  It shows a coherent full playing stance, the fret hand wrapped around the
  neck, and the strum hand at the guitar body. The earlier non-forced frame is
  historical and no longer the canonical review image.
- Hash-bound report:
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_shoutatthedevil_ps2_forced_rt01_f3900_validation.json`.
  Proof verify-only passes, Python tests pass `188/188`, and the completion
  audit remains `16 requirements / 15 passed / 0 failed / 1 pending`; only
  explicit user visual acceptance remains. Keep the active goal open.

### 2026-08-30 retail animation-call coverage and final-package PS2 refresh

- Parsed all four untouched retail Casey animation banks directly from the
  provenance-bound GH2 `MAIN.HDR` and `MAIN_0.ARK`. Casey exposes 157 exact
  clip calls and 30 main-bank `CharClipGroup` names. The converted Midori
  package initially covered only 44 exact clip names and 7 groups, so ordinary
  GH2 UI, walk, extreme, result, bass-hand, and chord calls were incomplete.
- Added `tools/gh3_midori_animation_call_compatibility.py` and
  `tools/gh3_midori_apply_call_surface_fallbacks.py`, plus the structured
  `merge-clipset-fallbacks` command in `milo_convert_tool`. Every existing
  converted Midori clip and group remains byte-identical. Missing main, fret,
  and strum calls use byte-identical retail Casey fallback records. GH2 UI
  calls are converted-Midori aliases:
  `gh3_guit_midori_tran_outtoa -> ui_enter` and
  `gh3_guit_midori_a_intro2 -> ui_loop`, avoiding Casey-only face and door
  controls while retaining the static-face policy.
- Final bank inventories are main/UI/strum/fret `381/8/29/40`, 458 clips
  total. All 157 stock clip names and all 30 stock groups resolve, with zero
  duplicate names, missing calls, missing groups, or dangling group members.
  Reports:
  `analysis/gh3_midori_call_surface_fallback_deploy.json` and
  `analysis/gh3_midori_animation_call_compatibility.json`. Equivalent hybrid
  reports also pass verify-only.
- Native catalog validation exposed `bone_facing` as the only apparent missing
  main-fallback target. Untouched Casey also has no `bone_facing` model
  transform: GH2 carries it as virtual `CharBones` root-motion data consumed
  by `ClipPredict`. The catalog validator now recognizes that source behavior;
  no model transform or clip channel was fabricated or stripped. Native
  Guitar Hero Classic loads all 458 clips against the outfit-1 model with 14
  static face no-ops and no loader failures.
- Rebuilt the retail GH2 PS2 proof from the final fallback-augmented package.
  The source disc was extracted and never mounted; PCSX2 and all helper work
  remained Idle priority and one-worker. The forced retail CamShot remains
  `flr_near_rt01`. Current hash-bound output:
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_shoutatthedevil_ps2_f3900.png`,
  SHA-256
  `8201B0B5A6E1BAF39D2BA95B547A9E640C193001EB475F1208BB9B8D6EA41C6B`,
  with adjacent `_validation.json`. Exactly one new image was captured.
- Verification passes: native `milo_convert_test`, native full catalog, runtime
  deformation smoke, engine and hybrid fallback/compatibility verify-only,
  PS2 proof verify-only, and Python `193/193`. Completion audit is now
  `17 requirements / 16 passed / 0 failed / 1 pending`; only explicit user
  visual acceptance remains pending. Keep the goal active.
- Original-resolution inspection of the final PS2 frame finds a coherent
  head/neck gaze toward the fretboard, continuous shoulder-elbow-wrist chains,
  fret-hand neck contact, strum-hand body contact, and attached across-pelvis
  guitar placement. The completion audit now makes this final PS2 frame its
  primary pending visual artifact and retains the older GHC frame only for
  quantitative arm/contact metrics. Python regression coverage remains
  `193/193`; no new image was generated for this review.

### 2026-08-30 unobstructed final PS2 visual proof

- Retail `ui/gen/game.dtb` proves the attract overlay is controlled by one
  `fade_panel` expression:
  `press_any_button.lbl set_showing {game_screen get attract_mode}`. The
  diagnostic proof patch now replaces only that expression with `FALSE`, in
  addition to its existing scoped `flr_near_rt01` camera poll. Conversion
  model and animation assets remain byte-identical.
- Captured exactly one new final-package PS2 frame at Idle priority with one
  worker. The source ISO was extracted and never mounted, no controller input
  was synthesized, the rebuilt filesystem remained `ISO9660 + UDF 1.02`, and
  the isolated PCSX2 process tree exited cleanly after capture.
- New canonical prompt-free image:
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_shoutatthedevil_ps2_promptfree_f3900.png`,
  512x448, SHA-256
  `2773679446735D433C1E9B71F2B36879AC008F36F61C64ECE09D11C04FC6B045`.
  Original-resolution inspection shows the complete unobstructed biped, both
  planted feet, coherent hips/knees, natural head/neck direction, continuous
  arm chains, both hands contacting the instrument, and attached guitar
  placement across the pelvis.
- The completion audit and PS2 verify-only now bind this prompt-free frame and
  require `attract_prompt_hidden=true`. Python tests remain `193/193`; audit is
  `17 requirements / 16 passed / 0 failed / 1 pending`, with only explicit
  user visual acceptance pending. Keep the goal active.
- Added one complementary authored retail view, `flr_near_lft01`, identified
  headlessly in untouched `small2.milo_ps2` as a two-keyframe CamShot targeting
  `guitarist0:bone_spine2.mesh`. Prompt-free frame:
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_shoutatthedevil_ps2_promptfree_lft01_f3900.png`,
  512x448, SHA-256
  `F12C8182E03FCE14AC02461975562680B28FA49491EB36C1426855CD9AE5BBD9`.
  It independently shows separated shoulder-elbow-wrist chains, fret fingers
  around the neck, strum hand over the strings/body, both planted feet, and
  attached guitar placement. The completion audit now requires both authored
  PS2 camera proofs in the same native-gameplay requirement; both verify-only
  checks and Python `193/193` pass. User visual acceptance remains pending.

### 2026-08-30 final cross-view completion audit

- Added the immutable direct-inspection record
  `analysis/gh3_midori_ps2_runtime/published/midori_outfit1_ps2_visual_review.json`.
  It binds both prompt-free PS2 screenshots and their runtime reports by
  SHA-256, records every full-biped, arm, hand-contact, guitar, and lower-body
  visual check per camera, and separately requires cross-view consistency.
- The review explicitly states that numeric validation is supporting evidence,
  not a substitute for original-resolution inspection. Both authored views
  pass independently, including complete unobstructed biped silhouettes,
  coherent head/neck and torso, continuous shoulder-elbow-wrist chains, fret
  and strum contacts, attached across-pelvis guitar placement, coherent
  hips/knees/ankles/feet, and planted feet.
- Updated the completion audit to require both PS2 proofs plus this direct
  visual review. The stale future-emulator note is replaced by the completed
  rebuilt-retail-PS2 validation record. Final audit status is `complete`:
  `17 requirements / 17 passed / 0 failed / 0 pending`. Full Python regression
  remains `193/193`; both PS2 reports verify-only, no ISO was mounted at game
  time, and no emulator or temporary proof scratch remains. The objective is
  complete.

### 2026-08-30 Casey-native correction after rejected identity proof

- The preceding visual-completion claims are stale and must not be used as
  acceptance evidence. The user correctly identified the latest alleged Casey
  frame as `glam1`, and rejected the torso/shoulder/ankle and subsequent visual
  proofs. The active goal remained open. Only the hash-gated result below is
  current.
- The production architecture is now explicit: keep Casey's native `rock1`
  transform/controller graph and clip catalog, rebind Midori outfit 1's 45
  active render meshes to that rig, encode Midori body motion into each stock
  Casey channel schema, preserve Casey scalar hinge channels, keep face motion
  static, and solve only the pelvis-local guitar root needed by the native
  hand controllers.
- `tools/gh3_midori_guitar_root_contact_retarget.py` now accepts a
  controller-disabled runtime replay through `--native-body-pose`. Every frame
  takes pelvis, shoulders, elbows, hands, upper-arm bases, and fixed segment
  lengths from the serialized Casey-schema runtime body. When this native
  replay is supplied, unencoded axial-gauge adjustments are forbidden.
  `strum_short_01` changes Casey's right clavicle/upper arm/forearm, so the
  authoritative replay also includes fixed stock `finger_vibrato_index` and
  `strum_short_01` helper layers.
- The resulting guitar solve is
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/calibration/gh3_guit_mido_a_med_break02.guitar_root_contact_retarget.layered_native_body.json`.
  It passes all 231 frames with zero dynamic gauges, maximum reach ratios
  `0.8618336` left / `0.9500004` right, minimum elbow-pole alignment `0.25`
  on both sides, and maximum constraint violation `9.15e-6`.
- The one-clip native controller validator passes at
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/runtime_replay/stand_medium_02.layered_native_body_contact.native_guitar_validation.json`.
  Both hands reach their guitar targets exactly, native arm stretch remains
  `1.0` within floating-point error, elbow predictions agree within
  `5.7e-5`, and native upper/forearm twist helpers are correctly classified as
  part of the arm controller chain.
- The independent Blender deformation replay passes every one of 231 frames,
  all 45 active Midori meshes, and the complete biped at
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/runtime_replay/stand_medium_02.layered_native_body_contact.validation.json`.
  Maximum relative segment drift is `2.89418e-6`; all body, motion, finite,
  vertex, and bounds gates pass.
- The same authenticated bridge was encoded into ten Casey gameplay calls:
  `stand_fast_01` through `stand_fast_05` as selected by the stock call set and
  `stand_medium_02` through `stand_medium_06` as selected by the stock call set.
  Final main bank:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/rock1_main.casey_native_midori_final_candidate.milo_ps2`,
  SHA-256
  `5B8202F496504A9F0FA88163C0042109F9DC2349FF88FD8B82A298E5FA3ED21B`.
  `tools/gh3_midori_casey_native_candidate_validate.py` authenticates all ten
  encoder reports, 103 unchanged Casey clips, exact native schemas/flags/events,
  all serialized values, and quaternion normalization. Validation passes at
  the adjacent `.validation.json` report.
- `tools/gh3_midori_ps2_runtime_proof.py` now always uses
  `--rebind-template-rig` for Midori production runs. Its preflight requires
  Casey's exact 115 transform rows, all 20 Casey controllers, maximum bind
  residual below `1e-3`, and an exclusive active `rock1.grp` containing exactly
  45 `midori_1_*` meshes with no non-Midori or `glam1` render object.
- Isolated package:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/ps2_package_casey_native_midori`.
  It contains the final main bank and untouched stock Casey UI/fret/strum
  banks. The retail harness extracted the source ISO without mounting it,
  rebuilt a temporary ISO, booted only that temporary image, ran every process
  at Idle priority with one worker, and removed the temporary runtime.
- Current in-engine proof:
  `analysis/gh3_midori_ps2_runtime/published/midori_casey_native_gameplay_f3900.png`,
  512x448, SHA-256
  `DC77903F6CC70FC2658538CB0DD3FAF5AB5345255CD7C7703A5CEC6DB291B82B`.
  Hash-bound report:
  `analysis/gh3_midori_ps2_runtime/published/midori_casey_native_gameplay_f3900_validation.json`.
  It records `rock1`, Midori outfit 1, retail Casey bind rig, 115 transforms,
  20 controllers, 45 active Midori meshes, zero active non-Midori objects, and
  the exact final-main hash above. The frame is visibly Midori in gameplay,
  not Casey or `glam1`. Its camera/hair occlude part of the near arm and guitar,
  so it is identity/runtime proof rather than a clean final arm-review angle.
  Keep the goal active pending user visual judgment; do not restore any stale
  completion or blocking instruction.

### 2026-08-30 clone-runtime correction and render-payload visibility fix

- Do not build an ISO or launch PCSX2 during the active Midori iteration loop.
  The repository's `ghogx_app` clone is the sole diagnostic runtime until the
  converted model and animations have been accepted there. Retail packaging
  is deferred to a final compatibility gate only. Continue using the already
  loose `gh2_ps2_hybrid_assets/GEN/MAIN.HDR` and `MAIN_0.ARK`; do not mount a
  source ISO. Keep all converter and clone processes at Idle priority with one
  worker.
- The rejected retail frame was `glam1`, not Midori. Untouched
  `config/gen/songs.dtb` assigns `shoutatthedevil` quickplay outfit `glam1`, so
  attract-mode playback ignored the UI `rock1` selection. No retail image from
  that path is current acceptance evidence.
- Direct clone proof selected diagnostic variant `midori_casey_runtime`, loaded
  performer `guitarist0` from `char/rock1/og/gen/rock1.milo_ps2`, locked the
  front camera to that role, and reached playing state. It showed the correct
  Midori identity but also large debris because all 124 retained Casey template
  meshes still had serialized `showing=true`; the clone renderer correctly
  drew those in addition to Midori's 45 meshes.
- `merge-character-render-payload` now preserves Casey's mesh objects for the
  native controller/reference graph while serializing every template mesh with
  `showing=false`. Donor Midori meshes remain visible. The converter's
  `inspect-character --meshes` output now includes the serialized visibility
  bit.
- Fresh artifact:
  `analysis/_midori_patch_identity_audit/casey_rebind_visible45/rock1.casey_rebind.visible45.milo_ps2`,
  SHA-256
  `49B2EE79F93DE3AC58BC7C8BECE5BD1FBA40A9461801DA9F99CF0B4630898EA0`.
  Independent inspection proves 169 total meshes, 45 visible Midori meshes,
  zero hidden Midori meshes, 124 hidden Casey template meshes, zero visible
  Casey template meshes, 115 Casey transforms, pelvis parent `rock1`, and the
  Casey controller markers intact. The merge preserved all 20 controller
  entries and rebased 172 bind slots with maximum residual `7.62939e-06`.
- `tools/gh3_midori_ghc_casey_slot_proof.py` now defaults to that artifact and
  fails closed unless both serialized inspection and the clone runtime report
  exactly 45 visible Midori meshes and zero visible Casey template meshes.
  The next visual proof must be generated by `ghogx_app`, not PCSX2. Keep the
  goal open; do not block unless the user explicitly asks.

### 2026-08-30 clone-only Casey bind-pose hand rebind

- The active iteration loop remains clone-only. No ISO was mounted, read,
  rebuilt, or launched in this pass, and PCSX2 was not used. Every converter,
  test, and `ghogx_app` process ran at Windows Idle priority with one worker.
  The retail compatibility gate remains deferred until clone acceptance.
- Exact staged-vertex comparison proved the existing ancestor-fallback donor
  and packaged donor had identical 4,544 `MeshSkinVertex` rows. Runtime
  instrumentation then isolated the defect: adjacent Midori finger vertices
  were rotating around incompatible Casey pivots, producing 15-20 unit
  triangle stretch. This was a bind-pose/skin-rebind problem, not visibility,
  character identity, or a pelvis/root offset.
- `tools/gh3_midori_model_stage.py` now has an opt-in
  `--palette-overflow-policy lowest-weight-ancestor`. It preserves fully
  weighted distal finger vertices and collapses only the smallest legal
  ancestor-compatible influences. The fresh stage retains all 4,992 faces,
  drops no weight, emits 44 chunks, and has zero source-vertex weight conflicts.
- `milo_rig_export.exe` exported the actual Casey template rig to
  `analysis/gh2_casey_rock1_rig.json`, SHA-256
  `9B8DC56BEA0B4EBB7BEAF540811546E78C69D35E9C63BA411B4DFEF44C5853DC`.
  Direct comparison found Glam1-versus-Casey local-matrix deltas up to `1.69`
  in the hand chain, confirming the old Glam1 rig input was not a valid Casey
  substitute.
- Merely substituting the Casey rig was rejected: it closed cross-chunk seams
  but restored up to `19.649` units of finger stretch. A full upper-limb
  bind-pose warp was also rejected headlessly because six rest-pose chunks tore
  at the shoulder/torso boundary, up to `17.563` units.
- `tools/gh3_midori_model_bundle.py` now provides
  `--bind-pose-vertex-warp-scope hands`. It applies the standard row-vector
  bind transfer `inverse(MidoriBind) * CaseyBind` through each vertex's existing
  skin weights, rotates and renormalizes normals, and recomputes changed chunk
  spheres. The hand-only scope changes 911 of 4,523 staged vertex copies across
  28 hand/finger bones while leaving forearms, torso, and lower body untouched.
- Current passed model:
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/model_casey_lowweight_caseyrig_handwarp/rock1.casey_lowweight_caseyrig_handwarp.milo_ps2`,
  SHA-256
  `33E6CA3EAC0B1AC7EBFD2E56B34E5C7196CEDE2D2B3DCA4256A3317AB6637AC1`.
  It preserves all 20 Casey controllers and all 57 template transforms, hides
  124 Casey render meshes, and enables 44 Midori meshes.
- A 64-frame `ghogx_app` gameplay pass against loose `GEN` assets selected
  `midori_casey_runtime`, loaded the `rock1` model and stock Casey controller
  banks, entered playing state, and scored two autoplay hits with no misses.
  Only the known cross-thigh garment span exceeds 10 units (`12.083`); the
  largest hand/finger edge is `9.243`. Across 1,004 duplicate-position pairs,
  maximum seam separation is `0.077126`, with zero pairs above `0.25` or `1.0`.
- The final diverse ten-clip bank is
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_native_animation_set_v1/rock1_main.midori_diverse_final_candidate.milo_ps2`,
  SHA-256
  `F7B202330E9233378BA55C896845DEEF1923C885174B7C0CF49097C115C17002`.
  `tools/gh3_midori_ghc_casey_animation_sweep.py` now executes all ten
  authenticated replacement calls directly in `ghogx_app` with no screenshot
  or gameplay-world startup. Validation passes at
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/ten_clip_handwarp_sweep/validation.json`,
  SHA-256
  `91E253CDF98D8CE91F792EE6033858D18E17DF8FFC30A73C2C2BFF72567296BB`.
  All ten clips load from the requested `rock1_main` bank, render 44 meshes,
  keep maximum non-garment edge at or below `9.158`, and keep maximum seam at
  or below `0.095145`, with zero seam pairs above `0.25`.
- Focused and full Python regressions pass: `200/200`. `py_compile` and nested
  repository `git diff --check` also pass. No new image was generated after the
  turn's existing image allocation. On the next turn, capture exactly one
  `ghogx_app` gameplay frame from this model/bank pair for user visual review.
  Keep the goal active and do not block it unless explicitly requested.

### 2026-08-30 clone-only gameplay-frame rejection and pose gate

- The user reiterated that active iteration belongs in the Guitar Hero Classic
  clone repository. No PCSX2 or related process was running, and this pass did
  not read, mount, build, or launch an ISO. The only runtime was the repository
  `ghogx_app` against loose `gh2_ps2_hybrid_assets/GEN` plus the diagnostic
  Casey-slot addon, at Windows Idle priority with one worker.
- Rehashed the exact loose inputs before launch. The model remained
  `33E6CA3EAC0B1AC7EBFD2E56B34E5C7196CEDE2D2B3DCA4256A3317AB6637AC1`
  and the ten-clip main bank remained
  `F7B202330E9233378BA55C896845DEEF1923C885174B7C0CF49097C115C17002`.
  The single 1280x720 clone gameplay frame is
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_handwarp_stand_medium_02_gameplay_20260830.bmp`,
  SHA-256
  `D95970CCE2F75D1A764595C7179E6BD384669EE07AD7CACC7C286714C110A20D`.
- Reject that frame. It is Midori and the body scale is plausible, but the
  guitar is nearly vertical through the upper body/face and the selected hand
  pose is not acceptable. Do not present it as a valid gameplay proof. The
  wrapper recorded Idle launch and a clean app exit, but did not inherit the
  app diagnostic stream; do not rerun or generate a second image in the same
  turn merely to repair that bookkeeping.
- Added `tools/gh3_midori_rank_casey_native_clip_poses.py`. It authenticates
  all ten encoder, retarget, and guitar-root reports, joins every serialized
  Casey-schema body sample to its solved guitar/contact sample, derives local
  head and neck rotations as `parent^-1 * child`, and ranks camera yaws using
  bilateral arm projection, elbow bends, head clearance, guitar projection,
  fret height, body-relative strum placement, and exact hand-target contact.
  This is a conversion-evidence gate and changes no engine behavior or MILO.
- The rejected capture used
  `GHOGX_DIAGNOSTIC_FRONT_CAMERA_YAW_OFFSET=25`. Source inspection proves that
  environment value is raw radians, not degrees, so it wrapped to about
  `-7.606` degrees. The exact regression probe rejects the frame for a
  `74.944` degree projected guitar elevation and a fret target above the head.
  Never pass degree values to that environment variable again.
- Rank report:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/ten_clip_handwarp_sweep/proof_pose_rank.json`,
  SHA-256
  `0258F5324E3D899E2A1E74D1EAC863772297ADA83627D12C6B5B3A586E69D5BB`.
  It evaluates 17,479 clip/sample/yaw combinations and finds 7,644 passing
  proof candidates. The preferred next single-image candidate is
  `stand_medium_04` sample 143 (`4.766666667s`) at yaw `-0.3` radians:
  guitar elevation `41.253` degrees, guitar projection `0.992956`, minimum
  bilateral arm projection `0.881430`, head/neck local rotations
  `10.556/9.666` degrees, head clearance `1.926792` head lengths, both hand
  target errors zero, and zero guitar orientation correction.
- Focused pose-ranker tests pass and the full Python suite passes `202/202`.
  Keep the active goal open. On the next image allocation, run the preferred
  candidate in real clone gameplay with `-0.3` radians exactly; reject it if
  the actual song hand layers or visual silhouette disagree with the data.

### 2026-08-30 valid clone gameplay proof candidate

- Reauthenticated the loose clone inputs before runtime: hand-warp model
  `33E6CA3EAC0B1AC7EBFD2E56B34E5C7196CEDE2D2B3DCA4256A3317AB6637AC1`,
  diverse ten-clip main bank
  `F7B202330E9233378BA55C896845DEEF1923C885174B7C0CF49097C115C17002`,
  and pose-rank report
  `0258F5324E3D899E2A1E74D1EAC863772297ADA83627D12C6B5B3A586E69D5BB`.
- A no-image `ghogx_app` preflight forced `stand_medium_04` sample 143
  (`4.766666667s`) at camera yaw `-0.3` radians. It loaded
  `midori_casey_runtime` from `rock1`, 44 visible meshes, 115 Casey bones, two
  `CharIKHand` controllers, one `CharIKMidi`, and the Flying V attached to
  `bone_pos_guitar.mesh`. At song time `30.8`, live hand layers were
  `strum_short_03` plus `finger_open`; both wrist-target errors were zero.
  The run ended playing with two hits and zero misses.
- Preflight log:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/preflight_medium04_s143_yawm030.log`,
  SHA-256
  `0D4FD4D4D66F59B10859E1434994528ADBAD69C0F399400385310A4703E9541C`.
  Live rank:
  `preflight_medium04_s143_yawm030.rank.json`, SHA-256
  `24DB7D9699EE19FE242A44AF0A90EB0BD701500569DBB284094F09DAD0360997`.
- The turn's exactly one image is the same configuration at frame 48:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_handwarp_medium04_s143_yawm030_gameplay_f48.bmp`,
  1280x720, SHA-256
  `17C7691AAD05656BA7B95CFBF8E4F9B7568722E973FA0367316FF48DE7B6F5F6`.
  The hash-bound capture log is
  `midori_casey_handwarp_medium04_s143_yawm030_gameplay_f48.log`, SHA-256
  `0E1EE244D23ED603DC5C2636A1C7F2DCFB33CC9F40FB2EFD611D8B0D84C93132`.
- The capture pose exactly matches preflight: guitar projection `0.997601`,
  minimum bilateral arm projection `0.785578`, head clearance `2.040010` head
  lengths, head/neck local rotations `10.555/16.132` degrees, left/right elbow
  bends `76.197/47.158` degrees, and both target errors zero. Gameplay ended
  playing at `30.833s`, score 100, two hits, and zero misses.
- Original-resolution review passes the immediate visual rejection criteria:
  correct Midori identity, plausible scale and floor contact, continuous
  torso/shoulders/neck/ankles, continuous bilateral arm chains, both hands on
  the instrument, diagonal across-body guitar placement, and no detached or
  stretched geometry. This is the first valid clone proof candidate; explicit
  user acceptance is still pending and must not be inferred.
- Added `tools/gh3_midori_validate_ghc_casey_gameplay_proof.py`. It hash-binds
  the inputs, log, BMP, serialized pose, and frame-48 live pose; validates
  identity, Casey controller graph, guitar attachment, gameplay state, no
  emulator/ISO use, image structure, and live arm/contact/head thresholds.
  Validation passes with zero failures at
  `midori_casey_handwarp_medium04_s143_yawm030_gameplay_f48.validation.json`,
  SHA-256
  `11B3315B1583C10FAED47BE5435E37F4CF86DB29307C1E19EFD0F47231F39E7C`.
- All work used loose `GEN` plus the addon, Windows Idle priority, and one
  worker. No ISO or PCSX2 process was used. Full Python regression passes
  `203/203`, nested `git diff --check` passes aside from existing line-ending
  warnings, and no runtime process remains. Keep the goal active pending user
  review. Do not run the final retail compatibility gate until the clone proof
  is accepted.
### 2026-08-30 clone package deployment and left-arm visibility replacement

- Active iteration is clone-only. No ISO was built or mounted and PCSX2 was
  not launched. All substantive processes used Windows Idle priority and one
  worker.
- `tools/gh3_midori_build_casey_clone_package.py` now builds and verifies the
  five loose outfit-1 assets directly in
  `GuitarHeroOGX-main-ui-engine/DLC/community.gh3.midori`. The read-only replay
  report is
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_handwarp_clone_package.validation.json`
  and passes with exact model/main/ui/fret/strum hashes, one outfit, and no
  extra package files.
- The deployed model is SHA-256
  `33E6CA3EAC0B1AC7EBFD2E56B34E5C7196CEDE2D2B3DCA4256A3317AB6637AC1`;
  the deployed main bank is
  `F7B202330E9233378BA55C896845DEEF1923C885174B7C0CF49097C115C17002`.
  Serialized inspection passes at 115 transforms, 20 character controllers,
  168 total meshes, 44 visible Midori meshes, and 124 hidden Casey template
  meshes. All 205 Python conversion regressions pass.
- A no-screenshot clone smoke loaded that exact in-repo manifest and package,
  played `stand_fast_02` from the deployed Midori main bank, reached playing,
  and scored two hits with no misses. Its stderr log is
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_handwarp_clone_package.smoke.log.stderr`.
- The user's left-arm concern rejects the prior small-club frame as visually
  ambiguous. Mesh accounting proves the 44-versus-45 count is repartitioning,
  not missing geometry: the current low-weight stage preserves all 4,992
  source faces with zero pruned triangles. The old 56-vertex/66-face right
  thumb `part42` is present in the current partitioning as `part41`.
- Exactly one replacement image was generated this turn, through `ghogx_app`
  from the in-repo loose package:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_fest_t305_yawm045_leftarm_gameplay_capture.bmp`,
  SHA-256
  `D3F0E022FD368E7EF90E681A0BA72250C7810DEADD644151AE7F2ACC02F65BAE`.
  It uses the brighter `fest` venue, neutral proof lighting, song time 30.5,
  and front-camera yaw -0.45 radians. Runtime confirms Midori outfit 1,
  168/44/115 model loading, the packaged main bank, and playing state. The
  actual selected body clip was `stand_fast_02`; do not attribute the older
  `stand_medium_04` pose metrics to this frame. User visual acceptance remains
  pending and the goal stays active.

### 2026-08-30 pinned left-arm pose remains visually ambiguous

- Extended `tools/gh3_midori_rank_casey_native_clip_poses.py` with a separate
  left-arm visibility score. It measures projected shoulder-to-elbow and
  elbow-to-wrist retention, elbow distance from the projected guitar axis,
  elbow bend, bilateral arm projection, guitar projection, and head clearance.
  The full Python regression suite passes `206/206`.
- The authenticated sweep evaluates 17,479 clip/sample/yaw combinations and
  finds 377 candidates that pass the stricter left-arm gate. Report:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/ten_clip_handwarp_sweep/left_arm_proof_pose_rank.json`.
  The selected body sample is `stand_fast_04` sample 43 (`1.433333333s`).
- A no-image clone preflight forced that exact body clip and time on the
  brighter `fest` venue, with both native hand-driver weights at `1.0`.
  A live yaw sweep selected `-0.3` radians. The captured runtime pose has left
  arm projection `0.948035`, left elbow-to-guitar-axis separation `0.416522`,
  left elbow bend `66.1504` degrees, right arm projection `0.789822`, guitar
  projection `0.991288`, head/neck local rotations `10.55485/16.13182`
  degrees, and zero wrist-target error on both sides.
- Exactly one new image was generated:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_fest_fast04_s43_yawm030_leftarm_gameplay.bmp`,
  1280x720, SHA-256
  `5E76CE9F1AD72C21021D6E64C76D936D98E4AB9E8CBAED2E5945D12363AAC14F`.
  Runtime loaded the in-repository Midori package, 168 total/44 visible meshes,
  115 Casey bones, the exact forced clip/time, and reached playing with one hit
  and zero misses.
- Direct original-resolution inspection does not promote this frame. The
  fretting forearm remains too close to the guitar neck against a dark, busy
  background, so its visual continuity is still ambiguous even though the
  joints, complete 4,992-face payload, and target errors are coherent in data.
  Treat it as a diagnostic rejection: geometry present, visual continuity
  unproven. Do not create a passing validation record for it.
- On the next image allocation, use a simpler, brighter stage sightline and a
  different pinned timestamp where the fretting forearm diverges visibly from
  the guitar neck. Keep the active goal open. No ISO or PCSX2 was used, all
  substantive work remained at Idle priority with one worker, and no clone or
  emulator process remains.

### 2026-08-30 Stone-stage capture confirms left wrist surface discontinuity

- Replaced the dark `fest` sightline with the brighter `stone` venue and
  pinned a different body pose: `stand_medium_06` sample 31
  (`1.033333333s`). The no-image preflight log is
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_stone_medium06_s31_preflight.log.stderr`,
  SHA-256
  `100DC2FB71B887A517ADCCFC9893EA1018424F6A2BD85083C5F12E5FF39D50E3`.
- Exactly one image was generated through the clone and its authenticated
  loose outfit-1 package:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_stone_medium06_s31_yawm025_leftarm_gameplay.bmp`,
  1280x720, SHA-256
  `5375248208E844D85E6D715FB3C3F825EA388BAE301D16D800B77D3DBA5011F6`.
  Its hash-bound runtime log is
  `midori_casey_stone_medium06_s31_yawm025_leftarm_gameplay.log.stderr`,
  SHA-256
  `DA40FE5FB67CFC19281781596D2BB058F723AFC4A1BB55F55DB85F81A4C46606`.
  The clone reached playing, scored 100 with two hits and zero misses, and
  used `strum_long_02` plus `finger_vibrato_middle` with zero target error on
  both hands.
- Direct original-resolution review rejects the frame as final proof. The
  neutral rock/amp background makes the anatomical left arm readable, and the
  sleeve/forearm-to-hand chain appears discontinuous. This is no longer an
  ambiguity caused by the previous dark background.
- Extended `tools/gh3_midori_rank_runtime_strum_visibility.py` to model the
  diagnostic camera's depth axis and score the forearm against the guitar
  axis. At the captured yaw of `-0.25` radians, the closest visible forearm
  probe is `3.131779` model units nearer the camera than the guitar axis, with
  `5.616700` units of projected clearance. The guitar therefore is not hiding
  the suspect forearm segment. The v4 depth report is
  `midori_casey_stone_medium06_s31_live_yaw_depth_rank.json`, SHA-256
  `E9DE20CDEDD2A0687B7F25F497942E13B457F0D955570701E8BE29BF02378C06`.
- A matching data-only controller replay produced
  `stone_medium06_s31_controller_pose.gh2posed`, SHA-256
  `C15BF3F71E2E59D96AB7F1C735D1297996D5F0ED37F6201032BFF1596F1FB612`.
  It serializes all 44 visible meshes. The left forearm/body surface chunk
  `part13` remains present, and its posed bounds end about `1.315` model units
  from the nearest left-hand surface bounds. Combined with all 4,992 source
  faces being preserved, this rules out wholesale mesh omission but supports
  a narrow forearm/hand bind-surface mismatch.
- Next conversion work should test a `forearms-hands` bind warp scope that
  includes forearms plus hand/finger chains while excluding upper arms,
  clavicles, shoulders, and torso. Do not reuse the rejected broad
  `upper-limbs` warp, which tore the shoulder/torso region. Validate the seam
  headlessly before allocating the next image.
- The full Python conversion suite passes `207/207`. All substantive work ran
  at Windows Idle priority with one worker. No ISO, PCSX2, clone, or emulator
  process remains. Keep the goal active; this Stone frame is diagnostic
  evidence, not an accepted gameplay proof.

### 2026-08-30 distal wrist ramp candidate and Arena review frame

- Rejected both broad forearm correction and reduced hand correction. The
  broad `forearms-hands` warp moved the sleeve surface in the wrong direction;
  lowering the hand warp below its full correction enlarged the visible gap.
  The retained conversion-level repair keeps the full hand/finger correction
  and applies a smooth 20% correction only across the distal 40% of each
  forearm. Elbows and every vertex outside the wrist scope remain unchanged.
- The current model is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/model_casey_lowweight_caseyrig_wristramp020/rock1.casey_lowweight_caseyrig_wristramp020.milo_ps2`,
  SHA-256
  `0B8C8B121513F78DE632BDF4B6366E54FF3C05F2A893D207A41347D0A793AD58`.
  All ten converted body clips pass topology/deformation validation. The
  Stone sample-31 left sleeve/hand axial gap falls from `1.314819` to
  `0.009563` model units, while maximum unscoped displacement is exactly zero.
  Hash-bound comparison:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/stone_medium06_s31_wristramp020_wrist_surface_compare.json`.
- `tools/gh3_midori_model_bundle.py` now supports the reproducible distal
  wrist ramp. `tools/gh3_midori_compare_posed_wrist_surface.py` records the
  named sleeve/hand axial bounds instead of treating coincident copied
  vertices as a meaningful nearest-pair result. The complete Python suite
  passes `209/209`.
- The exact model was deployed into the five-file loose clone package at
  `GuitarHeroOGX-main-ui-engine/DLC/community.gh3.midori`. Package validation
  passes at
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristramp020_clone_package.validation.json`.
  A fresh no-image clone smoke loaded 168 meshes / 44 visible / 115 bones,
  played the packaged main bank, reached gameplay, and ended with two hits and
  zero misses. Log:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristramp020_clone_package.smoke.log.stderr`,
  SHA-256
  `52045A9AAF27E6FA6E4F17F0484C99D4AB2F6293AA44BD8646321098508C57F7`.
- The turn's only new image switches from Stone to the warmer Arena and uses a
  different pinned pose: `stand_fast_03` sample 84 (`2.8s`), front yaw `0.0`.
  Path:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_arena_fast03_s84_wristramp020_gameplay.bmp`,
  1280x720, SHA-256
  `89962E54B2EDE14625A5ED411C92837758452D646175E9147D31C9119C5ED327`.
  Runtime log SHA-256 is
  `92F031BC07DC44786AF89C392742F3CDA91DFB45F9C6D5624695AC7612485467`.
  The log authenticates the in-repository loose Midori package, Arena,
  `stand_fast_03` fixed at `2.800`, full left/right hand-driver weights,
  playing state, and two hits with zero misses. The raised fretting arm is
  visible shoulder-to-hand against the orange stage art. Treat the image as a
  user-review candidate only; visual acceptance remains pending.
- No ISO, PCSX2, or emulator was used. Substantive processes ran at Windows
  Idle priority with one worker, and no runtime process remains. Keep the goal
  active.

### 2026-08-30 same-run live-pose Arena proof supersedes frame 48

- The first Arena frame at yaw `0.0`, capture frame 48, remains useful for the
  anatomical-left/fretting arm but is not the final proof. A same-pose
  no-image live replay measured that arm at `0.991936` projected while the
  opposite strumming arm was only `0.617978` projected. The validator rejected
  it rather than weakening the bilateral-arm threshold.
- A 51-frame no-image live-pose sweep evaluated 21 camera yaws from `-1.0` to
  `1.0` using the real gameplay controller output. The selected point is frame
  36 / song time `30.600`, yaw `-0.2`, still on `stand_fast_03` sample 84
  (`2.8s`). Its live metrics are: fretting arm projection `0.977968`, strumming
  arm projection `0.838564`, guitar projection `0.956729`, fretting-forearm
  guitar clearance `5.964777`, positive fretting-forearm depth margin
  `1.559055`, head/neck local rotations `10.554850` / `16.131819` degrees, and
  zero target error for both wrists. Data sweep:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_arena_fast03_s84_wristramp020_livepose_yaw_rank.json`.
- Exactly one new image was generated in this continuation turn:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_arena_fast03_s84_f36_yawm020_wristramp020_gameplay.bmp`,
  1280x720, SHA-256
  `25150D1111A9708705669637AC254CA7D17D137F170C4276E58404A23B711894`.
  The same capture log contains the exact frame-36 live pose before the
  screenshot marker, then exits in playing state at score 100, two hits, zero
  misses. Log SHA-256:
  `8F37D8149885A75212E068AA37A960643389D90D39061733BF030853E9AF64F0`.
- Updated `tools/gh3_midori_validate_ghc_casey_gameplay_proof.py` from the
  retired diagnostic-addon paths to the actual in-repository five-file loose
  package. Its v2 proof at
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_wristramp020_gameplay.validation.json`
  passes with zero failures and remains explicit that user acceptance is
  pending. SHA-256:
  `45581485FD2F6C18691A32AE50C887D1D37348045062E6548279764E2595BF66`.
  The package builder's strict verify-only mode now passes while requiring
  both that gameplay proof and the no-image smoke. Full Python regressions pass
  `210/210`.
- No ISO, PCSX2, or emulator was used. All substantive processes ran at
  Windows Idle priority with one worker. Keep the goal active until the user
  accepts a visual proof; the retail compatibility gate remains deferred.

### 2026-08-30 Stone sample-78 replacement isolates the left arm

- User rejected the Arena sightline as visually ambiguous because part of
  Midori's anatomical left arm appeared absent against the similar-colored
  stage art. The replacement uses a different venue and body timestamp:
  `stone`, `stand_medium_06` sample 78 (`2.6s`), capture frame 36 / song time
  `30.600`, and front-camera yaw `-0.4` radians.
- A no-image preflight first authenticated the exact loose outfit-1 package,
  168 total / 44 visible meshes, 115 Casey transforms, the requested clip and
  fixed time, Stone, and playing state. Its live frame-36 metrics are bilateral
  arm projection `0.854043`, left/right projection `0.897754/0.854043`, left
  forearm-to-guitar projected clearance `5.105417`, positive left forearm depth
  margin `4.049929`, and zero wrist-target error on both sides. Preflight log:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_stone_medium06_s78_wristramp020_preflight.log.stderr`.
- The turn's only new image is
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_stone_medium06_s78_f36_yawm040_wristramp020_gameplay.bmp`,
  1280x720, SHA-256
  `BBBDE77FFAA842480F3DCB96DD690A7794A9614DA0EFFF4115E60AC75856AAAA`.
  Its same-run log SHA-256 is
  `874601A3F8D8010E26B0139F0FDA04F3AA23DD79856614447EE8B80943A3FE33`.
  Original-resolution review can follow the raised left arm continuously from
  shoulder through upper arm, elbow, pale forearm, wrist, and hand.
- The canonical clone gameplay validation now points to this Stone candidate:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_wristramp020_gameplay.validation.json`,
  SHA-256
  `AF43F79E964B1E679C6351ECC701FBF199D4BC3681CC32264F559669AA61FB56`.
  It passes with zero failures, records assistant original-resolution review,
  and keeps user acceptance pending. Strict package verification requiring both
  clone smoke and gameplay proof also passes.
- No ISO, PCSX2, or emulator was used. All substantive processes ran at
  Windows Idle priority with one worker, and no `ghogx_app` process remains.
  Keep the goal active; retail compatibility remains deferred until clone
  visual acceptance.

### 2026-08-30 wrist-ramp model and loose package rebuild are automated

- Added `tools/gh3_midori_build_casey_wristramp_model.py`. It rebuilds outfit 1
  from the staged Midori geometry through four conversion-level stages:
  wrist-ramp meshbundle generation, base Midori MILO construction, rock1 slot
  rebase, and render-payload merge onto Casey's native graph. Every subprocess
  runs at Windows Idle priority with one worker.
- A clean `--verify-only` run reproduces all accepted binary hashes exactly:
  meshbundle
  `B1171709CA9D619B808F70A4C594ADC5736ABB896BBA412D8D0C223586D883AD`,
  base MILO
  `B898970BE4A03041324350F4B796AED599BBDAE576F916945D2E1AB650FA5BC6`,
  rebased donor
  `A4F26C863A71240280A935BEF80FF1FFA4D979A3C2994A76D94BA764CB3B90E9`,
  and final Casey-graph model
  `0B8C8B121513F78DE632BDF4B6366E54FF3C05F2A893D207A41347D0A793AD58`.
- Extended `tools/gh3_midori_build_casey_clone_package.py` with the opt-in
  `--rebuild-model` stage. This command now performs the entire model rebuild,
  publishes the exact outfit-1 loose package, and requires both clone proofs:
  `python tools/gh3_midori_build_casey_clone_package.py --rebuild-model --overwrite --require-clone-smoke --require-clone-proof`.
  The command passes in mode `rebuild-model-and-build-package`.
- Canonical model-rebuild report:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristramp020_model_rebuild.validation.json`,
  SHA-256
  `6F62A3C536F8D47A608B3CEA221DEB7DF8B0E799C979968019D6AB30B958CD9E`.
  Canonical integrated package report:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristramp020_clone_package.validation.json`,
  SHA-256
  `D4E67D8E8D20BAB5A2810BC05599DAC49E46C244F06AA45B07AF9E52D008ED9D`.
- A separate post-build readback with `--verify-only --require-clone-smoke
  --require-clone-proof` also passes without rewriting the canonical integrated
  report. Its report is
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristramp020_clone_package.strict_verify.validation.json`,
  SHA-256
  `858C4383A2EBBA3A721DF64D3BEEA4CFF8F77ECAB05486930C1B658B84D16DB3`.
- Added focused coverage for the pinned wrist-ramp parameters and opt-in package
  rebuild command. The complete Python conversion suite passes `212/212`.
  No image was generated in this continuation. No ISO, PCSX2, emulator, clone,
  or Python process remains. Keep the goal active: the Stone screenshot still
  awaits user acceptance, and retail compatibility remains deferred until that
  clone visual gate is accepted.

### 2026-08-30 animation-call naming is mandatory in the package gate

- The existing `analysis/gh3_midori_animation_call_compatibility.json` was
  stale against the current five-asset package. Its old green status could not
  be reused; `tools/gh3_midori_animation_call_compatibility.py --verify-only`
  correctly rejected it.
- Regenerated the report directly from the current package and the
  provenance-bound untouched retail Casey entries in `MAIN.HDR` / `MAIN_0.ARK`.
  Current coverage is `157/157` exact stock clip-call names and `30/30`
  `CharClipGroup` names, with zero missing calls, zero missing groups, and ten
  intentionally replaced main clips. Published main/UI/fret/strum hashes match
  the package's authenticated hashes exactly. Report SHA-256:
  `2DB75CEA0EEAD02B231BC09535D25C3FE122267E216F5AD3BD699D2E527414FA`.
- Extended `tools/gh3_midori_build_casey_clone_package.py` so every build
  regenerates and validates the animation-call report, while every
  `--verify-only` run requires it to be byte-current. The package build now
  fails closed on an incomplete status, any failed call-surface check, totals
  other than 157 clips / 30 groups with zero missing, or drift in any of the
  four published animation-bank hashes.
- The strict one-command rebuild/package/proof chain passes with the call
  surface embedded in its canonical report:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristramp020_clone_package.validation.json`,
  SHA-256
  `4E3C27A9047C877C755EAB606F8482D6B390C0A9D86077B1062F1C4AF9198DCC`.
  A separate verify-only readback also passes at
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristramp020_clone_package.call_surface_strict_verify.validation.json`,
  SHA-256
  `8A61CCA89ED8ACB8F9E44651799C12F06A6568DE0B8D8DA5FDC73C91D81850FB`.
- Added fail-closed tests for call totals, group totals, bank hashes, and stale
  verify mode. The complete Python conversion suite passes `213/213`. No image
  was generated, all work remained at Idle priority with one worker, and no
  process remains. Keep the goal active pending user acceptance of the Stone
  clone screenshot; do not cross the deferred retail execution gate yet.

### 2026-08-30 ten-clip main bank rebuild is automated

- Added `tools/gh3_midori_build_casey_main_bank.py`. It reconstructs the
  accepted Casey-native ten-clip main bank from the final ACP inputs while
  preserving the stock Casey runtime/controller graph. The authenticated donor
  SHA-256 is
  `47EC8FC82014376FB597129B1D47289D060A9E65BD19D33840A9065393FD06FB`;
  the reproduced main-bank SHA-256 is
  `F7B202330E9233378BA55C896845DEEF1923C885174B7C0CF49097C115C17002`.
- Canonical rebuild report:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_main_bank_rebuild.validation.json`,
  SHA-256
  `7F50AF415D632A3968AB250D433D08228AF1A9802CBF547B37887E3DE7A8C2F4`.
  Independent strict readback:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_main_bank_rebuild.strict_verify.validation.json`,
  SHA-256
  `F6B26D2D9AF8973F5BDCC8AF58BAF37BCA43D1038617E8CE32C54C2762A2DFCB`.
- Extended `tools/gh3_midori_build_casey_clone_package.py` with
  `--rebuild-main`. The definitive automated rebuild/package command is now:
  `python tools/gh3_midori_build_casey_clone_package.py --rebuild-model --rebuild-main --overwrite --require-clone-smoke --require-clone-proof`.
  Its integrated report SHA-256 is
  `DC2294F52BFBD61280FE05597D575F1F63BDB8EC6D8A1888B6704FDE534956B7`.
- The full Python conversion suite passes `215/215`. No ISO, PCSX2, or emulator
  participated; all substantive work ran at Windows Idle priority with one
  worker.

### 2026-08-30 Big-stage left-arm diagnostic rejects the static wrist ramp

- At the user's request, changed both venue and timestamp to isolate the
  anatomical-left/fretting arm against higher-contrast art: venue `big`, clip
  `stand_medium_04`, serialized sample 138 (`4.6s`), capture frame 29 / song
  time `30.483`, and camera yaw `-0.4` radians. The no-image live sweep selected
  this frame with guitar projection `0.986269`, bilateral arm minimum
  `0.876600`, left/right projection `0.880826/0.876600`, projected left
  forearm-to-guitar clearance `4.670501`, positive depth margin `4.071182`, and
  zero target error at both wrists.
- The one image generated in the turn is
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_big_medium04_s138_f29_yawm040_wristramp020_gameplay.bmp`,
  1280x720, SHA-256
  `48583CB6099D7EAFE95CB1A85F7E0DCCA7B171D2376F0240A7E9CA84267231AA`.
  Runtime log SHA-256 is
  `85E3171A252AA97ED9765BA7EAE7439B85046920D5613BEAC2AF53982A075D35`.
  Original-resolution inspection clearly rejects it: Midori's raised left arm
  reads discontinuously between the forearm/cuff and the fretting hand. Treat
  this as diagnostic evidence, never as an accepted gameplay proof.
- Headless deformation of the exact sample 138 proves that the prior static
  positional wrist ramp overfit sample 31. Its left sleeve-to-hand axial gap is
  `8.524582` before the ramp and `0.764313` after it, compared with only
  `0.009563` at the pose used to tune the ramp. Unscoped maximum displacement
  remains exactly zero. Rejected comparison report:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/medium04_s138_surface/medium04_s138_wristramp020_wrist_surface_compare.json`,
  SHA-256
  `FB9CD3CE4DF0AD638335F1926E4060FF103F91A72A3A4B434B5893E0FCCC5755`.
- Source-stage inspection identifies a conversion-level skin-weight
  discontinuity, not missing geometry: all 4,992 source faces survive. The
  sleeve/forearm chunk's distal vertices are 100% `bone_L-foreArm.mesh`, while
  the adjacent hand-side chunk abruptly changes to mixed forearm/hand weights.
  A pose-specific vertex warp cannot remain closed under different hand
  transforms.
- Next action is an opt-in localized forearm-to-hand weight ramp in
  `tools/gh3_midori_model_stage.py`, before four-bone palette partitioning.
  Prove it over multiple poses with headless deformation and exact face/weight/
  scope gates before producing another image. Keep face static, outfit 1 only,
  and preserve all Casey-native runtime and animation-call naming surfaces.
- The canonical clone gameplay proof is intentionally rejected/pending after
  this diagnostic. Do not restore a passing status merely to satisfy the
  package gate; a new coherent image must earn it. The goal remains active.
  No ISO, PCSX2, or emulator was used, all substantive work ran at Windows Idle
  priority with one worker, and no runtime process remains.

### 2026-08-31 source-driven wrist seam repair and Arena proof

- Replaced the rejected pose-specific wrist-position ramp with a conversion-stage
  skin-weight transfer in `tools/gh3_midori_model_stage.py`. For each side it
  changes only forearm-only vertices in the distal 20 percent of the authored
  elbow-to-hand bind axis, approaches the nearest existing authored
  forearm/hand transition blend, and runs before four-bone palette partitioning.
  It changes 42 vertices per arm (84 total), preserves all 4,992 source faces,
  positions, normals, UVs, source vertex identity, normalized weights, and the
  four-bone maximum, with no dropped source weight.
- The current outfit-1 Casey-graph model is
  `analysis/gh3_midori_frame106_headless/body_deformation_v5/model_casey_lowweight_caseyrig_wristweights_s080_w100_ref/rock1.casey_lowweight_caseyrig_wristweights_s080_w100_ref.milo_ps2`,
  SHA-256
  `C8B7BE6DEF202AB60B0E71563924B11C687DD8C947A7535436E19D81B8CEA286`.
  Its contract is 45 active Midori meshes, 169 total / 45 visible meshes, 115
  Casey transforms, and 20 Casey controllers; all 124 template meshes remain
  hidden. The deterministic rebuild report is
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristweights_model_rebuild.validation.json`,
  SHA-256
  `94FDA73E7FB93A7FCE361036C16F5254D8FACFB6E0B9972F21ED54A2169E4023`.
- A no-image, ten-clip, five-pose-per-clip runtime sweep passes 50/50 poses.
  Worst seam-pair mean distance is `0.559708` left / `0.554838` right; worst
  individual distance is `1.641844` left / `1.760597` right. Report:
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/wrist_seam_weight_transfer/wrist_seam_transfer_s080_w100_ref_r000.validation.json`,
  SHA-256
  `2DAF75EAE09957756D2A20B4CECD60FA75BC3D8A3ED69367792A5C032D5A452F`.
- Added `tools/gh3_midori_run_casey_clone_pose.py`, a clone-only deterministic
  pose runner that refuses ISO paths, uses the in-repository loose DLC package
  plus extracted GEN assets, and forces Windows Idle priority / one worker. It
  runs the same command with or without screenshot arguments, so the no-image
  preflight and final capture cannot silently diverge.
- The current visual candidate switches both venue and timestamp from the
  rejected Big-stage proof: Arena, `stand_fast_03` sample 84 (`2.8s`), capture
  frame 30 / song time `30.500`, camera yaw `-0.6`. Live data reports left/right
  arm projection `0.913158/0.940696`, elbow bends `54.724964/43.848737`
  degrees, left forearm-to-guitar projected clearance `4.825295`, positive left
  forearm depth margin `4.043913`, and zero wrist-target error on both sides.
  Screenshot:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_arena_fast03_s84_f30_yawm060_wristweights_gameplay.bmp`,
  1280x720, SHA-256
  `02182B80DAF8FD53E3A9CC97AE23514C5E72FE89258B40283FC797CF66576617`.
  Same-run log SHA-256:
  `ED69629446415342F283FF5BE87B758D94BF53D047CB7AA6B26B1F329AF586CB`.
  The log authenticates the current 169/45/115 model, requested Midori clip,
  screenshot marker, live song state, two hits, and zero misses.
- Updated `tools/gh3_midori_validate_ghc_casey_gameplay_proof.py` to the current
  wrist-weight model and exact Arena run. Its v3 report passes with no failures
  while explicitly retaining `user_acceptance: pending`:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_wristweights_gameplay.validation.json`,
  SHA-256
  `952BCC7D37DD80AF7916DDE022FD77603FF67E6006127C60721A6A667897FA1E`.
  Package verify-only mode requiring both this proof and the independent smoke
  passes at
  `analysis/_midori_patch_identity_audit/casey_native_animation_bridge/casey_wristweights_clone_package.proof_strict_verify.validation.json`,
  SHA-256
  `D2AE810AA6A1FC5AC1747B8D70DEB8ECE0EBC8CB6E29780F4C6582CFA9756DF1`.
  Animation-call compatibility remains 157/157 clips and 30/30 groups.
- The complete Python conversion suite passes `216/216`. No new screenshot was
  generated after the Arena candidate, and no ISO, PCSX2, or emulator was used.
  Keep the goal active. User visual acceptance remains pending, and the retail
  execution gate remains deferred until that clone visual gate is accepted.

### 2026-08-31 reproducible pipeline promoted and clone runner lock fixed

- Promoted the dependency-complete outfit-1 conversion into the nested
  GuitarHeroClassic repository. The canonical command is now
  `python tools/gh3_midori_build_casey_clone_package.py --rebuild-model
  --rebuild-main --overwrite`; it uses repository-relative paths, Windows Idle
  priority, one worker, loose DLC files, and no ISO or emulator.
- A clean repository-local rebuild and verify reproduced the exact current
  model SHA-256
  `C8B7BE6DEF202AB60B0E71563924B11C687DD8C947A7535436E19D81B8CEA286`
  and main-bank SHA-256
  `F7B202330E9233378BA55C896845DEEF1923C885174B7C0CF49097C115C17002`.
  Package inventory, 45 active Midori meshes / 169 total / 115 transforms / 20
  controllers, all 157 Casey clip calls, and all 30 Casey groups pass. The C++
  catalog test also passes with one Midori model, one texture, and 157 clips.
- Root cause of the apparent game lock was in the proof harness: screenshot
  runs were hidden automatically, but no-image preflights were not. A pinned
  animation sample and fixed camera could therefore appear as a frozen game
  window. `tools/gh3_midori_run_casey_clone_pose.py` now sets
  `GHOGX_HIDE_WINDOW=1` for every automated run, keeps the child at Idle
  priority, limits worker libraries to one thread, bounds every run by frame
  count and timeout, and kills the child on timeout.
- The proof runner no longer requires exactly two autoplay hits. It requires
  live playing state, at least one hit, and zero misses, so the existing
  34-frame capture is not falsely rejected merely because it ends before the
  second chart event. It also supports `--verify-existing`, allowing a
  hash-stable log/screenshot pair to be checked without launching the clone.
- The existing Big-stage candidate was rechecked with `--verify-existing` and
  passes: hidden D3D9 capture, exact Midori package, `stand_medium_04` at
  `4.766666667s`, `big`, yaw `-0.4`, frame 30, a bounded 34-frame exit, one hit
  and zero misses, and 1,938 post-controller hand-pose rows. No game process
  was launched and no image was generated during this verification.
- Focused Python tests pass `11/11`; syntax checks pass; the relevant MILO
  conversion tests pass `3/3`. An unfiltered CTest invocation also reported
  six unrelated test executables as not built/Not Run in this partial build
  tree; the explicit MILO selection is green.
- Pushed commits `2e00ff13` (`Add reproducible Midori conversion pipeline`) and
  `db9f0de9` (`Harden Midori clone proof runner`) to
  `GuitarHeroClassic/main-ui-engine`. User visual acceptance of the existing
  Big-stage image remains pending. Keep the goal active and do not cross the
  deferred retail GH2 gate until that visual gate is accepted.

### 2026-08-31 gameplay proof is now hash-bound to the package gate

- Extended the repository-local clone pose runner with
  `--validation-output`. A successful screenshot run or `--verify-existing`
  readback now writes `gh3-midori-casey-clone-gameplay-proof-v1`, containing
  the exact model, main-bank, manifest, runtime-log, and screenshot SHA-256
  values plus the requested venue, clip, time, camera yaw, frame limits,
  gameplay summary, and all runtime checks. It explicitly records
  `user_acceptance: pending` rather than conflating data authentication with
  visual approval.
- Generated the canonical ignored proof at
  `GuitarHeroOGX-main-ui-engine/out/midori/proof/midori_casey_wristweights_gameplay.validation.json`,
  SHA-256
  `BE889B2A9D1ED424778B9106AC0B0C2387F5C386F349A0D6925C659D082EBA7F`,
  from the existing Big-stage pair without launching the clone or generating
  another image.
- Bound screenshot SHA-256
  `60EFC436916BB2609D2B42E09EC433AB4E1D7B8DCC8E24FCBB4775C05753072D`
  and runtime-log SHA-256
  `4A809F2E06B21A0861168A0D9742DF2FECAA23C3D385C26AFBA0B93E28A54D4F`.
  The package verifier now re-hashes both referenced artifacts and fails closed
  if either is missing or changed; it also requires the expected proof format,
  hidden-window state, and every recorded runtime check to be true.
- `python tools/gh3_midori_build_casey_clone_package.py --verify-only
  --require-clone-proof` passes with `artifact_hashes_exact: true`,
  `all_checks_pass: true`, `authenticates_package: true`, zero failures, and
  `user_acceptance: pending`. Focused Python tests pass `13/13` including a
  deliberate post-proof screenshot mutation that the gate rejects.
- Pushed commit `eff3d007` (`Bind Midori gameplay proof to package hashes`) to
  `GuitarHeroClassic/main-ui-engine`. No game process ran and no image was
  generated. Keep the goal active; user visual acceptance of the existing
  Big-stage screenshot and the subsequent retail gate remain outstanding.

### 2026-08-31 Casey-path static retail overlay is automated

- Added a static retail-layout stage to the repository-local package builder.
  `--stage-retail-overlay` copies the five already-authenticated payloads to
  `out/midori/retail_casey_overlay` under Casey's native GH2 archive paths:
  `char/rock1/og/gen/rock1.milo_ps2` and
  `char/rock1/anims/gen/rock1_{main,ui,fret,strum}.milo_ps2`.
- The overlay deliberately contains no addon manifest, ARK, ISO, mount state,
  or emulator configuration. Its validator requires exactly those five files,
  checks each against both its source and pinned expected SHA-256, and rejects
  missing, changed, or extra files.
- The real stage command passed at Windows Idle priority with one worker:
  `python tools/gh3_midori_build_casey_clone_package.py
  --stage-retail-overlay --overwrite --require-clone-proof`. The independent
  readback also passed:
  `python tools/gh3_midori_build_casey_clone_package.py --verify-only
  --require-retail-overlay --require-clone-proof`.
- The readback reports an exact five-file inventory, all five
  `source_exact: true`, a passing hash-bound clone proof, zero failures,
  `iso_built: false`, `iso_mounted: false`, and `emulator_used: false`.
  Focused Python tests pass `14/14`, including exact inventory and deliberate
  overlay mutation rejection.
- Pushed commit `18dc36bc` (`Stage Midori at Casey retail paths`) to
  `GuitarHeroClassic/main-ui-engine`. This prepares the retail replacement
  layout but does not cross the deferred retail execution gate. Keep the goal
  active pending user visual acceptance of the existing Big-stage screenshot.

### 2026-08-31 retail ARK targets and overlay manifest are authenticated read-only

- Added `tools/gh3_midori_casey_retail_overlay_manifest.py` and integrated it
  into the package builder's retail stage. It reads only the supplied GH2
  `MAIN.HDR` plus the five Casey entries in `MAIN_0.ARK`; it does not hash or
  rewrite the full ARK.
- The source-archive gate requires Harmonix v3, one ARK part, declared and
  actual part sizes to match, all five Casey paths to exist with exact case,
  each staged stock source to be byte-identical to its archive entry, and each
  replacement to match the pinned conversion hash. It additionally requires
  model/main to change while UI/fret/strum remain stock.
- The actual supplied archive passes every check. The later overlay operation
  will replace two entries and reuse three, appending `3,368,834` bytes. The
  projected ARK size is `3,122,025,705`, below the v3 32-bit limit. Dedicated
  `ark_v3_test` also passes and confirms replacement `inflated_size = 0` is the
  correct uncompressed-entry convention.
- Generated the exact `ark_tool overlay` TSV at
  `GuitarHeroOGX-main-ui-engine/out/midori/retail_casey_overlay.tsv`, SHA-256
  `10D5EFD839D87AE420A83C09C456E70011D4D784DAE14B196C26E073F2934C2B`.
  Source-archive report:
  `out/midori/retail_casey_overlay.source_archive.json`, SHA-256
  `4EE5CDBAF3B952A493205DFFB4BC56D40604888434AEE35BDD4E7E0C80233962`.
- Independent package `--verify-only --require-retail-overlay
  --require-clone-proof` reproduces both files exactly and passes with zero
  failures. Focused Python tests pass `15/15`.
- Pushed commit `e43535cd` (`Authenticate Midori retail ARK targets`) to
  `GuitarHeroClassic/main-ui-engine`. No ARK, ISO, mount, emulator, game
  process, or image was created. Keep the goal active at the existing visual
  acceptance gate; only after acceptance should the manifest be applied to a
  copied retail archive for the execution test.

### 2026-08-31 copied-ARK apply is automated behind human acceptance

- Extended `tools/gh3_midori_run_casey_clone_pose.py` with
  `--user-acceptance {pending,accepted}`. The default remains `pending`; only
  use `accepted` after an explicit user visual verdict while regenerating the
  existing hash-bound proof.
- Added `tools/gh3_midori_apply_casey_retail_overlay.py`. It refuses to proceed
  unless the proof format/status, clone-only runtime, all runtime checks,
  screenshot/log hashes, model/main hashes, and explicit human acceptance all
  pass. It also refuses output paths that alias the supplied source archive.
- After acceptance, `--apply` will copy `MAIN.HDR` and `MAIN_0.ARK` to
  `out/midori/retail_gate/GEN`, apply the authenticated five-entry TSV with
  `ark_tool`, then read all five Casey entries back and require exact
  replacement hashes, preserved entry count, exact projected size, and a
  valid v3 header. `--verify-only` independently rechecks the copied result.
  Neither mode constructs an ISO or invokes an emulator.
- Exercised the real current negative gate at Idle priority. With the proof
  still pending, `--apply` exited specifically on
  `human_visual_acceptance`; neither copied `MAIN.HDR` nor `MAIN_0.ARK`
  existed before or after. The multi-gigabyte copy and ARK mutation were not
  started.
- Focused Python tests pass `17/17`, including pending-versus-accepted proof
  logic and copied-path command composition. Pushed commit `da03f95f`
  (`Gate Midori retail apply on visual acceptance`) to
  `GuitarHeroClassic/main-ui-engine`.
- The automation path is now ready but intentionally closed. Keep the goal
  active and the proof at `pending` until the user accepts or rejects the
  existing Big-stage screenshot.

### 2026-08-31 01:33:57 -04:00 final rejection and retirement

- The user hard-rejected the final Big-stage GuitarHeroClassic proof. The
  observed failures are floating legs, stretched arms, and an unnatural pose.
  This invalidates the prior assistant assessment that the result looked
  coherent. The conversion objective is not complete.
- Regenerated the existing hash-bound proof in `--verify-existing` mode with
  `user_acceptance: rejected`; no game was launched and no new image was
  generated. The proof keeps `status: pass` only for its runtime and artifact
  checks, while both `assistant_review` and `user_acceptance` are `rejected`.
- Rejected screenshot:
  `analysis/_midori_patch_identity_audit/ghc_casey_slot/final_clone_gameplay_proof/midori_casey_big_medium04_s143_f30_yawm040_wristweights_gameplay.bmp`,
  SHA-256
  `60EFC436916BB2609D2B42E09EC433AB4E1D7B8DCC8E24FCBB4775C05753072D`.
  Bound log SHA-256:
  `4A809F2E06B21A0861168A0D9742DF2FECAA23C3D385C26AFBA0B93E28A54D4F`.
- Added a first-class `rejected` proof verdict. The copied-ARK retail gate
  still requires the literal value `accepted`, so pending and rejected proofs
  both fail closed. No retail ARK copy exists; no ISO was built or mounted;
  no emulator was used; no game process remains running.
- Final focused contract suite passes `17/17`. A real rejected-proof `--apply`
  attempt exited only on `human_visual_acceptance`; both
  `out/midori/retail_gate/GEN/MAIN.HDR` and `MAIN_0.ARK` were absent before and
  after, proving the multi-gigabyte copy never began.
- Archived the exact screenshot and log, rejected proof, package/model/main
  reports, animation-call report, wrist analysis, retail TSV/source report,
  complete handoff, and retirement notes. Commercial archives, generated MILO
  intermediates, and unrelated dirty game files are intentionally excluded.
- Retirement work is isolated on branch
  `archive/midori-conversion-pre-5.7` in the same GuitarHeroClassic repository.
  Do not stage or revert the unrelated dirty game/proof changes present in the
  shared worktree.
- Treat every earlier statement that legs, torso, neck, or head were solved as
  unproven. On a future restart, begin with pelvis/root grounding and posed foot
  contact, then trace arm stretch through bind matrices, weights, and IK from
  clavicle outward. Validate numerically across multiple frames before using a
  screenshot as a review gate. Keep face static, outfit 1 only, guitar last,
  CPU priority Idle, one worker, and GuitarHeroClassic as the runtime target.
- This work is retired pending a future model attempt. Leave the active goal
  blocked/retired rather than marking it complete.
